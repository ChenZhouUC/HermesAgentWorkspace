...
- After editing patched source in `hermes-agent/`, regenerate the diff and verify it applies cleanly to a fresh HEAD:
  ...
  git apply --cached --check ~/.hermes/patches/local-patches.diff
  git apply --check --reverse ~/.hermes/patches/local-patches.diff
  ```
...
- Run `~/.hermes/hermes-agent/venv/bin/python3 ~/.hermes/scripts/test_patch_evidence.py --report-json <path>`. Each active engineering PATCH validation function must resolve to one full pytest node ID from a test file declared by that PATCH, produce a clean passed JUnit outcome, and collectively execute every owned Python production file; shared nodes, undeclared test files, partial implementation coverage, ambiguous names, missing collection, skip, xfail, error, or failure are release-blocking. Runtime, dedicated, archive, and external-verifier PATCHes must have a registered callable probe that executes exactly once in the current full run and leaves a `probe_results` receipt; prose tokens or an unconditional `contract_passed` status are not evidence.
...
- `~/.hermes/hermes-agent/venv/bin/python3 ~/.hermes/scripts/test_patch_evidence.py` — all active/archive definitions have exact four-section shape and current executable evidence; patched tests collect successfully; every non-pytest PATCH has a fresh registered probe receipt.
- `bash ~/.hermes/hermes-update.sh --final-audit --json` — after the last reconcile and all script/doc edits, the per-PATCH matrix, canonical suite with `--file-retries 0`, replay closure, docs/Wiki, runtime verifier/PIDs, transaction, and final cleanup all report `status=ok / mode=full`.
