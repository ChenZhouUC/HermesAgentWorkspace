...
本项目维护若干针对上游 `hermes-agent` 的本地补丁，以修复已知 Bug 或定制行为。完整记录（问题描述 / 根因 / 修复方案）见 [`patches/PATCHES.md`](patches/PATCHES.md)。当前补丁基线已刷新到上游 `a6a28ce3`（截至 2026-06-26 的 `main`，较 `5ecf3bf0` 前进 240 commits，release tag 维持 `v0.17.0 (2026.6.19)`），本轮新增本地 **PATCH-14（people-profile 人物画像注入）**。
