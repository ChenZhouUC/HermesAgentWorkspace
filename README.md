...
本项目维护若干针对上游 `hermes-agent` 的本地补丁，以修复已知 Bug 或定制行为。完整记录（问题描述 / 根因 / 修复方案）见 [`patches/PATCHES.md`](patches/PATCHES.md)。当前补丁基线已刷新到上游 `0f81b0d4`（截至 2026-06-26 的 `main`，较 `233ef98a` 前进 31 commits，release tag 维持 `v0.17.0 (2026.6.19)`）；**本轮无 patch 变更**——10 条活跃补丁全部 clean apply 到新基线、`local-patches.diff` 零语义漂移（仅 `index`/`@@` 元数据 rebase），本地改动均在 config-repo（群信息单一入口 `groups.yaml`、群聊 skill 白名单、`people.yaml` aliases、新增 `feishu-people-search` skill），不触及 `PATCHED_FILES`。
