...
| 步骤 | 操作                                      | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---- | ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | Preflight + transaction                   | 确认 hermes 可用、git 仓库存在、index 干净；先运行 `--self-test-patch-gates`，证明 Step 8b 的 34 个活跃工程 gate 与 6 个归档 gate 都有成功路径且被 Step 8c 聚合条件实际消费；`--update` 仅在无未完成事务时开放一次官方获取，默认/`--reconcile` 固定当前事务 SHA 且不做网络探测。staged 用户改动或 gate 集合漂移在任何 mutation 前 fail closed                                                                                                                                                                                     |
...
本项目维护 42 个按职责命名的活跃语义补丁：34 个工程内补丁、7 个升级期运行时补丁和 1 个外层配置仓库插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 full-index `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；`--self-test-patch-gates` 在每轮 preflight 自动证明所有活跃/归档 gate 的声明、成功赋值和 Step 8c 消费集合完全一致；`PATCH-UPDATE-TRANSACTION-PIN` 保证一次升级只取得一个 upstream snapshot，后续围绕固定 SHA reconcile；已归档的 `PATCH-LAUNCHD-WRAPPER-SUPERVISOR` 由上游实现与 Step 8b sentinel 继续监管；`PATCH-ENV-AMBIENT-CREDENTIAL-ISOLATION` 阻止 shell/`~/.secrets` 凭据越界进入 Hermes；`PATCH-MODEL-CONFIGURED-ONLY` 让 `/model` 只展示并切换 config 中的主模型/fallback；`PATCH-DOCTOR-TEST-NETWORK-ISOLATION` 让 doctor 单测不再继承宿主网络与凭据；`PATCH-MCP-TASKS-ASYNC-HANDOFF` 让标准 MCP task handle 确定性回执并结束当前 turn；`PATCH-GATEWAY-FAILOVER-STATUS-SILENCE`、`PATCH-TRUNCATED-TOOL-CALL-RECOVERY`、`PATCH-FEISHU-RESPONSE-BUDGET` 与 `PATCH-TOOL-CALL-DOUBLE-WRAP-RECOVERY` 共同守住群聊模型路由、长工具参数、回复长度和 deferred-tool 调用边界；`PATCH-GATEWAY-RESTART-CLEANUP` 在每次 restart 前审计并清理外层脚本与 outer/inner ignored 产物；`PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e 强制 verifier 管理，并用真实 `[URL](URL)` 引用形态区分可信身份与目标 provenance。当前基线为上游 `13ce0c5c675e843af70d19c9e5144249cd51c8d1`，85 个受管文件的内层 diff 与外层 bundle（排除已知 `package-lock.json` 归一化漂移）逐字节一致；最近完整回归结果见本周版本记录，sandbox verifier 同时证明 launchd wrapper 健康并把插件注册 trace 绑定真实 Gateway 子进程 PID。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------ | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v0.20.4            | 2026-08-20 | **2026-W34**：upstream `8ad055414 → 13ce0c5c6`（+836 commits），版本 v0.20.1 → v0.20.4；主线迁移 MCP 2.x + stateless protocol/OAuth，新增 project-local skill trust/quarantine 与安装安全，修复 profile-aware Gateway/session、launchd wrapped child supervisor，并持续扩展 provider catalog、Desktop 多 source/Bot Mode/preview/browser/tour 与 Nix Home Manager。周内本地先建立 `PATCH-MCP-TASKS-ASYNC-HANDOFF`（task handle 持久化后确定性回执、无第二次 LLM）和 HyperTeX 群 ID + 用户 ID 双 allowlist，随后新增 `PATCH-GATEWAY-RESTART-CLEANUP` 三态审计。升级时 84-file bundle 在 prompt/skill、gateway、approval、MCP 五处真实冲突；四处按上游新能力与本地不变量并存重解，`PATCH-LAUNCHD-WRAPPER-SUPERVISOR` 经 `7008fb81b3` 与 14 条官方回归证明完全吸收并归档。cleanup 续审补齐事务状态、skills prompt snapshot keep 与新 `website/**/__pycache__` remove 分类；MCP Tasks extension 先适配 2.x `is_error` 与 request `name_param` 契约，再用真实 SDK 2.x legacy session 修复 server 已创建 task、client 却被 core `CallToolResult.content` 预校验误报失败；08-20 复盘财务/Data Pipeline 两群后又区分出两类独立故障：Hermes 以 exact-task schema + `suggestedTaskId` 只读单次重试阻止 job/case ID 冒充 taskId，HyperTeX 则隔离 worker/RuntimeAgent stdin，防止子进程抢读 MCP `tasks/get` 帧造成 30 秒超时。随后复盘 SpaceSight 国内业务群创建需求文档失败：新增 `PATCH-GATEWAY-FAILOVER-STATUS-SILENCE` 隐藏聊天面的模型路由旁白，`PATCH-TRUNCATED-TOOL-CALL-RECOVERY` 让被 router 标成 `tool_calls` 的未闭合大参数执行 8k→16k→32k 有界重试，`PATCH-FEISHU-RESPONSE-BUDGET` 抽出 DM/group `response_char_limit`（本机均为 3000）并把单条 post 兜底提高到 16k、超大内容仍保留安全切分。财务群 PDF 任务复盘又新增 `PATCH-TOOL-CALL-DOUBLE-WRAP-RECOVERY`，安全修复模型把 HyperTeX deferred tool 冗余包在 `tool_call` 内一层的格式偏差；群长任务启用 180 秒无内部细节的 generic 心跳。HyperTeX 与群文档 delete 授信扩为周琛、孙可天、张文华、李冰洁四人（各登记 open_id + user_id），create/append/rebuild 对群成员开放且不扩大 assistant-user mention trigger；HyperTeX create 不再强制 `codex`，改由 `hermes` 账号权重抽取，iterate 沿用 case 原 Agent。终态 **42 active / 34 engineering / 85 files / 39 tests**，受管 runner **1654 passed / 0 failed / 3 skipped**（另 8 subtests），sandbox **42 passed**；可信维护者删除文档时的 Gateway Markdown `[URL](URL)` provenance 已修复，并穿过 deferred bridge + handler 回归；quote-chain/compaction 两个漏接的 Step 8c gate 已补齐，preflight 自测覆盖 34 个活跃工程 gate + 6 个归档 gate；85-file bundle 闭环在本轮刷新后全绿。root 仍 6 high，均为 Electron/postcss/nanoid 上游 lock/range 阻挡的 Desktop/Web build-chain P2，不影响飞书主链路，未使用 `--force`。 |
