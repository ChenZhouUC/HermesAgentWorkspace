...
- `pre_gateway_dispatch` 把入站消息的 `(platform, chat_id, chat_type, user_id)` 与**当前消息/明确引用消息**中的飞书资源 token 写入 `contextvars.ContextVar`，asyncio 会自动把该 context 传到所有后续 `await` / `create_task` 子任务里。历史回填 `channel_context` 不授予资源访问，避免后来的参与者复用旧群消息里的链接。
- `pre_tool_call` 读取 ContextVar：若 `platform != "feishu"` 直接放行；若 `chat_id` 在 owner 白名单里直接放行；群聊优先应用群专用 allowlist，不继承 outsider-DM 的 `vision_analyze` / `image_generate` 放行；其他私聊才使用基础安全 allowlist。配置解析失败时对 Feishu fail closed。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------ | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v0.20.1            | 2026-08-16 | **2026-W33**：upstream `cd9fbf9f1 → 45af7a71f`（周内 +1745 commits）。主线：周初完成 CVE、v34 personality、Gateway route/session、Browser Use 与 Windows/Desktop updater 收敛；随后补齐跨 surface `/model`/resume、后台通知与 secrets redaction、compression/model catalog、Slack streaming/task cards；本轮继续加强 updater stale-lock/shallow/native recovery、Gateway/cron drain 与 stale-claim、`key_cmd` secret source、`/save`/`/export`，以及 Desktop 多实例、多源 agent、未读状态、查找、附件和命名浏览器 session。PATCH：周内 **+8 / 2 归档 / 0 收缩**，终态 **36 活跃**；除 launchd supervisor P0 修复外，新增 ambient credential isolation 和 configured-only model policy：Hermes 不再继承 shell/`~/.secrets` 的模型凭据，`/model` 只动态读取 `config.model + fallback_providers`，并统一展开 `${VAR}` route，链外与 `--global` 均拒绝；主动切到任一 fallback 时跳过重复项并保留另一条容灾 route，compression 仍独立固定配置。08-16 修复 Bedrock Claude inference-profile 图片能力漏识别，Azure GPT-5.5、Opus 5、Gemini 3.5 Flash 三档图片全部 native；`PATCH-VIDEO-SIDECAR` 演进为 `PATCH-MULTIMODAL-SIDECAR`，为普通音频、STT 全失败语音、非 native 视频及本地空抽取/扫描缺口 PDF 提供只带当前媒体与有界上下文的 Vertex Flash 旁路，真实 image/audio/video/PDF wire canary 全绿。同日全面审计 `PATCH-FEISHU-GROUP-SANDBOX`：修复 clarify 误拦、群聊继承 vision/image、任意飞书 token confused-deputy、非可信文档 mutation、search 默认路径与长 web_extract 续读不可达；新增当前消息资源 provenance、可信 mutation 用户、精确本轮 cache artifact grant，群图片/音频/视频/Drive/PDF 与七类文档 consumer 回归全绿，插件 32 项通过。附件链进一步 path-free 收敛：native/抽取/sidecar 成功不再把 cache 路径或具体 ARN 写给模型，全部 reader 失败给明确 `FAILED`；Feishu 同发送者附件回填配置化为 300s/3 消息/6 文件/8s 并覆盖 audio，引用文本不再重复下载，混合扫描 PDF 按 coverage 补视觉读取。注册表规整为 8 类，活跃定义全部连续置于 Archive 前；`PATCH-GEMINI-CROSS-PROVIDER-TOOL-HISTORY` 从误置 Archive 恢复到活跃区（仅结构纠正）。bundle **74 files**，最终 **34 files / 1218 passed / 0 failed / 3 skipped**，full-index/cached/reverse/index-clean 全绿。配置链保持 `azure-foundry/gpt-5.5 → bedrock/Claude Opus 5 → vertex/google/gemini-3.5-flash`，compression 为标准 Vertex，统一 cap 700k；v35 → v37 把 subagent iterations 50→250、并发 3→10。夜间 Cron 新增隔离的飞书组织同步 stage 0，成功/失败只通知 owner 私聊，不进入日报或晚安内容。15 个 bundled skills 更新；npm 仍 6 high，均为 Desktop/web build 链 P2，不影响飞书主链路，禁止 `--force`。 |
