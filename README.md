...
本项目维护 24 个按职责命名的活跃语义补丁：22 个工程内补丁、1 个升级期运行时补丁和 1 个外层配置仓库插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；`PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e 强制 verifier 管理。当前基线为上游 `41a07f5b`，60 个受管文件的内层 diff 与外层 bundle（排除已知 `package-lock.json` 噪音）逐字节一致；最近完整回归为 1852 passed / 0 failed，sandbox plugin verifier 每轮绑定运行时当前 Gateway PID 通过。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v0.19.0            | 2026-07-29 | **2026-W31**：upstream `eb527605 → 41a07f5b`（+1177 commits）。Gateway/session、Voice/STT、Desktop/Photon、模型观测和安全工具继续演进；`PATCH-LAZY-ACTIVATION` 经裸 upstream 验证完全吸收并归档。solvepatch 中断后接管并将遗漏的 `tools/skill_manager_tool.py` import 捕获进 bundle，终态 60 files clean apply、24 个活跃 PATCH 均保留，23 files **1852 passed / 0 failed**，七层闭环与 sandbox verifier 全绿。依赖 19 个 active backend 全 current；npm 仅保留上游 Web/UI/Desktop 构建链 advisory，未使用 `--force`。 |
