...
本项目维护若干针对上游 `hermes-agent` 的本地补丁，以修复已知 Bug 或定制行为。完整记录（问题描述 / 根因 / 修复方案）见 [`patches/PATCHES.md`](patches/PATCHES.md)。当前补丁基线已刷新到上游 `233ef98a`（截至 2026-06-26 的 `main`，较 `a6a28ce3` 前进 104 commits，release tag 维持 `v0.17.0 (2026.6.19)`）；本轮把 **PATCH-14** 从 people-profile 扩展为**人物 / 群聊画像注入**（新增 git 入库的 `~/.hermes/groups.yaml` 群聊语言风格 + 能力栈，以及群聊「能力受限声明」规则）。
