...
| 8    | **Re-apply & verify patches**             | 原子回放 `patches/local-patches.diff`，再按 `PATCHES.md` 中每个语义补丁分别验证 gate；全部通过后才刷新 replay bundle 和 base commit。Step 8d 必须观察到 Gateway PID 实际替换，才确认 patched modules 已加载；旧 PID 残留令整轮非零。`PATCH-FEISHU-GROUP-ADMISSION` 修订的 bundled `llm-wiki` 随后同步到运行时 mirror 并重建 manifest 基线 |
...
补丁由 `hermes-update.sh` 全自动管理：Step 2/8c 以 full-index 保存 replay bundle，Step 3/4 执行 `PATCH-NPM-DEPENDENCY-HYGIENE`，Step 4b 执行 `PATCH-SKILLS-MIRROR-METADATA`，Step 7 保留已上游合并的 completion 回归 sentinel，Step 8 回放工程内 diff 并逐个验证语义 gate，Step 8d 重启 gateway 并要求 PID 替换，Step 8e 强制回归 `PATCH-FEISHU-GROUP-SANDBOX`。`PATCH-UPDATE-GATE-EXIT-STATUS` 保证 apply、sentinel、冲突、空 bundle、runtime sync、Gateway 未实际重载或外层 verifier 任一失败都令升级非零且不得误报成功。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------ | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v0.19.1            | 2026-08-03 | **2026-W32**：更新后审计发现 patch 落盘晚于 Gateway 启动，旧进程未加载合并转发展开与 Vertex hidden-thoughts 补丁，造成历史卡片仍为占位符及图片最终回复混入英文 thought。替换运行 PID 后既有修复恢复；本轮再让主会话 DM 纯 @ 与群聊一致进入意图推断，把 Step 8d 加固为必须观测 old PID → different new PID，并修复 Gateway 通用 500 字符引用上限把 12 条合并转发截到第 6 条的问题。Data Pipeline Workshop 复核继续补齐引用资源矩阵：单独 `/` 不再误判 command，DM 与 `@配置本人账号` 可恢复引用附件，重复显式引用不受窗口去重抑制；图片、视频、普通网页和 Drive 链接均有完整入站回归。真实主会话最终 `content` 与隐藏 `reasoning` 分离，真实卡片 12 条尾项完整送模；23 files **797 passed / 0 failed**，sandbox verifier 绑定新 PID 通过。 |
