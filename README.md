...
本项目维护 30 个按职责命名的活跃语义补丁：23 个工程内补丁、6 个升级期运行时补丁和 1 个外层配置仓库插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 full-index `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；新增 `PATCH-UPDATE-TRANSACTION-PIN` 保证一次升级只取得一个 upstream snapshot，后续围绕固定 SHA reconcile；`PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e 强制 verifier 管理。当前基线为上游 `33f8e96a72945afb29f3bc9ef9991940f0bedcf7`，64 个受管文件的内层 diff 与外层 bundle（排除已知 `package-lock.json` 噪音）逐字节一致；最近完整回归为 26 files 868 passed / 0 failed（2026-08-11 升级至该基线后；另有 3 skipped 为上游 firecrawl-anydoc 可选依赖 real-binding 测试，本机未装该包），sandbox plugin verifier 每轮绑定运行时当前 Gateway PID 通过。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v0.20.0            | 2026-08-11 | **2026-W33**：upstream `cd9fbf9f1 → 33f8e96a7`（周内 +488 commits，中继 `03fa32c92`；主线：31 CVE 修补与 aiohttp 3.14.3 / cryptography 50.0.0 回锁、Windows 编码大扫除（UTF-8/BOM/surrogate、Telegram UTF-16 offsets）、personality 单一所有权 + v34 一次性重置迁移、platform_toolsets 无效名 loud 校验（#38798）、Gateway session identity 持久化与 queued media 连续性、SessionDB 读连接池化与 fd 泄漏/孤儿 gateway 收割、launchd plist 持久化 nofile 下限、kanban review 生命周期一等化、Browser Use CLI 3.0 成为默认浏览器后端 `a1835c8c1`、read_file ipynb 渲染与 PDF gap 标注、skills +6 bundled 与 office 套件 clean-room 替换、blender MCP 因上游被入侵移除、Desktop HUD 全套 + Electron 回滚 40.10.2）。PATCH：30 活跃两轮逐个复核吸收，0 新增 / 0 归档 / 0 收缩（SKILL-CREATE-ROOT guard 未扩至 create、SOCKS extra 未上游声明、DOCUMENT-EXTRACTION 正交、CredentialPool 仍无 per-entry SA/project）。冲突：08-10 轮 3-way 自动融合、08-11 轮 clean apply，无人工冲突；session_context 新增 scope_id ContextVar 属既定并存型。周终态：26 files **868 passed / 0 failed / 3 skipped**（+21 为上游测试扩展；3 skipped 为 firecrawl-anydoc 可选依赖基线，装它属全新依赖留用户决策）；bundle 闭环两轮全绿。配置：v34 迁移重置 personality（kawaii→none，恢复 `/personality kawaii`）；删 feishu_group 的 vision_tools 死条目（verify.sh 同步，行为零变化）；`browser.backend: 'off'` 锁回内建 browser\_\*（Browser Use 需 uvx 拉新包，opt-in 留决策）；fallback_model 由 alibaba/qwen 切至 azure-foundry/gpt-5.5（codex-az 同源，端到端验证）。依赖：venv 未重建；root npm audit 残余 7 vulnerabilities（electron 越 range + node-tar 链 no-fix，Desktop/build 链）P2 不影响飞书主链路，三 workspace advisory 清零；mirror 终态 `+0/~1/-7`（llm-wiki 振荡 + office 陈旧脚本清理）。流程：playbook 新增 Step 5c 三操作自审 + hygiene 断言，摩擦表 −6 失效 row / +3 新规。                                                                                                                                                                                                                                                                                                                                                                         |
