...
本项目维护 30 个按职责命名的活跃语义补丁：23 个工程内补丁、6 个升级期运行时补丁和 1 个外层配置仓库插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 full-index `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；新增 `PATCH-UPDATE-TRANSACTION-PIN` 保证一次升级只取得一个 upstream snapshot，后续围绕固定 SHA reconcile；`PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e 强制 verifier 管理。当前基线为上游 `863e31318553cda8ad61df681d08175364d4164b`，64 个受管文件的内层 diff 与外层 bundle（排除已知 `package-lock.json` 噪音）逐字节一致；最近完整回归为 26 files 832 passed / 0 failed（2026-08-08 主会话 DM 回放并入 `PATCH-FEISHU-MISSED-EVENT-BACKFILL` 后），sandbox plugin verifier 每轮绑定运行时当前 Gateway PID 通过。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v0.20.0            | 2026-08-06 | **2026-W32**：upstream `26e0b1c → 863e3131`（周内 +738 commits，跨入 v0.20.0；主线为 secrets/profile scope、排空重启、Gateway SSE/Relay 生命周期、State/SQLite/CJK、模型与 tool observability、profile archive/import/export、Desktop session/tab 与 file/tool/skill 性能，后段追加 dashboard WebSocket reconnect、toolperf A/B、Actual provider、cache/compression durability、split-delivery/fallback/hygiene、LIKE escaping、atomic writes、Goals/Heartbeat/Refine、cron guard totality、CI test-slice 优化与 anydoc 文档抽取扩展）。周初完成画像全出站隐私、Feishu 引用资源/合并转发、group capability consumer、文档抽取收缩与 native restart gate；`PATCH-FEISHU-GROUP-ADMISSION` 保持部分吸收，`PATCH-DOCUMENT-EXTRACTION` 经 `b2598b41e` 进一步部分吸收 anydoc-only 格式，但 native PDF/HTML/PPTX/ODT、pypdf pin 与入站抽取仍本地。真实冲突集中在 Feishu lazy SDK import 与 approval 测试增量，分别按“保留上游 loader/None 判定，仅补资源 SDK 类型”和“上游 timeout/notify 生命周期 + 本地群聊审批硬拦并存”融合；本轮接管修正 `test_read_extract.py` 中 anydoc-only 与 native-overlap 的陈旧断言，最终 62 files clean apply，25 files **815 passed / 0 failed**，bundle/base/正反向回放与注册表闭环。周中新增 `PATCH-UPDATE-GIT-FETCH-RETRY`、`PATCH-FEISHU-MISSED-EVENT-BACKFILL`；凌晨审计又暴露同一任务反复完整 update 会继续取得 25、17 个新提交并造成额外 Gateway 重启，新增 `PATCH-UPDATE-TRANSACTION-PIN`，现强制一次 `--update` 固定 `TARGET_SHA`、后续 `--reconcile` no-network、失败状态 0600 持久化且仅 exit 0 清除，无变化复验不换 PID；活跃 PATCH 28 → 30，无归档。脚本同期补齐 partial-overlay fail-closed、裸树接管 trap、3-way index 清理、bundle byte/cached/reverse gate、source/dirty-index 拒绝及 Skills 双向变更清单；本轮又把测试/runtime 生成的 `__pycache__/` 与 `*.pyc` 加入 Skills mirror 排除并清理误镜像缓存，避免测试顺序制造 `+70` 噪音。sandbox verifier 21 passed 绑定最终 PID；Doctor 终态余 Browser 2 high、Web 3 high、UI-TUI 1 high 三组 P2 workspace/tooling advisory，不影响飞书主链路，禁止 `--force`，等待 upstream lock/range bump。周末（08-08）把主会话 DM 回放并入 `PATCH-FEISHU-MISSED-EVENT-BACKFILL`：修复回放事件按 `oc_` 前缀误判 chat_type 的盲区，`get_chat_info` 补采 `chat_mode`（实测 DM=`p2p`）作判别、查询失败 fail-closed 按群准入，DM quote 覆盖去重自动继承；新增 3 条 DM 回归与 Step 8b 锚点，26 files **832 passed / 0 failed**，bundle 经 `--reconcile` 刷新并完成 PID 替换。 |
