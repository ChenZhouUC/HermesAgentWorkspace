...
| 8e   | **Verify user plugins**                   | 强制验证 `PATCH-FEISHU-GROUP-SANDBOX` 等配置仓库用户插件：Step 8e verifier 数组必须与 PATCH evidence external registry 精确一致；verifier 必须存在且可执行，行为套件用 strict JUnit 拒绝 skip/xfail/xpass/零执行，并只输出一个机器回执。结构化检查 YAML/固定脚本与 people.yaml 的 open_id↔user_id 完整唯一映射，真实解析 owner/group toolset，运行 sandbox、身份同步及文档媒体 API 行为回归；用 Gateway 规范化的 `[URL](URL)` reply 穿过 deferred `tool_call` 验证可信群文档删除（fake script，零外部删除），并锁定 `insert_image` / `set_cover` 的 schema、来源边界、飞书上传/PATCH 契约和 runtime action trace；launchd wrapper 健康由 status 证明，插件注册 trace 绑定 `gateway.status.get_running_pid()` 返回的真实 Gateway 子进程 PID。任一失败令整次升级非零退出                                 |
...
Step 8e 当前为 `PATCH-FEISHU-GROUP-SANDBOX` 运行 129 条行为测试，并在同一 verifier 中继续执行身份同步与飞书文档媒体脚本回归；owner DM 的 fixed reader → chat-hash 私有 workspace → HyperTeX staging 正例、当前消息 Markdown 资源链接后紧接 CJK 正文的 provenance 正例、全 `HERMES_HOME` traceback 路径脱敏，以及历史手工目录/任意外部路径/未引用资源反例均属于硬门槛。未完成的同 turn claim 不因全局容量被驱逐；明确完成后释放、异常遗留按安全 TTL 退场。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v0.21.0            | 2026-09-08 | **2026-W37**：固定在 upstream `79445a496c`，本轮只做 no-fetch `--reconcile`。增强 `PATCH-LOCAL-PROFILES` 的默认称呼契约：`address` 显式值继续最高优先；缺省时由代码只基于公开 `name` 确定性生成中/英文候选，prompt 仅控制是否自然使用，不再依赖历史模仿。中文双字名可缩短（`安媛媛 → 媛媛`），单字名和低置信布局保留全名，已知复姓保守分割；英文支持 `Given FAMILY`、大写姓前置和单词姓名。aliases、括号昵称与历史消息不参与推导，派生值同步进入公开 redactor 集。实现、回归、`PATCHES.md`、playbook 隐私不变量与 Step 8b sentinel 已共演进；当前 canonical patch suite **44 files / 2215 passed / 0 failed / 6 skipped**，另有 2 个 test support modules、full PATCH evidence **2221 collected**、21/21 probe、118-file bundle、36 active + 7 archived gates、sandbox/identity-sync **161 passed**；Gateway 排空后重启加载新规则。同日实时读回并将群 `oc_4fedab188b969db25dfdb591b74fcfab` 的名称从“FDE 第四把交椅”更新为“FDE 庶民群”。npm 仍仅余 Electron/extract-zip 2 high，修复需 `--force` 跨越上游声明范围，按 P2 保留且不扩权处理。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
