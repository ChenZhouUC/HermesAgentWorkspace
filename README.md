...
| 3    | `hermes update`                           | 对上游 CLI 明确报告的早期 GitHub fetch 网络错误做最多 3 次有界重试；随后 git pull · 关键文件语法校验/失败自动回滚 · uv pip install · Skills Hub 同步 · 配置迁移确认 · gateway 进程重启。认证、分叉、安装与迁移错误不重试                                                                                                                                                                                                                                                                                                                                                           |
...
| 8    | **Re-apply & verify patches**             | 原子回放 `patches/local-patches.diff`，再按 `PATCHES.md` 中每个语义补丁分别验证 gate；全部通过后才刷新 replay bundle 和 base commit。Step 8d 使用排空感知的 `hermes gateway restart`，等待预算优先读取上游 native exit-wait（drain + after-turn + 余量，旧运行时回落 drain），并必须观察到 Gateway PID 实际替换，才确认 patched modules 已加载；旧 PID 残留、restart 非零或超时令整轮非零。预算可用 `bash ~/.hermes/hermes-update.sh --print-restart-wait-seconds` 只读获取。`PATCH-FEISHU-GROUP-ADMISSION` 修订的 bundled `llm-wiki` 随后同步到运行时 mirror 并重建 manifest 基线 |
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------ | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v0.20.0            | 2026-08-05 | **2026-W32**：upstream `26e0b1c → 36cb5ae55`（周内 +593 commits，跨入 v0.20.0；主线为 secrets/profile scope 与排空重启、Gateway SSE/Relay 生命周期、State/SQLite/CJK、模型目录与 bounded model/tool observability、profile archive/import/export、Desktop session/tab 状态及 file/tool/skill 性能，Feishu SDK 延迟导入已上游化）。周初完成画像全出站隐私、Feishu 引用资源/合并转发、group capability consumer、文档抽取收缩与 native restart gate；`PATCH-FEISHU-GROUP-ADMISSION` / `PATCH-DOCUMENT-EXTRACTION` 保持部分吸收。`d1afa160 → 36cb5ae55`（+332）的真实冲突为 Feishu lazy SDK import 和 approval 测试增量重叠，分别按“保留上游 loader/None 判定，仅补 RESOURCE-ACCESS SDK 类型”与“上游 timeout/notify 生命周期 + 本地群聊禁止审批卡测试并存”融合；新增 `PATCH-UPDATE-GIT-FETCH-RETRY`，28 个活跃 PATCH 无归档。升级域名已加入 `.env` 精确 `NO_PROXY`；脚本补上有界 fetch 重试、只读预算入口、source 拒绝和保护 untracked 的逐路径恢复。失败轮 Skills 删除由恢复轮 `+70/~1/-0` 补回，最终幂等复跑为 `+0/~1/-0`。周末 62 files clean apply，bundle/base/正反向回放与注册表闭环，25 files **797 passed / 0 failed**，Gateway PID 替换且 sandbox verifier 21 passed；npm 根/Web/TUI 仍有 2/3/1 high，自动 fix 被上游 lock/range 挡住，禁止 `--force`，待 upstream lockfile bump。 |
