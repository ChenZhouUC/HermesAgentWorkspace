...
| 步骤 | 操作                                      | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ---- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | Preflight + transaction                   | 确认 hermes 可用、git 仓库存在、index 干净；先运行 `--self-test-patch-gates` 和 `--self-test-patch-evidence`，证明 42 个活跃/9 个归档 PATCH 都有持久化真实回归边界、运行时证据或归档回归，且 Step 8b 的 34 个活跃工程 gate 与 6 个归档 gate 都有成功路径并被 Step 8c 聚合条件实际消费；`--update` 仅在无未完成事务时开放一次官方获取，默认/`--reconcile` 固定当前事务 SHA 且不做网络探测。staged 用户改动或证据/gate 集合漂移在任何 mutation 前 fail closed                                                                                                                                                              |
...
| 8    | **Re-apply & verify patches**             | 原子回放 `patches/local-patches.diff`（3-way 成功后自动清 staged index），再按 `PATCHES.md` 中每个语义补丁分别验证 gate；受管路径 live diff 覆盖、full-index 逐字节一致、cached 正向、worktree 反向和 index-clean 全部通过后才刷新 replay bundle/base。`scripts/test_patch_evidence.py` 逐个核对 PATCH 定义、真实测试 token、运行时边界、SOCKS/OpenClaw/npm/Skills/FTS5 证据，并收集全部规范回归文件。回贴后再用最终 `gateway.py` 验证/刷新 plist，必须同时得到 current definition、launchd wrapper PID 与真实 Gateway 子进程；Step 8d 仅在事务 `runtime_dirty=1` 时排空重启并要求 PID 替换，无变化 reconcile 不制造 PID |
...
| 9    | `hermes doctor` + `hermes gateway status` | 验证更新结果；若 gateway 因 update / post-patch restart 处于未加载状态，脚本会自动补一次最终恢复（`install --force` / `start`）后再判定。最终提交前必须再次运行 `python3 scripts/test_patch_evidence.py` 与规范回归 runner，保证 AI 自演进证据晚于最后一次脚本/注册表修改                                                                                                                                                                                                                                                                                                                                                |
...
本项目维护 42 个按职责命名的活跃语义补丁：34 个工程内补丁、7 个升级期运行时补丁和 1 个外层配置仓库插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 full-index `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；`--self-test-patch-gates` 与 `--self-test-patch-evidence` 在每轮 preflight 自动证明 gate 集合、PATCH 生命周期和每个 PATCH 的真实回归边界一致；`PATCH-UPDATE-TRANSACTION-PIN` 保证一次升级只取得一个 upstream snapshot，后续围绕固定 SHA reconcile；已归档的 `PATCH-LAUNCHD-WRAPPER-SUPERVISOR` 由上游实现与 Step 8b sentinel 继续监管；`PATCH-ENV-AMBIENT-CREDENTIAL-ISOLATION` 阻止 shell/`~/.secrets` 凭据越界进入 Hermes；`PATCH-MODEL-CONFIGURED-ONLY` 让 `/model` 只展示并切换 config 中的主模型/fallback；`PATCH-DOCTOR-TEST-NETWORK-ISOLATION` 让 doctor 单测不再继承宿主网络与凭据；`PATCH-MCP-TASKS-ASYNC-HANDOFF` 让标准 MCP task handle 确定性回执并结束当前 turn；`PATCH-GATEWAY-FAILOVER-STATUS-SILENCE`、`PATCH-TRUNCATED-TOOL-CALL-RECOVERY`、`PATCH-FEISHU-RESPONSE-BUDGET` 与 `PATCH-TOOL-CALL-DOUBLE-WRAP-RECOVERY` 共同守住群聊模型路由、长工具参数、回复长度和 deferred-tool 调用边界；`PATCH-GATEWAY-RESTART-CLEANUP` 在每次 restart 前审计并清理外层脚本与 outer/inner ignored 产物，同时保留 Gateway 下一次启动必须消费的 `.clean_shutdown` graceful-exit receipt；`PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e 强制 verifier 管理，并用真实 `[URL](URL)` 引用形态区分可信身份与目标 provenance。当前基线为上游 `fd760435c6688a2b6c6b7436dde30e267237baef`；受管路径的内层 diff 与外层 bundle（排除已知 `package-lock.json` 归一化漂移）必须逐字节一致，精确数量以 `hermes-update.sh --print-patched-files` 为权威。最近完整回归结果见本周版本记录，sandbox verifier 同时证明 launchd wrapper 健康并把插件注册 trace 绑定真实 Gateway 子进程 PID。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v0.20.5            | 2026-08-23 | **2026-W34**：upstream `8ad055414 → fd760435c6`（累计 +1566 commits），升级并收敛在 v0.20.5。周内主线覆盖 Gateway/systemd/control-socket/loop-watchdog、State/SQLite 原子恢复与 corruption fail-closed、更新器 profile/事务/venv shim、Bot Mode、Bedrock Responses、OpenCode Free、GLM-5.3，以及 Browser/Desktop artifact、tool-call 与渲染边界。逐项轮询 42 active + 9 archive PATCH：终轮上游 55 个变更路径仅 3 个与 88 个受管路径相交，upstream/local hunk 区间零交叉，裸 upstream 未满足任何新吸收条件；无新增吸收/归档/收缩，`PATCH-DOCUMENT-EXTRACTION` 维持既有部分吸收。PATCH evidence 机器证明 51 个定义精确四段、34 个工程 PATCH 与 34 个 Step 8b gate 一一归属，并为 9 个 Archive PATCH 运行当前 pytest/CLI/行为或负向退役审计；最终权威回归 **39 files / 1679 passed / 0 failed / 3 skipped**，34 active + 6 archived gates、bundle byte/cached/reverse/index-clean 全绿。周内自演进修复：配置恢复快照与 Gateway `.clean_shutdown` receipt 显式纳入 cleanup keep policy；带 `tee` 的升级入口强制 `pipefail`；Bedrock/Codex wire cap、active evidence 假阳性及 6 个旧 Archive 块的非规范段名/过期测试引用均已修复；bundle evidence 改为临时目录自动回收，并将历史遗留 52 个约 106 MB 目录移入 Trash。终态 sandbox **54 passed** + **6 identity-sync tests**，Gateway supervisor `12668 → 16471`、真实子进程 `16472`。Skills mirror `+1/~0/-0`；monorepo npm audit 为 6 high，均属 Desktop/Web/UI 构建链 P2，不影响飞书主链路且未使用 `--force`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
