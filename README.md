...
| 步骤 | 操作                                      | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---- | ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | Preflight + transaction                   | 确认 hermes 可用、git 仓库存在、index 干净；先运行 gate 集合自测和 `mode=quick` PATCH evidence，证明结构、专用探针与 runtime contract 在任何 mutation 前闭合。所有数量从脚本/JSON 动态读取，不在 README 复制；quick 不能替代终态 full evidence。`--update` 仅在无未完成事务时开放一次官方获取，默认/`--reconcile` 固定事务 SHA 且不做网络探测；staged 用户改动或 evidence/gate 漂移 fail closed                                                                                                                                        |
...
本项目维护 42 个按职责命名的活跃语义补丁：34 个工程内补丁、7 个升级期运行时补丁和 1 个外层配置仓库插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 full-index `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；preflight quick audit 只守结构与 contract，终态 full evidence 将验证函数唯一绑定完整 pytest node ID 并拒绝 skip/xfail，同步输出 owned-files/upstream-overlap 矩阵；`--final-audit --json` 再统一执行 canonical suite、Archive probes、bundle/docs/Wiki、sandbox runtime、Gateway PID 与最终 cleanup。`PATCH-UPDATE-TRANSACTION-PIN` 保证一次升级只取得一个 upstream snapshot；`PATCH-ENV-AMBIENT-CREDENTIAL-ISOLATION`、`PATCH-MODEL-CONFIGURED-ONLY`、`PATCH-MCP-TASKS-ASYNC-HANDOFF`、Feishu scope/response/tool-call 系列与 `PATCH-GATEWAY-RESTART-CLEANUP` 继续守住凭据、模型、异步任务、群聊和重启边界；`PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e verifier 管理。当前基线为上游 `fd760435c6688a2b6c6b7436dde30e267237baef`；受管路径与 bundle 必须逐字节一致，精确数量以只读入口为权威。最近完整回归见本周版本记录。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v0.20.5            | 2026-08-23 | **2026-W34**：upstream `8ad055414 → fd760435c6`（累计 +1566 commits），升级并收敛在 v0.20.5。周内主线覆盖 Gateway/control-socket/loop-watchdog、State/SQLite fail-closed、更新器事务/venv shim、Bot Mode、模型链与 Browser/Desktop 边界。逐项轮询 42 active + 9 archive PATCH：终轮上游仅 3 个受管路径相交且 hunk 零交叉，无新增吸收/归档/收缩，`PATCH-DOCUMENT-EXTRACTION` 保持既有部分吸收。AI 自演进将 51 个定义统一为精确四段；active 验证函数唯一绑定完整 pytest node ID 并以隔离 JUnit 拒绝歧义、skip/xfail，Archive 使用当前行为或 secret-safe/provider-independent 退役探针；新增 `--final-audit --json` 单一终态入口覆盖逐 PATCH matrix、canonical suite、bundle、docs/Wiki、runtime 与最终 cleanup。最终 **39 files / 1679 passed / 0 failed / 3 skipped**，34 active + 6 archived gates、88-file bundle 全绿；sandbox **54 passed** + **6 identity-sync tests**，Gateway `16471/16472`。`.clean_shutdown`、tee/pipefail、Bedrock/Codex wire cap、旧 Archive 过期证据和 evidence 临时目录泄漏均已修复，52 个约 106 MB 历史目录移入 Trash。Skills mirror `+1/~0/-0`；npm 6 high 均属 Desktop/Web/UI 构建链 P2，不影响飞书主链路。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
