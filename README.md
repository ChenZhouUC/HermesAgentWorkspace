...
本项目维护若干针对上游 `hermes-agent` 的本地补丁，以修复已知 Bug 或定制行为。完整记录（问题描述 / 根因 / 修复方案）见 [`patches/PATCHES.md`](patches/PATCHES.md)。当前补丁基线已刷新到上游 `e3db1ef9`（截至 2026-06-26 的 `main`，较 `0f81b0d4` 前进 28 commits，release tag 维持 `v0.17.0 (2026.6.19)`）；**本轮新增 PATCH-15**（Feishu 群聊裸 @ 触发时回看并附上同发送者最近 ~120s 的图片/文件，改 `plugins/platforms/feishu/adapter.py`），共 11 条活跃补丁全部 clean apply 到新基线、`local-patches.diff` 字节 `187661 → 200097`（PATCH-15 净增）；配套 config-repo 扩 `feishu-docs` skill（新增 `read_sheet.py`/`read_bitable.py`/`download_feishu_file.py`/`read_feishu_url.py`/`feishu_render.py` + `SKILL.md`），不进 `local-patches.diff`。
