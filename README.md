...
本项目维护 39 个按职责命名的活跃语义补丁：31 个工程内补丁、7 个升级期运行时补丁和 1 个外层配置仓库插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 full-index `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；`PATCH-UPDATE-TRANSACTION-PIN` 保证一次升级只取得一个 upstream snapshot，后续围绕固定 SHA reconcile；`PATCH-LAUNCHD-WRAPPER-SUPERVISOR` 保证 launchd stderr wrapper 启动的真实 Gateway 子进程保留受监管身份；`PATCH-ENV-AMBIENT-CREDENTIAL-ISOLATION` 阻止 shell/`~/.secrets` 凭据越界进入 Hermes；`PATCH-MODEL-CONFIGURED-ONLY` 让 `/model` 只展示并切换 config 中的主模型/fallback；`PATCH-DOCTOR-TEST-NETWORK-ISOLATION` 让 doctor 单测不再继承宿主网络与凭据；`PATCH-MCP-TASKS-ASYNC-HANDOFF` 让标准 MCP task handle 确定性回执并结束当前 turn；`PATCH-GATEWAY-RESTART-CLEANUP` 在每次 restart 前审计并清理外层脚本与 outer/inner ignored 产物；`PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e 强制 verifier 管理。当前基线为上游 `8ad055414bcae75486952c5080d366679e074c1b`，84 个受管文件的内层 diff 与外层 bundle（排除已知 `package-lock.json` 噪音）逐字节一致；最近完整回归结果见本周版本记录，sandbox verifier 同时证明 launchd wrapper 健康并把插件注册 trace 绑定真实 Gateway 子进程 PID。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v0.20.1            | 2026-08-18 | **2026-W34**：upstream 保持 `8ad055414`，本周做本地语义 PATCH 收敛。`PATCH-MCP-TASKS-ASYNC-HANDOFF` 通用协商 `io.modelcontextprotocol/tasks`、按 capability 注册 `tasks_get/update/cancel`，task handle 持久化后直接结束 turn；续审补齐 `input_required`/混合工具批次、completed + `isError`、Streamable HTTP 路由头与“task ID + 链接”最小前台边界，core 不含 HyperTeX 业务分支。HyperTeX sandbox 扩为 create/iterate/list/get + Tasks utilities，并用群 ID + 用户 ID 双 allowlist 仅向财务专享 AI 小助手、Data Pipeline Workshop、AI 解放生产力开放；当前 6 个群名由已跟踪 `channel_aliases.json` 覆盖。新增 `PATCH-GATEWAY-RESTART-CLEANUP`：policy 显式分类全部 outer 运维脚本及 outer/inner ignored 路径为 keep/remove/review，默认 dry-run/JSON 审计，apply 仅把 blacklist 项移入 Trash，未知项阻断 restart。PATCH 终态 **39 活跃 / 31 工程内**，受管集合 **84 files / 38 tests**；replay 对 upstream 不存在或 ignored managed-new 文件改用临时 index 生成完整 full-index bundle。终态回归 **38 files / 1352 passed / 0 failed / 3 skipped / 8 subtests**，sandbox **41 passed**，Step 8b/8e、bundle/base、cached 正向、worktree 反向、index-clean 与 playbook/transaction/cleanup self-test 全绿。配置仍为 v37，Gateway 受 launchd 监管；剩余 6 high 均为 Electron/postcss/nanoid 上游 lock/range 阻挡的 Desktop/Web build-chain P2，不影响飞书主链路，未使用 `--force`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
