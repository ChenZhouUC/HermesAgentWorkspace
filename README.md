...
本项目维护 38 个按职责命名的活跃语义补丁：30 个工程内补丁、7 个升级期运行时补丁和 1 个外层配置仓库插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 full-index `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；`PATCH-UPDATE-TRANSACTION-PIN` 保证一次升级只取得一个 upstream snapshot，后续围绕固定 SHA reconcile；已归档的 `PATCH-LAUNCHD-WRAPPER-SUPERVISOR` 由上游实现与 Step 8b sentinel 继续监管；`PATCH-ENV-AMBIENT-CREDENTIAL-ISOLATION` 阻止 shell/`~/.secrets` 凭据越界进入 Hermes；`PATCH-MODEL-CONFIGURED-ONLY` 让 `/model` 只展示并切换 config 中的主模型/fallback；`PATCH-DOCTOR-TEST-NETWORK-ISOLATION` 让 doctor 单测不再继承宿主网络与凭据；`PATCH-MCP-TASKS-ASYNC-HANDOFF` 让标准 MCP task handle 确定性回执并结束当前 turn；`PATCH-GATEWAY-RESTART-CLEANUP` 在每次 restart 前审计并清理外层脚本与 outer/inner ignored 产物；`PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e 强制 verifier 管理。当前基线为上游 `13ce0c5c675e843af70d19c9e5144249cd51c8d1`，82 个受管文件的内层 diff 与外层 bundle（排除已知 `package-lock.json` 归一化漂移）逐字节一致；最近完整回归结果见本周版本记录，sandbox verifier 同时证明 launchd wrapper 健康并把插件注册 trace 绑定真实 Gateway 子进程 PID。
