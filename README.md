...
本项目维护 33 个按职责命名的活跃语义补丁：26 个工程内补丁、6 个升级期运行时补丁和 1 个外层配置仓库插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 full-index `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；新增 `PATCH-UPDATE-TRANSACTION-PIN` 保证一次升级只取得一个 upstream snapshot，后续围绕固定 SHA reconcile；`PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e 强制 verifier 管理。当前基线为上游 `222465d84709379b65173b0283a6eea87516acfa`，68 个受管文件的内层 diff 与外层 bundle（排除已知 `package-lock.json` 噪音）逐字节一致；最近完整回归为 28 files 1042 passed / 0 failed（2026-08-12 升级至该基线后；另有 3 skipped 为上游 firecrawl-anydoc 可选依赖 real-binding 测试，本机未装该包），sandbox plugin verifier 每轮绑定运行时当前 Gateway PID 通过。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------ | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v0.20.0            | 2026-08-12 | **2026-W33**：upstream `cd9fbf9f1 → 222465d84`（周内 +570 commits，中继 `03fa32c92` / `33f8e96a7` / `c0106e50e`）。主线：31 CVE 修补与 aiohttp/cryptography 回锁、personality 单一所有权 + v34 一次性重置迁移、`platform_toolsets` 无效名 loud 校验（#38798）、Gateway session identity 持久化与孤儿 gateway 收割、Browser Use CLI 3.0 成为默认浏览器后端 `a1835c8c1`、blender MCP 因上游被入侵移除；周末两轮转向 **Gateway 路由语义 + Windows/跨平台加固 + Desktop 更新器重写**——显式 route rejection marker `9829746df`、选择性 multiplex allowlist `c8f235a10`、routed profile busy mode `a31be4803`；Windows 审批扩到破坏性命令/路径 `e1caf88c6`、MCP PATHEXT `4a2198bf5`、agent-loop papercuts `ee472a7fd`；Desktop 更新器改 quit-first hand-off（`c991e3f62` / `503e61b3b` / `c9f0b6824` + 6 收敛修复）；scale-to-zero 经 flaps socket 自挂起 `356c702b5` 并排除常驻 watcher `5fffe5606`；`verify_on_stop` 全面改 opt-in `14692ec91`。PATCH：**33 活跃**，周内 **+3 新增 / 0 归档 / 0 收缩**——`PATCH-VIDEO-SIDECAR`（fallback 链首个视频档单次旁路读取）、`PATCH-COMPACTION-LIFECYCLE-SILENCE`（上游只登记 compaction 的 start 边，done 边逃过噪声抑制与 `progress_notices` 开关，两次把 `✓ Context compaction complete` 发进工作群）、`PATCH-FEISHU-QUOTE-CHAIN-SESSION`（入站把飞书 `root_id` 当 `thread_id` 回退，每条引用链切出独立 session，同群 2h 内 3 个 session 反复重载 skill 与回填；与 `PATCH-FEISHU-NORMAL-REPLY` 是同一 conflate 的入站/出站两侧，可被不同上游 PR 分别吸收故不合并）。冲突：三轮均无人工冲突；`session_context` scope_id ContextVar、`gateway/run.py` multiplex/busy-mode/scale-to-zero 增量、`tools/approval.py` Windows 破坏性层均属既定并存型，与本地 Feishu/sandbox/Vertex/视频/compaction 不变量正交。**吸收判定的方法教训**：grep 命中 ≠ 已吸收——`platform_toolsets`(18) 是上游既有基础设施而非补丁的 `platform_toolset_options`；`HERMES_GATEWAY_TOKEN`(1) 恰是删除型补丁要删的那行；`approval.py` 虽被上游大改但对 `/private` realpath 别名零命中。周终态：**28 files 1042 passed / 0 failed / 3 skipped**（+142 逐项对账：`test_telegram_noise_filter` 首次纳入 +133、本轮新增 compaction 边界 +8 与 quote-chain +1；3 skipped 为 firecrawl-anydoc 可选依赖基线）；bundle 闭环三轮全绿，受管文件 64 → 68。配置：v34 重置 personality（恢复 `/personality kawaii`）；`browser.backend: 'off'` 锁回内建 browser\_\*；主链路 `azure-foundry/gpt-5.5` + fallback（[0] `vertex-fallback/google/gemini-3.1-pro-preview` 兼视频旁路、[1] `alibaba/qwen3.7-plus`）；清空废弃 `vertex.project_id`；三档窗口同为 ~1M 故 `compression.threshold_tokens` 150k → 600k（原值把 `threshold: 0.7` 压成 14% 占用）、`protect_last_n` 20 → 40；群聊 skill 白名单增开 `excel-processing`。依赖：venv 四轮未重建；root npm audit 7 → **1 high**（electron 越 stated range 需 `--force`）P2 不影响飞书主链路。流程：**Step 8e 拦下唯一 P0**——`verify.sh` 硬断言群 skill allowlist，增开 skill 未同步 verifier 即整轮非零，正是「扩大群可读面必须显式且被验证」的设计意图；playbook 新增 Step 5c 三操作自审 + hygiene 断言，摩擦表 −6/+4。 |
