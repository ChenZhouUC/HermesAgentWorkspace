...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v0.20.6            | 2026-09-04 | **2026-W36**：本周未获取新 upstream，内层固定 `11c8c05dc3`。9 月 3 日飞书文档事故与两轮全 PATCH 深审修复：Sheets Markdown+CJK provenance、53.8 MiB PPTX 回填超时与混合图文覆盖误报、附件流式预算/deadline/显式失败、跨 worker claim、宿主路径、损坏配置 credential/configured-only fail-open、多 endpoint route、MCP task provenance 持久化、skill scope，以及 update/final-audit 的原子锁、完整状态快照、stash/EXIT、cleanup 跨 workspace 进程归属、bundle/base/lockfile 审核缺口均已收束。sandbox 升至 `0.7.14`；权限来源与群工具面未扩大。9 月 4 日扩展 `PATCH-FEISHU-GROUP-ADMISSION`：任何发送者 @本人 时，由独立无工具辅助模型仅查看匿名化当前正文和直接 quote，按本地 per-sender 阈值判断未披露 AI 代写；本人不再因 configured-human 身份绕过。Markdown/规整列表校准增强，检测超时通过 `auxiliary.feishu_ai_authorship.timeout` 暴露，当前为 30 秒，并关闭同 provider transient retry 和跨 provider failure fallback。仅分类成功、高分且拒绝文案成功送达时才在主 Agent 前短路；低分、披露、分类失败/超时或拒绝投递失败均继续原画像回复链路。成功拒绝会启动 300 秒发送者冷却；冷却期间，该发送者在任意群里再次 `@本人`、`@Hermes` 或两者同时 @ 时，都会从本地短句池随机收到中英双语的拟人化生气回复和加粗的动态剩余秒数，不再调用分类器或主 Agent。未触发机器人的普通群消息和 DM 不受影响；冷却不因重复触发顺延，重启后清空。首次拒绝文案同样为中文在上、英文在下。群回复把当前发送者及正文中所有能由热加载人员索引解析的 `@姓名/别名` 原位转为同一 `md.text` 内的 Feishu 原生 mention，缺失当前发送者时自动补，避免独立 post 元素造成换行。模型 fallback/恢复/超时/最终 provider failure 全部只进日志；Codex `<50k` 首字节后事件 idle 阈值从 12 秒修正为 60 秒。同轮修复 PATCH evidence 在 hermetic `npm audit` 超时时误报回归缺失的问题，现按 P2 `telemetry_unavailable` 记录，critical 仍阻断。终态数字以本轮 final-audit 为准：**53/53 full PATCH evidence**、canonical **43 files / 2124 passed / 0 failed / 6 skipped**、另有 2 个 test support modules、98-file bundle、35 active + 7 archived gates、sandbox/identity-sync **161 passed**。                                                                                                                                                                                              |
