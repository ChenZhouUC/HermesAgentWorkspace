...
| 8    | **Re-apply & verify patches**             | 原子回放 `patches/local-patches.diff`（3-way 成功后自动清 staged index），再按 `PATCHES.md` 分别验证 gate；受管路径 live diff、full-index 逐字节、cached 正向、worktree 反向和 index-clean 全部通过后才刷新 bundle/base。full PATCH evidence 将验证函数唯一解析为完整 pytest node ID，并以隔离 JUnit outcome 拒绝同名歧义、未收集、skip/xfail/error/failure；Archive 运行当前行为或 secret-safe 退役探针。回贴后必须同时得到 current definition、launchd wrapper PID 与真实 Gateway 子进程；runtime dirty 才排空重启，无变化不制造 PID                         |
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v0.20.5            | 2026-08-27 | **2026-W35**：upstream `503d863fcd → 9aa7530f7b`（+889 commits），仍为 v0.20.5；主线集中于 Desktop/fleet session、Gateway reconnect/profile route、update 多后端与 SSH ownership、MCP transport/catalog、批量 Tool Search、lean compaction、terminal backend、安全审批及 v39 BFL core-tool 迁移。逐项轮询原 42 active + 9 archive PATCH：25 个活跃 PATCH 与 36 个受管路径重叠，5 个真实冲突按“上游增强 + 本地不变量并存”融合；无既有 PATCH 新吸收/归档/收缩，`PATCH-DOCUMENT-EXTRACTION` 保持部分吸收。新增 `PATCH-MCP-STDIO-WATCHER-LIFECYCLE`，修复上游 stdio watcher coroutine 双创建泄漏；同时修复 quick evidence 在 Step 2 前错误要求旧 bundle parity 的恢复死锁、final-audit 对同版本跨周的误判，并适配 Tool Search `queries[]/names[]` 后的 sandbox verifier。8 月 27 日 SpaceSight Tech Sharing Group 现场又暴露 fallback producer 文案已变为 `Model fallback:`、旧 regex/测试/gate 同步假绿；现改为生产 `_format_fallback_notice()` 驱动 consumer 回归，聊天面静默、programmatic 面保留。同日主会话实抓 HyperTeX stdio 正常启动却被 PID 存活判定反向误报退出；现修正为任一 child 存活即不得 fast-fail，并补 alive/dead/mixed 与真实 handler 回归。终态 **39 files / 1731 passed / 0 failed / 9 skipped**（3 anydoc 可选依赖、6 无首发期 toolset 条件跳过），88-file bundle、35 active + 6 archived gates、43+9 PATCH evidence、sandbox/identity-sync **60 passed**和 Gateway 双 PID 闭环。首次 HTTPS acquisition 因 HTTP/2 失败，在 `target_sha=pending` 时按规则仅恢复一次并以进程级 SSH 443 固定目标；官方 updater 自举 restart 一度留下 registered-but-down Gateway，随后 pinned reconcile 恢复。Skills mirror `+1/~0/-11`（新增 llm-wiki、清理 `.DS_Store`）；config v38→v39。npm Web 4 high、UI-TUI 3 high 仍为 Vite peer/lock P2，不影响飞书主链路，未使用 `--force`；未登录 provider 与未配置可选工具为 P3。                                                                                                                                                                                                                                                                                                                                                                                                                           |
