...
本项目维护 24 个按职责命名的活跃语义补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；`PATCH-FEISHU-GROUP-SANDBOX` 属外层配置仓库插件，不进入内层 diff，由 Step 8e 强制 verifier 管理。当前基线为上游 `eb527605`，内层 diff 与工作区（排除已知 `package-lock.json` 噪音）逐字节一致；最近完整回归为 1909 passed / 0 failed，sandbox plugin verifier 通过。
