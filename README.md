...
Step 8e 当前为 `PATCH-FEISHU-GROUP-SANDBOX` 运行 117 条行为测试，并在同一 verifier 中继续执行身份同步与飞书文档媒体脚本回归；owner DM 的 fixed reader → chat-hash 私有 workspace → HyperTeX staging 正例，以及历史手工目录/任意外部路径反例均属于硬门槛。
