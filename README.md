...
本项目维护 30 个按职责命名的活跃语义补丁：23 个工程内补丁、6 个升级期运行时补丁和 1 个外层配置仓库插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 full-index `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；新增 `PATCH-UPDATE-TRANSACTION-PIN` 保证一次升级只取得一个 upstream snapshot，后续围绕固定 SHA reconcile；`PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e 强制 verifier 管理。当前基线为上游 `03fa32c92dd445eb64c7f67434dd91b32c40701d`，64 个受管文件的内层 diff 与外层 bundle（排除已知 `package-lock.json` 噪音）逐字节一致；最近完整回归为 26 files 868 passed / 0 failed（2026-08-10 升级至该基线后；另有 3 skipped 为上游 firecrawl-anydoc 可选依赖 real-binding 测试，本机未装该包），sandbox plugin verifier 每轮绑定运行时当前 Gateway PID 通过。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v0.20.0            | 2026-08-10 | **2026-W33**：upstream `cd9fbf9f1 → 03fa32c92`（+409 commits；主线：31 CVE 修补横跨 Python/npm lock 与 aiohttp 3.14.3 pin `7537de9e7`/`45f31de4e`、Windows 编码大扫除（auth/.env/cron UTF-8/BOM/lone-surrogate、Telegram UTF-16 offsets `762f1c588`/`566b5b16a`/`022d196f3`）、personality 单一所有权 + v34 一次性重置迁移 `244d29664`、platform_toolsets 无效名 loud 校验（#38798 动机）、Gateway session identity 持久化 `d2a4d373e` 与 queued media 连续性簇、FTS5 sanitizer/corrupt fallback、compression prune + gpt-5.6 原生 compaction、read_file ipynb 输出渲染与 PDF gap 标注、skills +4 bundled 与 office 套件 clean-room MIT 替换、Desktop HUD 模式全套 + Electron 回滚 40.10.2）。PATCH 生命周期：30 活跃全部逐个复核吸收判断，0 新增 / 0 归档 / 0 收缩；重点核验 SKILL-CREATE-ROOT write-guard 未扩大到 create、SOCKS extra 仍未上游声明、DOCUMENT-EXTRACTION 与 ipynb/PDF-gap 增量正交、VERTEX-FALLBACK 之 CredentialPool 仍无 per-entry SA/project 语义。冲突：64 文件 3-way 自动融合，无需人工解冲突。周终态：26 files **868 passed / 0 failed / 3 skipped**（+21 来自上游 test_read_extract 40→54 与 test_run_progress_topics 18→25 扩展；3 skipped 为 firecrawl-anydoc 可选依赖 real-binding 测试，本机未装，装它属全新依赖留用户决策）；bundle/base/full-index 正反向与 index-clean 闭环全绿。配置：config v33→v34 迁移将 personality 由 `kawaii` 一次性重置为 none（上游有意设计防陈年状态复活，恢复用 `/personality kawaii`）；`--fix` 路径校验暴露 `platform_toolsets.feishu_group` 的 `vision_tools` 为自初始提交起的无效死条目（解析为空、沙箱层本就不放行 vision），已从 config.yaml 与 verify.sh 契约删除，运行时行为零变化；`sandbox_group` 在 doctor CLI 校验上下文为插件未加载误报，普通 doctor 不出现。依赖：venv 未重建仅 aiohttp 升 3.14.3；root npm audit 残余 7 vulnerabilities（electron 2H 修复越 range + node-tar 链 1C4H no-fix，均在 Desktop/build 链）P2 不影响飞书主链路；Browser/Web/UI-TUI 三 workspace advisory 经上游 CVE 修补清零。                                                                                                                                                                                                                                                                      |
