...
本项目维护 27 个按职责命名的活跃语义补丁：22 个工程内补丁、4 个升级期运行时补丁和 1 个外层配置仓库插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 full-index `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；`PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e 强制 verifier 管理。当前基线为上游 `26e0b1c`，64 个受管文件的内层 diff 与外层 bundle（排除已知 `package-lock.json` 噪音）逐字节一致；最近完整回归为 829 passed / 0 failed，sandbox plugin verifier 每轮绑定运行时当前 Gateway PID 通过。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------ | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v0.19.1            | 2026-08-03 | **2026-W32**：更新后审计发现 patch 落盘晚于 Gateway 启动，旧进程未加载合并转发展开与 Vertex hidden-thoughts 补丁，造成历史卡片仍为占位符及图片最终回复混入英文 thought。替换运行 PID 后既有修复恢复；本轮再让主会话 DM 纯 @ 与群聊一致进入意图推断，并修复 Gateway 通用 500 字符引用上限把 12 条合并转发截到第 6 条的问题。Data Pipeline Workshop 复核继续补齐引用资源矩阵：单独 `/` 不再误判 command，DM 与 `@配置本人账号` 可恢复引用附件，重复显式引用不受窗口去重抑制；图片、视频、普通网页和 Drive 链接均有完整入站回归。随后 Data Pipeline Workshop 与 SpaceSight 救命稻草共同暴露运行 consumer 仍把 Feishu group 映射成 DM：群聊继承 `tool_progress: new` 并拿到 terminal/drive 工具面，既外显调用链，又看不到承载受控文档动作的 `feishu_doc_manage`。现已让主 Agent、busy ack、reasoning/footer、proxy streaming、background task 和 slash commands 全部统一走 source-aware key，并补真实 `_run_agent` DM 正例、双群负例与群 `/verbose` 配置写回隔离回归。真实主会话最终 `content` 与隐藏 `reasoning` 分离，真实卡片 12 条尾项完整送模。画像风控复审又确认问话人 open_id 与历史/合并转发发送者均可命中本地画像且历史标签只含公开白名单；同时补齐未知字段 private-by-default、技术 ID/未公开别名与下属原始计数过滤、`/background`、临时消息 fallback、流式 TTS 的脱敏旁路，移除后台完成消息的 prompt 回显，并把 `people.yaml` / `groups.yaml` 设计为 owner 可读写且自动收敛 `0600`。26 files **829 passed / 0 failed**，sandbox verifier 绑定新 PID 通过。流程复盘又确认旧 Step 8d 的 `stop/start` 会在短宽限后强杀在途任务，现改为按 `restart_drain_timeout + 30s` 等待的 planned restart，并新增“最高有效边界回归 + 测试清单防退化 + namespace consumer 成对断言 + 最后修改后的终态 PID 屏障”；Vertex thought 抑制再补真实 Chat Completions wire-shape 回归，补齐落盘、语义、运行态三类 AI 幂等。 |
