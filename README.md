...
- `group_chart_generate` 只接受 Hermes 已从文本/XLSX/CSV 提取出的结构化数据，不接受原始 workbook 路径、任意绘图代码/kwargs、URL 或输出路径。独立 renderer venv 使用固定版本 Matplotlib + Seaborn：常规业务图由 Matplotlib 模板控制排版，关系/分布/分类/回归/矩阵图由 Seaborn 渲染；进程沙箱明确禁止网络，PNG 仍落到当前群哈希工作区并通过 `MEDIA:` 发送。主会话对应 `secure_chart_generate`。
...
# 5. 运行 54 条 sandbox 行为测试 + Feishu 组织身份同步回归，并检查当前 gateway PID 的 runtime 注册日志
```
...
| 8e   | **Verify user plugins**                   | 强制验证 `PATCH-FEISHU-GROUP-SANDBOX` 等配置仓库用户插件：Step 8e verifier 数组必须与 PATCH evidence external registry 精确一致；verifier 必须存在且可执行，行为套件用 strict JUnit 拒绝 skip/xfail/xpass/零执行，并只输出一个机器回执。结构化检查 YAML/固定脚本与 people.yaml 的 open_id↔user_id 完整唯一映射，真实解析 owner/group toolset，运行 54 条行为测试并追加 Feishu 组织身份同步回归；用 Gateway 规范化的 `[URL](URL)` reply 穿过 deferred `tool_call` 验证可信群文档删除（fake script，零外部删除）；launchd wrapper 健康由 status 证明，插件注册 trace 绑定 `gateway.status.get_running_pid()` 返回的真实 Gateway 子进程 PID。任一失败令整次升级非零退出                                                                                                                                   |
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v0.20.6            | 2026-08-27 | **2026-W35**：upstream `503d863fcd → 8966b0a700`（累计 +1171 commits），升级到 v0.20.6；主线覆盖 Desktop/fleet/managed SSH update、Gateway control-socket 排空与 restart recovery、MCP transport/liveness、WeCom 原生流式、Browser real-profile、SQLite、Skills schema、安全审批与模型目录。逐项轮询升级前 43 active + 9 archive PATCH：24 个 active 与 17 个受管路径相交；`PATCH-COMPACTION-LIFECYCLE-SILENCE` 被 upstream 完全吸收并归档，`PATCH-MCP-STDIO-WATCHER-LIFECYCLE` 的 PID liveness 被部分吸收。三轮负向审计新增 `PATCH-TEST-RUNTIME-STATE-ISOLATION`，并封堵 ownership/零 owner、gate/probe 漏消费、node 路径错绑、零 collect/support 虚增、Archive skip/overlap 漏报、canonical 数量/flake、测试后 bundle/sandbox/外层工作树漂移及 Gateway PID reuse 等假绿。终态 **39 files / 1732 passed / 0 failed / 9 skipped**，另有 2 个 test support modules 做独立语法/ownership 校验；90-file bundle、43 active + 10 archive、35 active + 7 archived gates、53/53 full PATCH evidence、sandbox/identity-sync **60 passed**。生产 `gateway_state.json` 与 spawn ledger 均保持真实 Gateway 身份；Skills mirror `+1/~0/-0`、config v39。npm 仅余 Desktop Electron/extract-zip 2 high，为 P2 且未用 `--force`；未配置可选能力为 P3。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
