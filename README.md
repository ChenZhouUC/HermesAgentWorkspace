...
**作用**：bot 同一个 Feishu 应用账号同时服务多个会话时，按 `chat_id` + `chat_type` 区分工具权限——配置中列出的 owner DM 拥有完整工具集，其他 Feishu DM 只能调用基础安全白名单；Feishu 群聊/频道额外拥有只读知识工具、按群隔离的临时文件工具、受控的飞书文档脚本入口，以及按群显式启用的沙箱图片/图表生成入口。生成结果或当前消息/显式回复中的图片可以通过固定文档 action 插入正文或设置封面，不开放任意宿主路径与任意 URL 下载。HyperTeX MCP 使用一条窄策略：owner DM 直接可用；群聊同时命中 `trusted_feishu_chat_ids_for_group_hypertex` 与 `trusted_feishu_user_ids_for_group_hypertex` 才能调用。创建/迭代任务固定使用 `hermes` Contributor，新建 case 固定 `freestyle` 类型；执行器、模型/provider、会话、权重与路由决策均由 HyperTeX 服务端私有管理，调用侧不得请求、猜测或复述，sandbox 会删除模型或旧客户端传入的相关字段。本轮飞书附件经私有目录暂存后自动注入，不把宿主缓存路径暴露给模型；未开通群或非可信成员即使能看见工具说明也会在调用期被拒绝。非 Feishu 来源（CLI/TUI、cron 调度器、内部事件）一律放行不拦截。
...
- `group_chart_generate` 只接受 Hermes 已从文本/XLSX/CSV 提取出的结构化数据，不接受原始 workbook 路径、任意绘图代码/kwargs、URL 或输出路径。独立 renderer venv 使用固定版本 Matplotlib + Seaborn：常规业务图由 Matplotlib 模板控制排版，关系/分布/分类/回归/矩阵图由 Seaborn 渲染；进程沙箱明确禁止网络，PNG 落到当前群哈希工作区，可按用户目的地写入文档、设置封面或通过 `MEDIA:` 发送。主会话对应 `secure_chart_generate`。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v0.20.6            | 2026-08-30 | **2026-W35**：upstream `8966b0a700 → e387cbc0aa`（+337 commits，累计 +1508），覆盖 prompt/tool schema 精简、动态 schema 刷新、Gateway turn-hold/异步阻塞、图片错误恢复与 auxiliary vision 路由、skill_manage 原子批操作、anydoc 0.2.4/typed OCR、通用语音转码及 Desktop/Kanban 可靠性。逐项审计 43 active + 10 archive PATCH；26 active/28 唯一路径与 7 archive/12 唯一路径相交，合并去重为 35 条。`agent/chat_completion_helpers.py` 与 `tools/read_extract.py` 两处 3-way 冲突按语义合并；`PATCH-DOCUMENT-EXTRACTION` 进一步部分吸收，其余 active 无新增吸收，Archive 探针保持通过。深度审计修复 pytest 后台线程、对象终结器、未 await coroutine、测试返回值及部分 collection 异常仅 warning 仍计 passed 的假绿，并把 overlap 派生计数纳入终审硬门禁；五类异常均有真实 fault injection；真实图文文档复盘还修复正文图片比例、legend 基于 artist 碰撞图内优先、碰撞后横图右置/竖图下置并限制 20% 外置占比和 blue/green palette 契约，并为 X/Y tick labels 增加 tight-layout 后 10% 空间预算、倾斜与省略号回退；长 donut legend 超限时采用紧凑化而非整图失败。8 月 30 日主会话重绘失败发生在媒体工具调用前：Azure/Bedrock/Vertex 均因连接、无事件流或凭据解析网络错误不可用，384 条消息/约 333k tokens 放大了延迟，并非图片模型 fallback 全挂。终态 **39 files / 1761 passed / 0 failed / 6 skipped**，另有 2 个 test support modules；90-file bundle、43 active + 10 archive、35 active + 7 archived gates、53/53 full PATCH evidence、sandbox/identity-sync **106 passed**（含 media/chart 回归）。Gateway planned restart、Doctor、配置 v39 与 cleanup 全部闭合；npm 仅余 Electron/extract-zip 2 high，为 P2 且不影响飞书主链路。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
