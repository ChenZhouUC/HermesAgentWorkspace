...
| 8    | **Re-apply & verify patches**             | 原子回放 `patches/local-patches.diff`（3-way 成功后自动清 staged index），再按 `PATCHES.md` 分别验证 gate；受管路径 live diff、full-index 逐字节、cached 正向、worktree 反向和 index-clean 全部通过后才刷新 bundle/base。full PATCH evidence 将验证函数唯一解析为完整 pytest node ID，要求节点文件属于该 PATCH，并以隔离 JUnit + Python 调用轨迹拒绝同名歧义、跨 PATCH 复用、未登记测试文件、任一 owned production `.py` 未执行、未收集、skip/xfail/error/failure；Archive 运行当前行为或 secret-safe 退役探针。回贴后必须同时得到 current definition、launchd wrapper PID 与真实 Gateway 子进程；runtime dirty 才排空重启，无变化不制造 PID |
| 8e   | **Verify user plugins**                   | 强制验证 `PATCH-FEISHU-GROUP-SANDBOX` 等配置仓库用户插件：verifier 必须存在且可执行；结构化检查 YAML/固定脚本与 people.yaml 的 open_id↔user_id 完整唯一映射，真实解析 owner/group toolset，运行 54 条行为测试并追加 Feishu 组织身份同步回归；用 Gateway 规范化的 `[URL](URL)` reply 穿过 deferred `tool_call` 验证可信群文档删除（fake script，零外部删除）；launchd wrapper 健康由 status 证明，插件注册 trace 绑定 `gateway.status.get_running_pid()` 返回的真实 Gateway 子进程 PID。任一失败令整次升级非零退出                                                                                                                            |
...
PATCH evidence 以反引号中的完整路径建立 ownership，拒绝子串冒认和零受管文件 PATCH；每个 PATCH 的 evidence node 在独立 pytest 进程中运行，只有显式登记的模块级配置文件可用 import 作为执行证据。终态审计还要求当前升级范围精确落在 HEAD、逐个 overlap PATCH 有唯一吸收结论、canonical 完成数等于 collect 数，并在全部测试后重新验证 bundle、复跑 sandbox 与核对持久 Gateway 状态身份，防止测试副作用或时序漂移制造假绿。
