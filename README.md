...
| 4b   | Skills 镜像同步                           | `rsync --delete` 镜像上游拥有的 skill 内容，保留本地 `.bundled_manifest` / `.curator_state` runtime metadata；新增 skill 自动添加、上游孤儿自动清理，rsync 失败令整轮非零；`my-skills/` 不受影响（`PATCH-SKILLS-MIRROR-METADATA`）                                                                                                                                                                                                                |
...
| 8    | **Re-apply & verify patches**             | 原子回放 `patches/local-patches.diff`，再按 `PATCHES.md` 中每个语义补丁分别验证 gate；全部通过后才刷新 replay bundle 和 base commit。Step 8d 使用排空感知的 `hermes gateway restart`，等待 `agent.restart_drain_timeout + 30s` 并必须观察到 Gateway PID 实际替换，才确认 patched modules 已加载；旧 PID 残留、restart 非零或超时令整轮非零。`PATCH-FEISHU-GROUP-ADMISSION` 修订的 bundled `llm-wiki` 随后同步到运行时 mirror 并重建 manifest 基线 |
...
本项目维护 27 个按职责命名的活跃语义补丁：22 个工程内补丁、4 个升级期运行时补丁和 1 个外层配置仓库插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 full-index `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；`PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e 强制 verifier 管理。当前基线为上游 `d1afa160`，62 个受管文件的内层 diff 与外层 bundle（排除已知 `package-lock.json` 噪音）逐字节一致；最近完整回归为 790 passed / 0 failed，sandbox plugin verifier 每轮绑定运行时当前 Gateway PID 通过。
...
> 版本记录按 ISO 自然周（周一至周日）聚合，每周最多一条。版本与日期取该周最后一次升级，说明汇总周内 upstream 范围、PATCH 生命周期事件、最终验证和重大摩擦；同周后续升级更新既有 row，不新增 row。

| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v0.19.1            | 2026-08-03 | **2026-W32**：upstream `26e0b1c → d1afa160`（+261 commits，主题：secrets profile-scope 化约 20 条、排空重启入主干 `db3f7e4eb`、session watchdog/stall-notify 系列、file/search/patch/terminal 工具增强、a2a 插件默认 OFF、cron/approval scope 细化）。周初更新后审计发现 patch 落盘晚于 Gateway 启动，旧进程未加载合并转发展开与 Vertex hidden-thoughts 补丁，造成历史卡片仍为占位符及图片最终回复混入英文 thought。替换运行 PID 后既有修复恢复；再让主会话 DM 纯 @ 与群聊一致进入意图推断，并修复 Gateway 通用 500 字符引用上限把 12 条合并转发截到第 6 条的问题；引用资源矩阵补齐（单独 `/`、DM/`assistant_user` 附件回填、重复引用不受去重抑制）。随后修复运行 consumer 把 Feishu group 映射成 DM 的第二层回归（source-aware key 统一 + 真实 `_run_agent` 正反例回归）。画像风控复审确认问话人与历史标签只含公开白名单，补齐未知字段 private-by-default、`/background`/interim/流式 TTS 脱敏旁路，`people.yaml`/`groups.yaml` owner-rw 自动收敛 `0600`（217 条生产配置全量探针 0 泄漏）。全量补丁审计对照 +251 上游确认 27 补丁功能正常、6 归档哨兵未回滚：`PATCH-DOCUMENT-EXTRACTION` 收缩（XLSX/DOCX/IPYNB 抽取已上游，受管集合 64→62）、脚本补齐 EXIT trap 全窗口恢复/completion 失败非零/8d native exit-wait 前向兼容、清理 4 个遗留 stash。周内第二次升级拉平 `d1afa160`：3-way 仅 `approval.py` 1 处"并存"型真冲突（上游 cron 函数与本地群审批同点插入），预判热点全部自动并存，NORMAL-REPLY 对撞未触发；修正 GROUP-ADMISSION 陈旧哨兵（`_with_current_author_prefix` 已被重构移除、gate 自 `417283b` 起误报），body-prefix 子项判定为上游吸收。终态 62 files clean apply、base=`d1afa160`、25 files **790 passed / 0 failed**（−46 为收缩移出的上游测试、+7 上游新增）、七层闭环与 sandbox verifier（PID 71207）全绿；npm audit **0 vulnerabilities**，Skills mirror `+0/~1/-0`。 |
