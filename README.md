...
| `scripts/`                   | 日报/组织同步、PATCH evidence、终态审计、清理和 Wiki lint |
...
├── spawn-ledger.json      # 进程身份账本（.gitignore 排除；损坏副本同样保留）
...
| SC Claude Fable 5.1        | `CLAUDE_SC_BASE_URL` / `CLAUDE_SC_AUTH_TOKEN`                            | 与 `~/bin/claude-sc` 共用 endpoint/token，经用户级 model-provider 插件接入 |
...
当前主模型是 `azure-foundry/gpt-5.6-sol`。`~/.secrets` 仅作为人工取值来源：需要把 `AZOPENAI_*` 显式复制/映射到 Hermes 自己 `.env` 的 `AZURE_FOUNDRY_*`，Hermes 不继承 shell 中的同名或其它 provider 变量。

Fallback[0] 使用 `claude-sc` 同源的 SC Anthropic Messages endpoint：`.env` 保存 `CLAUDE_SC_BASE_URL` 与 `CLAUDE_SC_AUTH_TOKEN`，用户级 `claude-sc` model-provider 插件负责 Bearer 认证与 Claude Code 兼容请求头，不修改 `hermes-agent` 上游源码。
...
# 私有模型不依赖 models.dev；显式声明已实测的图片能力。
providers:
  claude-sc:
    models:
      claude-fable-5-1:
...
> **三档统一策略**：GPT-5.6 Sol / Claude Fable 5.1 / Gemini 3.7 Flash 的 context metadata 分别约 1.05M / 1M / 1.048576M。统一 `threshold=0.7` + `threshold_tokens=700000`，并关闭 GPT-5.5 的 85% autoraise，使 provider 切换后的 compressor 始终在约 700k 触发，给 system prompt、工具 schema、输出预算与估算误差留下约 300k 余量。

> **主动切换与 fallback**：严格模式下 `/model` 只能在上述三条 route 间做 session-scoped 切换，`--global` 和链外模型会被拒绝。切到 SC Claude 时会跳过 fallback 链中重复的 SC route、保留 Vertex；切到 Vertex 时先回退 SC Claude、再跳过重复 Vertex。`auxiliary.compression` 始终优先 Vertex，不因主模型切换而改写；只有 Vertex 摘要失败时，备用摘要模型才随“当前主模型”变化。

> **多模态 native-first + sidecar**：三条 route 的图片均走原生输入：Azure GPT-5.6 Sol、SC Claude Fable 5.1、Vertex Gemini 3.7 Flash 都直接接收像素。GPT-5.6 Sol 与 Fable 5.1 不作为视频/音频原生输入模型；此时仅把当前音频/视频及最多 4,000 字 caption/引用交给 fallback 链中的 Gemini Flash sidecar，主会话模型与历史不切换。普通语音优先本地 STT，全部失败才旁路；PDF/纯文本优先本地可信抽取，抽取为空或 PDF 覆盖率探测发现扫描/图片页缺口时才旁路，DOCX/XLSX/PPTX 等未列入 Flash 原生 MIME 的格式不外发。PPTX 中任何含图片、图表或 OLE 的页面都会在抽取块顶部列为 `PPTX visual coverage status: INCOMPLETE`，即使同页已有标题文字；纯图片 deck 也不会被误报为已完整阅读。成功的 native、抽取和 sidecar 结果不再向模型暴露宿主 cache 绝对路径；全部读取链失败时注入明确的 `FAILED` 状态并要求如实告知用户，不允许退化为“自行打开路径”。
...
| `/new`、`/reset`、`/resume` 或显式会话终止 | 回到 `config.model`       | 配置链                                       | 配置的压缩模型                     | 形成逻辑会话边界并清除旧 override                                      |
...
- fallback 链不会因为主动切换主模型而失效。默认 Azure 主模型按 `SC Claude → Vertex` 容灾；主动切到 SC Claude 后跳过重复 SC route、保留 Vertex；主动切到 Vertex 后保留 SC Claude 并跳过重复 Vertex。具体型号始终动态来自配置，不在补丁中写死。
...
- v0.21.1 已取消 idle/daily 自动会话重置，原 `session_reset` 配置不再生效。会话通过 `/new`、`/reset` 等显式操作建立新边界；空闲压缩与存储归档是独立策略。
...
- 对有文本层的 PDF、DOCX、XLSX、PPTX 等常见文档，Gateway 先在可信边界完成抽取，再以 `[Begin extracted attachment: ...]` / `[End extracted attachment]` 块注入**当前这一轮**。看到该块即代表模型已经拿到文档的可抽取文本，不应再笼统表述为“无法读取”；若块内同时有 PDF/PPTX 的 `INCOMPLETE` 标记，只能确认文字层，不能声称已检查缺失的扫描页或图片型幻灯片。
...
| ---------------------------------------- | ------------------ | ------------------------------------ | -------- | ------------------------- |
| 主 agent (`agent.reasoning_effort`)      | high               | gpt-5.6-sol（Azure，Responses 路径） | 不展示   | 不展示                    |
| 子 agent (`delegation.reasoning_effort`) | high               | 默认继承主模型                       | 不展示   | 不展示                    |
| Fallback[0] (`fallback_providers`)       | high               | claude-sc/claude-fable-5-1           | 不展示   | 不展示                    |
...
- 主模型 gpt-5.6-sol 走 `azure-foundry`（codex-az 同源 Azure 端点，Codex Responses 路径），对话本身仍可能返回 reasoning；但当前 `display.show_reasoning: false`，前端不展示 thinking 段落。若临时改回 `true`，会重新显示前缀 `💭 **Reasoning:**` 的内容。
- SC fallback 由 `~/.hermes/plugins/model-providers/claude-sc/` 提供，复用 `~/bin/claude-sc` 的 endpoint/token，并保留 Hermes 自己的工具循环、会话与 fallback 状态；Fable 5.1 已实测文本、工具调用与图片输入。
...
- 从 Azure/SC Claude 会话 fallback 到 Gemini 时，历史工具调用没有 Gemini 原生 `thought_signature`；`PATCH-GEMINI-CROSS-PROVIDER-TOOL-HISTORY` 会在出站副本中注入官方兼容哨兵，避免末级 Vertex 立即 HTTP 400。
...
主模型是 `azure-foundry/gpt-5.6-sol`；标准 Vertex 承担四件事：

1. **fallback[1]** —— Azure 与 SC Claude 都失败后接管
...
> 本 README 下文出现的所有 `hermes gateway restart` 都继承同一前置要求：先运行 cleanup dry-run，确认所有 outer/inner operational scripts 与 ignored 路径均已分类且 `review=0`，再执行 apply。`review` 项必须先更新 `scripts/cleanup_policy.json` 和测试，不能直接删除；Gateway 在优雅退出后短暂写入的 `.clean_shutdown` 是下次启动消费的 crash-recovery receipt，必须按运行态文件保留。
...
- `pre_tool_call` 读取 ContextVar：若 `platform != "feishu"` 直接放行；若 `chat_id` 在 owner 白名单里，普通工具直接放行；HyperTeX 只允许 create、iterate 与只读 `tasks_get` 三个入口，创建/迭代固定 Contributor / `freestyle`，自动暂存本轮附件；可信群还可显式引用当前群工作区内由受控工具产生的数据文件，sandbox 会校验 realpath 不越界、拒绝 symlink/缺失文件、去重并复制到私有 staging，不接受任意宿主绝对路径。随后删除 `agent` / `model` / `provider` / `executor` / `routing` 等调用侧私有路由提示。执行路由只由 HyperTeX 服务端决定且不向调用侧暴露。每个入站 turn 最多放行一次 HyperTeX 调用：创建后同轮轮询、查询失败后同轮重试、list/get_case fallback 都在调用 MCP 前被拦截，避免本地 transport 异常占住主会话。群聊优先应用群专用 allowlist，不继承 outsider-DM 的 `vision_analyze` / `image_generate` 放行；HyperTeX 工具面虽加入 `feishu_group`，调用期仍要求当前 `chat_id` 和 `user_id` 分别命中两个 HyperTeX allowlist。MCP/delete 执行授信名单独立于 `feishu.assistant_user_ids`，新增维护者不会顺带获得群聊 @人触发 Hermes 的 admission 语义。其他私聊才使用基础安全 allowlist。配置解析失败时对 Feishu fail closed。
...
- 群聊不拥有 `terminal` / `process` / `write_file` / `patch` / `vision_analyze` / 原生 `image_generate`。`group_cache` 用结构化参数完成当前群工作区内的数据读写；`feishu_doc_manage` 只映射固定文档 action，其中 `insert_image` / `set_cover` 接受生成工具返回的相对 `workspace_path`，或当前 turn 图片附件的 `attachment_index`。两类来源互斥，文件需通过 magic/type、普通文件、symlink、20 MiB 和工作区/provenance 检查；已有文档必须在当前消息或显式引用中出现。正文图片支持准确 top-level index 或唯一标题/段落锚点、对齐、caption 和尺寸；封面支持裁切偏移。脚本按飞书契约执行 document-scoped media upload + block/document PATCH，并在失败时补偿新图片块、旧封面与版本表。
- `group_image_generate` 使用可替换的私有后端执行 catalog → 上传引用图 → image job → 轮询 → 下载链路。当前通过 `trusted_feishu_chat_ids_for_group_image_generation: ['*']` 向所有已准入及未来加入的飞书群开放，每个入站 turn 最多调用一次；各群仍写入独立的哈希工作区。若用户指定文档目的地，生成结果的 `workspace_path` 直接交给 `feishu_doc_manage`，只有同时要求群消息副本时才发送 `MEDIA:`。主会话默认使用同一协议的 `secure_image_generate`，后端替换不改变 skill 或业务侧调用方式。
- `group_chart_generate` 只接受 Hermes 已从文本/XLSX/CSV 提取出的结构化数据，不接受原始 workbook 路径、任意绘图代码/kwargs、URL 或输出路径。独立 renderer venv 使用固定版本 Matplotlib + Seaborn：常规业务图由 Matplotlib 模板控制排版，关系/分布/分类/回归/矩阵图由 Seaborn 渲染；笛卡尔图默认同时显示 X/Y major grid，并明确显示底部 X / 左侧 Y major ticks；坐标轴 tick-label 带宽上限为绘图区对应尺寸的 20%，完整文本优先通过旋转、换行和降低刻度密度适配，不再用省略号损失信息；标题、注释与外置 legend 只预留一次实际所需空间，避免重复固定 gutter，使主绘图区保持紧凑。进程沙箱明确禁止网络，PNG 落到当前群哈希工作区，可按用户目的地写入文档、设置封面或通过 `MEDIA:` 发送。主会话对应 `secure_chart_generate`。
...
private_doc_workspace_root: ~/.hermes/tmp/feishu-doc-assets
...
group_doc_image_max_bytes: 20971520
...
- 调整飞书文档脚本：只能在代码中的 action→固定文件映射和 `allowed_feishu_script_actions_for_outsider_groups` 两处显式加入，并为参数约束补测试；禁止改回目录级脚本 allowlist。图片/封面 action 还必须同步 `manage_doc_image.py` API 契约哨兵、`test_manage_doc_image.py` 和 runtime 注册 trace，确保后续 Agent/CLI 更新无法漏适配而继续报绿。
...
# 5. 运行 sandbox、Feishu 组织身份同步与文档媒体写入行为回归，并检查当前 gateway PID 的 runtime 注册日志
```
...
# 所有修改与最后一次 reconcile 之后：单一终态 JSON（含最终 cleanup）
bash ~/.hermes/hermes-update.sh --final-audit --json
```
...
| 步骤 | 操作                                      | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | Preflight + transaction                   | 确认 hermes 可用、git 仓库存在、index 干净；先运行 gate 集合自测和 `mode=quick` PATCH evidence，证明结构、专用探针与 runtime contract 在任何 mutation 前闭合。quick 不校验尚未由 Step 2 捕获的 live overlay ↔ canonical bundle parity，后者只在 Step 2/8c 与 full/final audit 执行，保证手工解冲突后的 `--reconcile` 可重入。所有数量从脚本/JSON 动态读取，不在 README 复制；quick 不能替代终态 full evidence。`--update` 仅在无未完成事务时开放一次官方获取，默认/`--reconcile` 固定事务 SHA 且不做网络探测；staged 用户改动或 evidence/gate 漂移 fail closed                                                                                                                                                                                                                                         |
...
| 8    | **Re-apply & verify patches**             | 原子回放 `patches/local-patches.diff`（3-way 成功后自动清 staged index），再按 `PATCHES.md` 分别验证 gate；受管路径 live diff、full-index 逐字节、cached 正向、worktree 反向和 index-clean 全部通过后才刷新 bundle/base。full PATCH evidence 将完整 node ID 精确绑定 path/class/function，要求每个可执行 `test_*.py` 至少 collect 一个 node；`conftest.py` / helper 等 support module 单独登记并做语法检查，不再计入测试文件数。隔离 JUnit + Python 调用轨迹拒绝同名歧义、跨 PATCH 复用、未登记测试文件、任一 owned production `.py` 未执行、skip/xfail/xpass/零执行/error/failure；Archive/dedicated pytest 使用同样的 strict JUnit，Archive overlap 从声明路径独立计算。回贴后必须同时得到 current definition、launchd wrapper PID 与真实 Gateway 子进程；runtime dirty 才排空重启，无变化不制造 PID |
| 8e   | **Verify user plugins**                   | 强制验证 `PATCH-CLAUDE-SC-PROVIDER`、`PATCH-FEISHU-GROUP-SANDBOX` 等配置仓库用户插件：Step 8e verifier 数组必须与 PATCH evidence external registry 精确一致；verifier 必须存在且可执行。SC verifier 检查 fallback/config/secret、provider 注册、Claude Code 请求身份、reasoning/cache/tool/image 契约和 Gateway freshness；sandbox verifier 继续执行严格行为测试与运行时注册核验。任一失败令整次升级非零退出。                                                                                                                                                                                                                                                                                                                                                                                         |
| 9    | `hermes doctor` + `hermes gateway status` | 验证更新结果；若 gateway 因 update / post-patch restart 处于未加载状态，脚本会自动补一次最终恢复后再判定                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 10   | **`--final-audit --json`**                | 必须晚于最后一次 reconcile、playbook/evidence/cleanup/verifier 修改：单一入口重跑逐 PATCH matrix、canonical 文件套件、Archive probes、bundle、README/PATCHES/Markdown/Wiki、Doctor 安全/配置状态、sandbox runtime trace、Gateway 双 PID 与最终 cleanup。只有 `status=ok / mode=full` 才能报告完成；用户明确要求 commit 时，pre-commit hook 后还要再次运行并要求 outer clean                                                                                                                                                                                                                                                                                                                                                                                                                            |

Step 8e 当前先执行 `PATCH-CLAUDE-SC-PROVIDER` 的 13 项离线契约检查，再为 `PATCH-FEISHU-GROUP-SANDBOX` 运行 129 条行为测试并继续执行身份同步与飞书文档媒体脚本回归；owner DM 的 fixed reader → chat-hash 私有 workspace → HyperTeX staging 正例、当前消息 Markdown 资源链接后紧接 CJK 正文的 provenance 正例、全 `HERMES_HOME` traceback 路径脱敏，以及历史手工目录/任意外部路径/未引用资源反例均属于硬门槛。未完成的同 turn claim 不因全局容量被驱逐；明确完成后释放、异常遗留按安全 TTL 退场。
...
本项目维护 47 个按职责命名的活跃语义补丁：38 个工程内补丁、7 个升级期运行时补丁和 2 个外层配置仓库用户插件补丁。完整责任边界、依赖、验证 gate 和上游吸收条件见 [`patches/PATCHES.md`](patches/PATCHES.md)。工程内改动仍以单一 full-index `local-patches.diff` 作为原子 replay bundle，但每个语义补丁在 Step 8b 独立验收；preflight quick audit 只运行 quick-eligible 探针并明确保留 full-only 项，终态 full evidence 将验证函数唯一绑定完整 pytest node ID、要求节点文件属于该 PATCH、禁止跨 PATCH 共用同一 node，并用调用轨迹证明 evidence 逐文件进入全部 owned production `.py`，不能以同 PATCH 单点命中代替其余实现文件；runtime/dedicated/archive/external 四类非 pytest PATCH 则由统一 registry 自动执行 callable probe 并写入本轮 `probe_results`，缺执行回执、重复/未知回执或 deferred 状态全部拒绝。`--final-audit --json` 再以 `--file-retries 0` 和后台线程、对象终结器、未 await coroutine、返回值与 collection 异常 warning-as-error 运行 canonical suite，统一闭合 Archive probes、bundle/docs/Wiki、sandbox runtime、Gateway PID 与最终 cleanup。`PATCH-UPDATE-TRANSACTION-PIN` 保证一次升级只取得一个 upstream snapshot；`PATCH-UPDATE-FLEET-RECEIPT-FRESHNESS` 让当前 live fleet 身份覆盖陈旧失败 receipt 的假警报；`PATCH-ENV-AMBIENT-CREDENTIAL-ISOLATION`、`PATCH-MODEL-CONFIGURED-ONLY`、`PATCH-MCP-TASKS-ASYNC-HANDOFF`、`PATCH-MCP-STDIO-WATCHER-LIFECYCLE`、`PATCH-TEST-RUNTIME-STATE-ISOLATION`、`PATCH-FEISHU-ADMIN-CONTROL-SCOPE`、Feishu scope/response/tool-call 系列与 `PATCH-GATEWAY-RESTART-CLEANUP` 继续守住凭据、模型、测试隔离、异步任务、群聊和重启边界；`PATCH-COMPACTION-LIFECYCLE-SILENCE` 已由 upstream v0.20.6 吸收并转为归档回归探针；`PATCH-CLAUDE-SC-PROVIDER` 与 `PATCH-FEISHU-GROUP-SANDBOX` 不进入内层 diff，由 Step 8e verifier 管理。当前基线为上游 `28bf67b17607af53c147431f117d07955dc870da`；受管路径与 bundle 必须逐字节一致，精确数量以只读入口为权威。最近完整回归见本周版本记录。

管理员会话命令与 Gateway 管理权限独立登记为 `PATCH-FEISHU-ADMIN-CONTROL-SCOPE`；原生短命令的群聊禁审批修订既有 `PATCH-FEISHU-GROUP-APPROVAL`。两者虽然共享部分源码和测试文件，但各自维护不同不变量、独立测试节点和上游吸收条件；具体归属理由只在 `PATCHES.md` 对应四段定义中维护。

PATCH evidence 以反引号中的完整路径建立 ownership，拒绝子串冒认和零受管文件 PATCH；每个 PATCH 的 evidence node 在独立 pytest 进程中运行，完整 node ID 不再退化成裸函数名，只有显式登记的模块级配置文件可用 import 作为执行证据。每个可执行 PATCH 测试文件都必须真实 collect；`conftest.py` / helper 等 support module 单独报告并做语法检查，不再充当“零测试的已回归文件”。Archive/dedicated 探针也必须用 hermetic JUnit clean pass；Archive overlap 按其声明路径计算，不因退出当前 bundle 而漏轮询。终态审计还要求当前升级范围为 ancestor→HEAD、逐个 overlap PATCH 有唯一吸收结论、canonical 完成数等于 collect 数、Step 8e 与 external registry 同集，并在全部测试后重新验证 bundle、复跑 sandbox、完成 cleanup 后核对 Gateway PID/start-time 身份和审计前后外层工作树 fingerprint，防止测试副作用或时序漂移制造假绿。

补丁由 `hermes-update.sh` 全自动管理：事务层由 `PATCH-UPDATE-TRANSACTION-PIN` 区分唯一一次 `--update` 和任意次 no-network `--reconcile`；Step 1 先运行 patch-gate 集合自测，Step 2/8c 以 full-index 保存 replay bundle，并在 canonical bundle/base 发布前强制完整受管集合、逐字节、cached 正向、worktree 反向和 index-clean 闭环；Step 3/4 执行 `PATCH-NPM-DEPENDENCY-HYGIENE`，Step 4b 执行 `PATCH-SKILLS-MIRROR-METADATA`，Step 7 保留已上游合并的 completion 回归 sentinel，Step 8 回放工程内 diff 并逐个验证语义 gate，Step 8d 仅在运行态脏时排空在途任务、重启 gateway 并要求 PID 替换，无变化 reconcile 不重启；Step 8e 强制回归 `PATCH-FEISHU-GROUP-SANDBOX`。fetch retry 另有 side-effect-free `--self-test-fetch-retry`，所有非 pytest PATCH 探针由 evidence registry 轮询而非手写 main 调用。若 Step 8d 后又修了运行时代码/配置，`hermes-update.md` Step 5b 要求再次执行同样的终态运行屏障并把 verifier 绑定到最终 PID。`PATCH-UPDATE-GATE-EXIT-STATUS` 保证 staged/部分 overlay、apply、sentinel、gate/探针集合漂移、canonical flake retry、冲突、部分或空 bundle、replay integrity、runtime sync、Gateway 未实际重载或外层 verifier 任一失败都令升级非零且不得误报成功；常规更新禁止用短宽限 `stop/start` 强杀来换取新 PID。

手动恢复 `hermes-agent/` 内补丁：

```bash
cd ~/.hermes/hermes-agent && git apply ~/.hermes/patches/local-patches.diff
# 若有冲突：git apply --3way ../patches/local-patches.diff，逐处按语义解决；
# 然后 git add <冲突文件> && git restore --staged -- <全部 staged patch 文件>，
# 再运行 hermes-update.sh --reconcile 让 Step 2 捕获完整 resolved overlay。
# 只有 3-way 无法展开时才退回 git apply --reject 并处理 .rej。
...
| `/model gpt-5.6-sol --provider azure-foundry`      | 当前会话切回默认主模型               |
...
| 类型                | 存放位置               | 管理方式                                                  |
| ------------------- | ---------------------- | --------------------------------------------------------- |
| **Hub 官方 Skills** | `~/.hermes/skills/`    | Hermes/upstream 自动管理；本 profile 默认隐藏，不参与发现 |
| **自定义 Skills**   | `~/.hermes/my-skills/` | 手动维护，随主仓库入库，通过 `external_dirs` 加载         |

#### Hub 官方 Skills 更新机制

官方 Skills 目录 `~/.hermes/skills/` 只负责保存 upstream 内容，更新时按两层机制与上游保持同步；默认不会把这些内容暴露给 agent：

1. **运行时同步**（`hermes` 每次启动时）：自动运行 `sync_skills`，将新 skill 复制到本地、更新已变更的 skill。上游移除 skill 时仅清理其 manifest 记录，对应目录仍可能留作本地孤儿；输出 `removed from manifest` 不代表目录已删除。
2. **更新时镜像**（`hermes-update.sh` Step 4b）：使用 `rsync --delete` 将 `hermes-agent/skills/` 完整镜像到 `~/.hermes/skills/`，确保新增、更新、删除三向同步。官方源文件不进入本地 replay patch；运行态 metadata（manifest/curator/usage/hub state）按镜像策略保留。

默认配置为 `skills.hide_bundled: true`，因此官方 bundled Skills 仍会更新到磁盘，但不会出现在 Skills index、`skills_list`、`skill_view` 或 slash command 中。设为 `false` 才重新显示官方 Skills。自定义 skill 存放于 `~/.hermes/my-skills/`（通过 `external_dirs` 注册），镜像同步**不会触及**自定义 skill；本地维护版 `llm-wiki` 位于 `my-skills/research/llm-wiki/`，随外层仓库演进。
...
当前共有 22 个 local skills；以 `hermes skills list` 和 `my-skills/*/*/SKILL.md` 为权威来源：
...
| productivity         | `educational-doc-writing`, `excel-processing`, `feishu-docs`, `feishu-groups`, `feishu-people-search`, `habitat-mcp-operations`, `hypertex-mcp`, `technical-investor-docs`, `technical-translation` |
| red-teaming          | `agent-reconnaissance`                                                                                                                                                                              |
| research             | `llm-wiki`, `wiki-content-extraction`                                                                                                                                                               |
...
每次先只读拉取完整在职组织快照，按 `config.yaml` 的唯一 owner open_id 和稳定的 leader ID 递归取得直属及间接下属，再逐人完整翻页查询飞书汇报。日报按模板名包含“日报 / 日汇报 / 每日汇报”或英文 daily 识别；不同日报模板均计入，以 task_id 去重，不把周报混入。接口首个 page_token 必须为空字符串、page_size 最大 20；查询有节奏地串行执行，并对实测 HTTP 400 / 99991400 限流及临时网络错误有界重试。任一人的查询、分页、正文或摘要覆盖校验失败，整轮失败，不发送不完整汇总。

人数、篇数、覆盖率、多篇提交人数和各模板篇数由代码计算并置于消息顶部，另列本窗口未查询到日报的人员；这不代表他们缺勤或未完成工作。模型使用现有 Hermes 路由且不提供工具，只负责归纳进展、问题/阻塞和计划；长资料完整分批，逐批核对全部汇报 ID。主会话沿用 nightly 的地址并限定在 sandbox owner 私聊列表，另可向显式开启 morning 的群额外投递。零篇日报仍发明确的零统计，模型不生成虚构内容。

群聊统一在 `groups.yaml` 中配置两个独立布尔开关：`morning_greeting` 缺省为 `false`，只有显式 `true` 才接收晨报；`nightly_greeting` 缺省为 `true`，显式 `false` 才停止晚安。当前所有群都显式写出两个字段，仅“AI 解放生产力”开启 morning，其他群关闭；nightly 保留此前各群选择。修改下次运行即生效，不需重启；主会话晨报不受群开关影响，`--preview` / `--force-preview` 历史回放始终只投主会话。每个群独立保存 UUID 和逐条回执，群投递失败重跑不会重复发主会话；投递前再次检查开关，关闭/移除群会取消本轮该群待发消息。

晨报固定为五块：“统计概览 → 统计范围 → 产品迭代 → 项目交付 → 主要卡点”。产品迭代按产品模块/主题聚合不针对特定项目的平台、APP、嵌入式设备、算法、数据逻辑、通用流程和团队赋能；项目交付按具体项目合并研发、测试、算法、实施等情况。主要卡点只按模块/项目汇总重要的未解决问题、推进瓶颈、外部依赖或明确待核实事项，不把所有待办自动当成卡点。下一步计划直接并入对应产品模块或项目条目，不再单列。模型返回结构化分组，代码合并同名主题为一行，并拒绝旧三段式、泛化项目名或把多个项目用斜杠混为一组；缺乏事实时不推导或编造。

Markdown 排版只使用上述五个加粗标题，下面以 `- 【产品模块或项目名称】：人员及内容；下一步……` 的形式一条主题一行，不为每个主题另占小标题。固定发送四条完整消息：① 统计概览 + 统计范围；② 产品迭代，标题同行原生 @孙可天；③ 项目交付，标题同行原生 @张文华；④ 主要卡点，标题同行原生 @当前 owner（本机为周琛）。两个指定负责人从本次完整在职通讯录按姓名唯一匹配，owner 按配置身份解析；缺失或同名歧义会阻断发送，不猜测 ID，也不把普通 @文本冒充原生提及。测试回放也遵循这四条布局，仍只发送到 owner 主会话。

正文人名加下划线。发送层使用飞书 `post` 的原生 `bold` / `underline` 样式、bullet 字符和 `at` 元素；实测 `post/md` 的兼容内容会把 `<u>` 保留成普通字符，因此不依赖 HTML 下划线标签。显示姓名优先取中文全名，无中文时保留英文名，去掉括号昵称。每条消息在投递前核验完整 payload 大小，不再按字数任意切开主题；如果仍超出安全长度会整轮拒绝发送，不截断内容。四条消息分别保存回执与提及身份，失败时只补投尚未成功的条目。
...
| 版本               | 日期       | 周摘要                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v0.21.2            | 2026-09-12 | **2026-W37**：upstream `79445a496c → 28bf67b176`（+1936 commits），v0.21.2 / config v44。周内补齐飞书顶层 files、真实入站回归、确定性称呼和 SC fallback；适配事件、媒体、nonstream/MCP batch/profile 重构，fleet receipt 部分吸收，仅保留严格 PID inventory。群审批覆盖隔离后端、纯扫描告警、卡片/文字与原生 slash 确认；管理命令仅允许 owner 群内新会话及主 DM Gateway 管理，稳定身份按平台多 ID 交集授权，拒绝先于取消任务。修复 Skills external 缺省创建与非法策略放行；审计器拒绝 unittest skip/expected-failure，镜像和 CJK 探针验证真实生产入口。最新 +204 commits 加强 multiplex 授权/MCP 缓存、turn generation 与失败重放边界；三文件冲突合并 scope-aware Feishu 配置和本地只读 Skills，退役 Wisdom 工具但保留私有数据，移除已删除模块的缓存规则。curator 官方迁移为 14 天 stale / 30 天 archive，保留自定义技能治理。最终 canonical **46 files / 2891 passed / 0 failed / 6 skipped**，另有 2 个 test support modules、full PATCH evidence **2897 collected**、57/57 PATCH、22/22 probe、124-file bundle、38 active + 7 archived gates、sandbox/identity-sync **161 passed**；74 项 auditor 与 22 项 cleanup 回归通过。2623 → 2891 的覆盖增量：前一提交 owner 多 ID +228、本轮配置组合 +3、上游移除 editorial 测试 -5、本次注册入口 +41、超时双路径 +1。新增默认/独立 profile 配置接线与 env 优先反例，定向 multiplex 回归通过。199 个 Python 包保留，仅 Hermes 升版。P2：npm 11 项（2 moderate / 9 high）限于精确 pin 的 Desktop/build/test 链，待上游 bump；cryptography metadata 来自安全 override，均不影响飞书主链路。ChatBI 早期占位提示经正式加载复核，不写 secret 到 tracked YAML。同 SHA 全量轮询修复 Skills 缓存/平台优先级及 MCP Tasks untrusted 审批旁路；41 项入口回归持续执行。依赖 pin 精确核对 Feishu extra/feature，迁移执行后核对 env 保持与完整 archive，新增四个假绿反例。全部目标保持固定，未 fetch/pull。终审新增独立 Codex watchdog 测试 PATCH：辅助调用保留 50ms 预算、event/真实 watchdog 和 owner 清理；主请求以事件同步检查首次/重连/停滞阶段，五个 deadline/phase 注入反例均失败。                                                                                                                                                                                                                                |
| v0.21.0            | 2026-09-05 | **2026-W36**：upstream `11c8c05dc3 → 79445a496c`（+5603 commits），升级到 v0.21.0。官方更新完成 Python/Node 环境、web build、config v39→v40 与 bundled skills 镜像（新增 `research/llm-wiki`）；本地 44 个 active + 10 个 Archive PATCH 随上游大规模模块拆分迁移到 `turn_*`、`run_*`、`mcp_tool_*`、`slash_commands_*` 与 `doctor_*` owner modules。深审补齐 Feishu AI-authorship/冷却、原生 mention 与 Markdown post、configured-only model、MCP task/watcher、multimodal sidecar、history retention 和群聊 capability scope 的回归证据，并同步升级器及 sandbox verifier；同 SHA 轮询又实抓 live Gateway 已匹配 HEAD、历史失败 receipt 却永久误报 restart pending，新增 `PATCH-UPDATE-FLEET-RECEIPT-FRESHNESS` 与 final-audit 负向门禁修复；Data Pipeline Workshop 真实复测进一步补齐“裸 @周琛 + 直接引用 AI Markdown 长文”评分入口，现场内容从旧布局 `0.0` 恢复为 `0.9`，并修复自动 @消息者挤占首 token、导致表格/列表等块级 Markdown 退化的问题，以多人 @、代码边界和 post fallback 真群/单测共同验收。为缩短后续回归，逐 PATCH 隔离 pytest 改为有界并行，final-audit 并发执行 full evidence、canonical suite、Wiki lint 与 Doctor，同时保留 post-canonical sandbox、cleanup、bundle 和 Gateway identity 的顺序门禁。当前 canonical patch suite **44 files / 2206 passed / 0 failed / 6 skipped**，另有 2 个 test support modules、full PATCH evidence **2212 collected**、118-file bundle、36 active + 7 archived gates、sandbox/identity-sync **161 passed**。npm 仅余 Electron/extract-zip 2 high，修复需 `--force` 升到超出上游声明范围的 Electron 40.10.6，按 P2 保留且不强制升级。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
