...
| MCP SDK 主版本升级后本地 extension 仍使用旧字段或非正式请求类型                   | 2026-08-19 从 MCP 1.28.1 升到 2.0.0 后实抓两层兼容问题：① Pydantic 模型把 `CallToolResult.isError` 属性改名为 `is_error`，camelCase 只保留序列化 alias；② SDK 2.x `ClientSession.send_request()` 在序列化前读取 `type(request).name_param`，generic `RootModel` 会让所有 task-aware tools/call 在到达 server 前报 `AttributeError: name_param`。处置：错误字段双拼写读取；tools/call 始终使用 SDK 正式 `CallToolRequest`，只有 custom task methods 用显式 `name_param=None` 的 raw request；结果校验优先公共 `validate_tool_result`、回落旧私有方法。测试必须模拟真实 SDK 的请求类元数据访问，不能只保存 `model_dump()`。上游原生吸收 `PATCH-MCP-TASKS-ASYNC-HANDOFF` 或 extension 不再自行构造请求/结果后删除本 row。                                                                                                                                                             |
