...
> **用法**：在 Claude Code 会话中说一句 `阅读 ~/.hermes/hermes-update.md 按计划做`，agent 即按本 playbook 一条龙完成"升级内层官方源码 checkout + 回贴本地 patch + 对齐外层监管记录/文档"，中途不打断、不追问。
>
...
**补丁功能回归**（sentinel grep 之外的行为验证）：patch 回贴成功后，用 `venv/bin/python -m pytest -q` 跑 `PATCHED_FILES` 里全部 `tests/**` 文件，与 `PATCHES.md` § 当前版本摘要里记录的上轮基准（passed 数）对比。出现新失败先定性：`git stash push -- <相关源文件+测试>` 后在裸上游复跑——裸上游同样失败则是上游自身问题（记录进摘要与下方摩擦表，不动代码）；仅打补丁后失败才是真回归，需要修补丁。
...
| `test_approval.py` 的 verifier temp cleanup 测试在 macOS 失败   | 上游 `0c8bcd339` 的 `_is_verification_artifact_cleanup` 对 temp_dir 做 `realpath`（`/tmp`→`/private/tmp`）而 operand 不做，Darwin 上恒不匹配，其自带测试预期失败；裸上游同样失败、与本地 patch 无关，行为等同该修复落地前（清理仍走审批）。上游修复后删除本 row                                                                 |
