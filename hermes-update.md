...
toolchain_audit_state:
  ...
  last_deep_audit_outer_commit: 9e88f091489af58cdb071ee0e82609d504b954cf
  trigger: explicit-upgrade-plus-thread-warning-false-green
```
...
| pytest 后台线程异常只发 warning，PATCH/canonical 仍计为 passed | 2026-08-29 升级到 `e387cbc0aa` 时，`test_direct_session_db_flushes_share_marker_claim` 的 `_BarrierDB` 未跟进上游新增的 `SessionDB.flush_token_counts()`，工作线程抛出 `AttributeError`，pytest 主断言仍通过并只发 `PytestUnhandledThreadExceptionWarning`；若只看 exit code、JUnit testcase outcome 或 passed 数会形成真实假绿。处置：补齐 fixture 的 no-op `flush_token_counts()`，并由 `scripts/test_patch_evidence.py` 的逐 PATCH/Archive/dedicated pytest 入口、`scripts/final_upgrade_audit.py` 的 canonical runner 和 `plugins/sandbox/verify.sh` 统一传 `-W error::pytest.PytestUnhandledThreadExceptionWarning`；`test_patch_evidence_auditor.py` 用真实后台线程 fault injection 证明旧行为会被拒绝。若未来 pytest 默认把 unhandled thread exception 作为失败且所有受管 runner 可证明继承该默认值，可删除显式 warning filter 与本 row。 |
