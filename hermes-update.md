...
| 重写 `PATCHES.md` §「最近一次升级」摘要后活跃 PATCH 少一个 | 2026-08-11 实抓：按 `s.index("\n---", i)` 定界旧摘要会**跨过紧随其后的 `### [PATCH-*]` 定义块**（摘要与下一个补丁块之间的 `---` 不是摘要的结束符），一次替换静默吞掉 `PATCH-SKILL-CREATE-ROOT` 整块，Step 5 第 3 条断言（活跃块数 vs 注册表口径）才暴露。改写摘要时定界必须用**下一个 `### [PATCH-` 标题**而非 `---`；若已吞块，从 `git show HEAD:patches/PATCHES.md` 取回该块按原顺序插回，再复跑 Step 5 断言。上游吸收无关，本 row 长期有效，除非摘要节与补丁定义节被拆成两个文件。 |
