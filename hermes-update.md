...
| 群聊授信名单只登记 open_id，将来 tenant `user_id` 开始到达就会集体失效 | 2026-08-20 起 `plugins/sandbox/config.yaml` 的 `trusted_feishu_user_ids_for_group_{mutations,hypertex}` 与 `config.yaml` 的 `feishu.assistant_user_ids` **只登记 app-scoped `open_id`**，唯一来源是 `people.yaml`。依据：adapter `primary_id = user_id or open_id` 每事件只产出一个值，实测 `logs/agent.log{,.1}` 全量 `sender=user:` 500+ 条真实入站 **100% 为 `ou_` 形态、短 ID 0 次**（本 app 无 contact scope），且 `_sender_is_configured_assistant_user` 对三种形态任一命中即通过——故旧的双形态登记是死条目，反而让"加人"必须抓包或调 contact API。**风险**：若本 app 日后取得 contact scope 使 `sender_id.user_id` 开始到达，`primary_id` 会翻转为短 ID，届时**所有**成员一次性失配、群聊 HyperTeX 与文档删除集体拒绝（fail-closed，是拒绝而非越权）。**判定**：症状为可信成员被报"当前身份不是维护者"或 HyperTeX 静默拒绝，而 `sandbox: registered` trace 里名单正常；此时 `grep -o "sender=user:[A-Za-z0-9_]*" logs/agent.log` 若出现非 `ou_` 形态即确诊。**处置**：为名单每个成员补齐短 ID（来源仅 `sender_id.user_id` 或 `GET /open-apis/contact/v3/users/:id?user_id_type=open_id`，不能由 open_id 离线推导），并同步 `verify.sh` 的 `expected_mutation_users` / `expected_hypertex_users` 与运行态 trace 循环。**退场条件**：上游或飞书侧确定 `primary_id` 形态恒定，或 Hermes 改为同时比较三种形态的规范化身份（那时单形态登记天然安全），可删除本 row。 |
