...
| MCP SDK 主版本升级后本地 extension 仍读取旧字段名                                 | 2026-08-19 从 MCP 1.28.1 升到 2.0.0 后实抓：Pydantic 模型把 `CallToolResult.isError` 属性改名为 `is_error`，camelCase 只保留序列化 alias；直接属性读取会让 Tasks extension 的普通结果验证崩溃。处置：extension 与 core 一样用 snake/camel 双拼写 helper，并成对测试成功结果执行 validation、error 结果跳过 validation；以后 MCP SDK 字段迁移先对照 SDK 模型而不是只看 wire JSON。上游原生吸收 `PATCH-MCP-TASKS-ASYNC-HANDOFF` 或 extension 不再自行构造 `CallToolResult` 后删除本 row。                                                                                                                                                                                                                                                                                                                                                                                            |
