...
Preflight 的 `--self-test-patch-evidence` 是 **quick 模式**，只证明四段结构、专用探针、runtime contract 和归档边界存在；输出必须带 `"mode": "quick"`，不得作为最终行为回归，也**不得在 Step 2 捕获现场之前要求 live overlay 与旧 canonical bundle 逐字节一致**——手工解冲突后的 overlay 必然暂时领先于 bundle，这条 parity 只在 Step 2/8c 和 full/final audit 执行。完整证据由 `python3 scripts/test_patch_evidence.py --report-json <path>` 生成：每个 active 工程 PATCH 的验证函数必须唯一解析为完整 pytest node ID，节点文件必须出现在该 PATCH 的 `文件` 清单，并在隔离环境中通过 JUnit 证明 outcome 为 passed；同一个 node 不得被多个 PATCH 共用，以免相邻补丁借绿。full 模式还用标准库调用轨迹记录每个 node 实际执行的仓库文件：凡声明拥有 Python 生产代码的 PATCH，证据节点必须逐个触达全部 owned production `.py`，不能只命中其中一个就整块放行；仅拥有测试文件的 hermetic/portability PATCH 才免除此项，非 Python surface 由其 runtime/gate contract 验收。同名歧义、跨 PATCH 复用、未登记测试文件、部分或完全未触达 owned implementation、未收集、skip、xfail、error 或 failure 均阻断。Archive 上游吸收项执行当前 pytest/CLI/行为探针；因需求退役项只证明旧 capability/resolver/config-key surface 未复活，names-only 检查不得读取 secret 值，也不得硬编码当前生产 provider/model。isolated bundle/mirror/trace 临时目录必须自动回收。
...
dry-run 必须把 `scripts/`、`hermes-update.sh`、插件 verifier 及 outer/inner `git status --ignored` 的全部条目分类为 `keep/remove/review`；`review`、policy error、required script 未跟踪、活跃 pytest/CDP 进程或 apply 失败均阻断 restart。apply 只移动 blacklist 项到 Trash，不能删除 review 或 tracked 文件。清理通过后记录 `hermes gateway status` 的当前 supervisor PID，执行 `hermes gateway restart`，按脚本 `gw_restart_wait_seconds()` 同款预算（新运行时的 `_get_restart_exit_wait_budget()` 优先，旧运行时回落 `restart_drain_timeout`，+30s）轮询到不同的新 supervisor PID，再在新 PID 下重跑全部 Step 8e verifier 和 `hermes gateway status`。新 launchd plist 可能直接监管 `hermes_cli.stderr_timestamp` wrapper；此时 status PID 是 wrapper，verifier 必须另用 `gateway.status.get_running_pid()` 取得真实 Gateway 子进程并把注册 trace 绑定后者。两层 PID 都必须健康。如果 barrier 之后又改了任何运行时输入，必须重新执行本步骤；完全只读的“已是最新”审计且能证明当前进程晚于最后一次运行态修改时可跳过。
...
- README 周键唯一与当前周摘要有效内容不超过 1500 字；`PATCHED_FILES` 数组/快照/注数一致；全部 active + Archive 定义精确四段；playbook 摩擦表列结构与 Wiki lint 正常
...
| 首轮 updater 已拉到含修复的新源码，却在自动重启时报 stale module / `line_input` ImportError       | 这是 updater 自举窗口：`hermes update` 进程在 pull 前已加载旧版 `update_cmd`，因此本次 pull 才引入的全量 `sys.modules` purge 修复不能倒过来修复当前旧进程。只要事务已固定 `TARGET_SHA` 且 checkout 已到该 SHA，禁止再次 `--update`；直接运行 `bash ~/.hermes/hermes-update.sh --reconcile`，由新进程加载新 updater、完成 patch 回贴和 planned restart。2026-08-20 从 `13ce0c5c` 升到 `f43eabee5` 首次实抓，目标提交 `044acf2bf7` 已包含类修复。若未来官方 updater 改为 exec 新解释器继续 post-pull 阶段，或旧进程也能在 pull 前预装通用 purge，自举窗口消失后删除本 row。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 新增 self-test 在 `--reconcile` 中污染请求模式，误开 acquisition                                  | self-test 只能在 subshell 中运行，不能把 `_REQUESTED_MODE`、`_ACQUIRE_UPSTREAM`、`HERMES_AGENT` 或事务字段留在主流程；`--self-test-patch-evidence` 当前对 transaction/fetch self-test 使用 `( ... )` 隔离。回归命令：先执行 `bash hermes-update.sh --self-test-patch-evidence`，再在无事务现场执行 `bash hermes-update.sh --reconcile`，日志必须为 `Starting local reconciliation ... (no fetch/pull)`，且不得创建 `refs/hermes-update/target`。2026-08-20 曾因 `_self_test_transaction` 把 `_REQUESTED_MODE=update` 留在父 shell，导致一次 `--reconcile` 错误固定 `c47f0b4590e`；现已修复并将该事件写入周摘要。若未来新增 self-test 修改全局 shell 状态，必须扩展 subshell 边界或显式状态快照/恢复。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| MCP stdio 正常启动却立即报 `subprocess ... has exited`                                            | 先核对 MCP stderr 的启动/初始化、agent fast-fail 时间和随后 idle recycle，不能把 1–2 秒错误当作工具 timeout 或服务崩溃。2026-08-27 12:08:24 HyperTeX 正常启动，12:08:25 被 Hermes 误判退出，12:09:26 同一连接又正常执行 60 秒 idle recycle；根因是 `_stdio_children_dead()` 在 `psutil.pid_exists(pid)` 为真时返回了 `True`，而正确的“至少一个 child 存活”分支成为不可达代码。处置：任一 PID 存活立即返回 `False`，遍历完全部死亡才返回 `True`；直接覆盖 alive/dead/mixed 语义，并让 live child 穿过真实 tool handler，证明不会 fast-fail。该规则与“watcher 只创建一次”共同归属 `PATCH-MCP-STDIO-WATCHER-LIFECYCLE`。上游修正 predicate 并提供同等级真实 handler 回归后删除本 row。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| HyperTeX MCP 已支持按用户权重选 Agent、Hermes 仍强制写入固定 Agent                                | 2026-08-19 契约复核：`hypertex_create_case` 省略 `agent` 时按 owner 的 Agentic weights 抽取，`hypertex_iterate_case` 省略时沿用 case 当前 Agent；显式值才覆盖。Hermes sandbox 若继续注入 `codex` 会绕过权重并让 iterate 静默换 Agent。处置：create/iterate 都主动删除模型传入的 `agent`，仅保留 `hermes` contributor、new case `deck` 类型和安全暂存附件；测试和 verifier 成对断言 create 无 agent、iterate 无 agent，最终 Gateway trace 必须含 `hypertex_agent_policy=weighted-create/sticky-iterate`。若未来 HyperTeX 移除权重/粘性契约，按其最新 MCP schema 重解，不恢复本地猜测默认值。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| PATCH evidence 缺少本补丁测试却被相邻 gate 的 `test_*` 借绿 | 旧 `scripts/test_patch_evidence.py` 在验证段没有测试 token 时，会从 PATCH ID 周围固定字符窗口寻找任意 `test_*`；相邻 gate 足以制造假阳性，且裸文件名 typo 也未验证。处置：工程 PATCH 的验证段必须显式写出权威 runner 中真实存在的测试路径/函数；专用 audit 以 PATCH→函数映射登记；Step 8b gate header 必须按 PATCH ID 唯一，并且每块只能激活一个未重复占用的 `*_PATCH_OK`，映射全集必须等于声明全集。用临时 PATCHES 变体做“删掉本补丁测试名”和“拼错测试名”两个负例。若未来注册表迁为机器可读 schema，可把这些关系迁入 schema 后删除本 row。 |
