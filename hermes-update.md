...
toolchain_audit_state:
  ...
  last_deep_audit_outer_commit: 25634d241deedc95f2d638cdeb46149ddfafdf82
...
final-audit 不是旁观者：它必须先取得与 update/reconcile 相同的事务锁，并在运行开始与结束分别记录 outer/inner HEAD、transaction 内容、bundle/base/lockfile-review 指纹和受管工作树摘要；任一字段漂移都判定本轮证据失效。这样可阻止审计期间另一进程刷新 bundle、推进事务或改写 lockfile 后仍拼出绿色 JSON。
