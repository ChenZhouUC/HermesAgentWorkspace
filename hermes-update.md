...
| Skills mirror 出现成批 `+N new` / `-N removed`（如 `+70` / `-94`）                | 2026-08-05 同日多轮曾删 94 个非上游文件、同 SHA 复跑又补回 70 个 bundled 文件（`my-skills/` 不受影响）；终态虽可由镜像正确收敛，但未知运行时过程的批量消失/恢复仍须可追踪。2026-08-06 复发的 `+70` 经路径清单定性为 `hermes-agent/skills` 下测试/运行时生成的 `__pycache__` / `.pyc`，已加入 Step 4b 排除并清理误镜像缓存。脚本成功路径现逐条留存 `>f+++` / `*deleting` 路径（`+ added:` / `- deleted:` 行）；下轮复发先读升级日志并关联同期 gateway/curator 记录，再判断是应修外部写入还是把合法 runtime 产物加入排除集合。镜像终态以排除 runtime state 与 bytecode cache 后的双向文件集合和内容与源树精确一致为准 |
