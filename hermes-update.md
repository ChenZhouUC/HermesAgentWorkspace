...
| `hermes doctor` 报 web / ui-tui build-tool 高危                                   | 先按 P2 定性：Web/UI/TUI/Desktop/browser tooling 的 build-chain advisory 不在飞书 gateway 主链路上，且现有修复需要上游 lock/range 或 peer 约束更新时，等待上游 bump，不阻塞；最终报告和 README row 只留案说明“不影响飞书主链路”。若 `npm explain` 证明某 high 进入 gateway runtime 或飞书消息处理路径，立即提升为 P0。                                                                                                                                                                                                                                                                           |
...
| `npm audit fix` 因 ui-tui ESLint peer 冲突报 `ERESOLVE`                           | 先用 `npm audit --json` 区分根运行时、Web/UI/Desktop 构建链与可破坏修复，并用 `npm explain <package>` 定位 critical/high 的真实依赖链。已知 ui-tui `eslint@9` 与 `eslint-plugin-react@7.22` 的 peer 冲突会让自动 fix 与手工重试同样失败；不要用 `--force` 倒退 Electron/ESLint 大版本。根 `npm install` 恢复 agent-browser 能力后，保留可解释的 bundle 外 lock drift；剩余只在开发/构建链或 Desktop/browser tooling 的 advisory 归为 P2，记录为待上游 lockfile/range bump，报告“不影响飞书主链路”。                                                                                              |
...
| Skills mirror 每轮报 `+0/~1/-0`（llm-wiki）                                       | 固定源码振荡，非漂移：Step 4b 在补丁还原态把镜像 llm-wiki 刷回上游版（~1），Step 8c 再回同步补丁版 + re-baseline。`PATCH-SKILLS-MIRROR-METADATA` 会从 `rsync --delete` 排除 `.bundled_manifest` / `.curator_state` / `.usage.json` / `.usage.json.lock` / `.curator_backups` / `.curator_suppressed` / `.hub` / `.archive`，不得再把 runtime state 删除数计入 mirror；rsync 非零必须使整轮失败                                                                                                                                                                                                   |
| Skills mirror 出现成批 `+N new` / `-N removed`（如 `+70` / `-94`）                | 2026-08-05 同日多轮曾删 94 个非上游文件、同 SHA 复跑又补回 70 个 bundled 文件（`my-skills/` 不受影响）；终态虽可由镜像正确收敛，但未知运行时过程的批量消失/恢复仍须可追踪。脚本成功路径现逐条留存 `>f+++` / `*deleting` 路径（`+ added:` / `- deleted:` 行）；下轮复发先读升级日志并关联同期 gateway/curator 记录，再判断是应修外部写入还是把合法 runtime 产物加入排除集合。镜像终态以排除 runtime state 后的双向文件集合和内容与源树精确一致为准                                                                                                                                                |
...
| adapter.py `FeishuAdapterSettings` 构造块 3-way 冲突（secrets scope）             | 已知预案（2026-08-03 审计，post-26e0b1c `f4b268b78`）：上游把 `FEISHU_APP_SECRET`/`FEISHU_ENCRYPT_KEY`/`FEISHU_VERIFICATION_TOKEN` 读取改走 `_get_scoped_secret`，改写点恰在本地 `PATCH-FEISHU-GROUP-ADMISSION`/`PATCH-FEISHU-RESOURCE-ACCESS` 扩展的同一 settings 结构体与 import 块，逐字冲突几乎必然但**语义正交**：解法是把上游 scoped-read 三行套进本地版 settings，本地新增的 `FEISHU_*` env 读取同步改用 `_get_scoped_secret`，不存在逻辑取舍。伴随 `dbbfcff56` 对 `authz_mixin.py` 同类改写，处理方式相同。                                                                              |
