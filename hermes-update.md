...
> **用法**：在支持本地工具的 AI 会话中说一句 `阅读 ~/.hermes/hermes-update.md 按计划做`，agent 即按本 playbook 一条龙完成"升级内层官方源码 checkout + 回贴本地 patch + 依赖自愈 + 补丁回归修到全绿 + 对齐外层监管记录/文档 + playbook 自身增补/清理/修订自审"，中途不打断、不追问，**收尾零遗留**。
>
...
- `~/.hermes`：用户的 Hermes 配置备份仓库。它监管 `hermes-update.sh`、`patches/local-patches.diff`、`patches/PATCHES.md`、`config.yaml`、`plugins/`、`my-skills/`、README/wiki 等升级记录；**升级完成后只在这个外层仓库提交**。其中 `PATCH-CLAUDE-SC-PROVIDER`、`PATCH-FEISHU-GROUP-SANDBOX` 这类用户插件补丁不进入内层 unified diff，而由外层 Git + Step 8e 强制 verifier 监管。
...
- **外层插件层**：`PATCH-CLAUDE-SC-PROVIDER`、`PATCH-FEISHU-GROUP-SANDBOX` 等配置仓库补丁由外层 Git 和独立 `verify.sh` 监管，不得进入内层 `PATCHED_FILES` 或 replay bundle。
...
- 从 `hermes-agent/hermes_cli/__init__.py` 读取 `__version__` / `__release_date__` → 仅作安装展示摘要。不要在事务快照里调用 `hermes --version`，其 update-check 可能发起额外网络版本检查；版本摘要只读取本轮固定 checkout
- `hermes doctor` 头部摘要
...
- `uv pip check --python venv/bin/python` 的声明冲突须先对照 `pyproject.toml` / `uv.lock` 中的显式 override。上游为安全修复有意越过旧 SDK metadata cap 时，按既有 pin 与对应能力回归验收并登记原因；不得为了让 metadata 检查转绿而降级安全依赖。
...
- 依赖恢复完成后重新运行 Step 8e 登记的全部 verifier（当前包括 `plugins/model-providers/claude-sc/verify.sh` 与 `plugins/sandbox/verify.sh`）。这一步不能省：SC verifier 必须重新证明 provider API、配置、凭据、Claude Code 请求身份、reasoning/cache/tool/image 契约及 Gateway freshness；sandbox verifier 必须重新证明 YAML、toolset、行为测试和 runtime trace 全绿。运行态证据必须绑定 `hermes gateway status` 返回的**当前 PID**，不能用任意一条历史记录代替。
- 处置结果写进升级摘要与摩擦表。
...
测试文件清单与上轮外层提交中的 `hermes-update.sh` 对比，passed 数与 `PATCHES.md` § 当前版本摘要对比。禁止直接调用 `pytest`；runner 会隔离 HOME/凭据、固定时区/locale，并逐测试文件放进独立子进程，结果才与 Hermes CI 口径一致。PATCH 终态显式使用 `--file-retries 0`，并把 unhandled-thread、unraisable、`RuntimeWarning`、test-return-value 与 collection warning 五类执行完整性信号提升为错误：普通开发套件可以把 pass-on-retry 标成 FLAKY 供定位，但升级验收不得让首轮失败被自动重跑洗绿，也不得把测试线程、对象终结器、未 await coroutine、返回值伪断言或部分未收集测试留作 warning 后仍计为 passed。unittest 型探针另要求真实 stderr 运行计数非零、终态为无 skip/expected-failure 附加状态的 clean OK；不能把 exit 0 或 stdout 中的伪计数当作全量执行。**回归终态 0 failed 是必要条件，不是充分条件**：passed 数只是遥测，数量下降、测试文件消失、collection 异常或关键用例被改名/跳过都必须逐项解释，不能拿“仍然 0 failed”掩盖覆盖面退化。出现新失败先定性、再按分支当轮修到转绿，不留遗留：
...
| **外层用户插件**  | `PATCH-CLAUDE-SC-PROVIDER`、`PATCH-FEISHU-GROUP-SANDBOX` 检查外层 diff + 各自 `verify.sh`；`config.yaml` / `plugins/` / `my-skills/` 不得伪造进内层 replay bundle。verifier 必须验证各插件依赖的上游 API、配置和运行态 freshness；sandbox 另核对真实 Gateway 子进程注册日志。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **画像隐私边界**  | `people.yaml` 必须被 Git 忽略；`people.yaml` / `groups.yaml` 都保持 owner 可读写的普通文件并由热加载器与 Step 8b 收敛为 `0600`，不得用只读位或 immutable 破坏同账号 VSCode 手工编辑。人物字段采用公开白名单，只有 `name` / `role` / `department` / `address` 可输出，任何未来新增字段必须默认“模型可读、输出禁止”，技术 ID 与未公开别名只用于匹配。`address` 为空时的默认称呼必须仅从公开 `name` 确定性派生，不得从 aliases、括号昵称或历史模仿；中文单字名与低置信布局保留全名，prompt 只控制是否自然使用代码已确定的候选。`PATCH-LOCAL-PROFILES` 的行为回归还必须覆盖当前问话人匹配、历史/合并转发发送者仅渲染公开白名单，以及 final、stream、fallback、interim、streaming TTS、`/background` 等独立可见/可听文本路径的脱敏。只检查普通 final reply 不足以证明“只能使用、不能外显”。 |
...
- 上游把类/方法迁入新模块或补齐测试 fixture 时，必须把本地调用点、owned files 与行为 evidence 一起迁移，并核对同名定义是否被 3-way 重复保留。不能在旧 facade 复活已迁走的 class，也不能让旧 no-op stub 覆盖上游新增的真实计数/断言；clean apply 不免除此项。
- 注册工具的 wrapper、缓存提前返回与协议专用方法都是独立执行入口：验证必须穿过真实 registry dispatch，不能用底层 helper/list 的通过替代。缓存命中先复核当前会话策略、启用状态和实际来源，再决定是否复用；会话 capability namespace 先于进程默认平台。协议 mutation 继承 operator trust/审批规则，不能因未走普通 tools/call 就略过安全地板。
- 配置驱动 PATCH 必须成对验证显式配置、缺省路径与错误形态；现有生产配置恰好显式设置某值，不能掩盖文档承诺的 fallback 未实现。权限地板必须覆盖所有提前返回（含隔离后端），并保留 owner/普通会话反例。“群聊不得审批/发卡”还必须覆盖公共 action gate、纯扫描告警与真实 Gateway 通知出口，不能仅用 terminal/execute_code 拦截代表所有入口；纯扫描 warn/block 须分别在 yolo/off、allowlist、session/permanent 批准与隔离后端下验证拒绝，不能把“没有发卡”当成“没有执行”。工具审批与原生 slash confirmation 是两套入口，必须分别覆盖，关闭确认的提前执行分支也要保持群权限地板。卡片与纯文字 fallback 都须拒绝，队列不得残留，DM 与无告警命令的正例继续成立。
- 群内管理员新建/分支 session 与 Gateway 重启是不同权限：前者按真实命令注册表覆盖全部别名，后者同时要求 owner 发送者与配置中的主 DM；能够改变主会话位置或间接重启的管理入口也必须受限。不能用关闭工具审批替代 slash 分派权限；Base adapter 的取消/排队动作与 runner 的空闲/忙碌分派都要验证，拒绝必须发生在中断现有任务之前。管理员群内明确发起的新会话命令不应另发审批卡，主 DM 原生确认策略保持。管理员身份以 `feishu.assistant_user_ids` 首项为权威，主 DM 的 home 配置与 sandbox owner DM 必须一致，严禁按显示名识别。
...
摘要计数的可解析句式以 `scripts/final_upgrade_audit.py` 的派生检查为准；重写摘要时先核对其 parser，不能用临时“预期”等标签替换机器消费的终态字段。
...
- 归属判定与理由写入对应 PATCH 的四段定义中：并入时说明继承哪个不变量，新增时说明与相邻 PATCH 的失效、验证或吸收条件有何区别。多个 PATCH 共享文件时，按函数或 hunk 说明边界，并保持 evidence node 独立；不能仅凭文件相同合并生命周期，也不能只在会话总结里解释拆分。
...
- 配置仓库用户插件补丁（当前 `PATCH-CLAUDE-SC-PROVIDER`、`PATCH-FEISHU-GROUP-SANDBOX`）不进入 `PATCHED_FILES` / `local-patches.diff`。它必须同时具备：外层 Git 跟踪的插件/配置文件；独立 `verify.sh`；`hermes-update.sh` Step 8e 固定登记；verifier 缺失、不可执行或失败时 `FINAL_RC=1`；`PATCHES.md` 对应块。provider verifier 至少要离线穿过 profile 注册、runtime resolver、请求构造与认证 header，并核对当前 Gateway 晚于运行态输入；安全插件 verifier 还需结构化解析 YAML、真实 toolset、行为测试和运行时注册。对渐进式工具披露必须穿过真实 `handle_function_call(tool_call → underlying)`，证明 scope gate、底层 hook 和 handler 全部执行。
...
#    verifier 的整体 JUnit 回执不能替代文档中的插件分项条数，所以这两处"活契约"数字
#    仍需独立绑定实际收集数（2026-08-20 实抓：修 HyperTeX 附件名后回归 42 → 52，两处仍写 42，
#    任何机械 gate 都没报）。这里把文档数字绑回真实收集数，纳入 Step 5 数字漂移防线。
plugin_pat_n = int(re.search(r"插件(?:现有)? \*\*(\d+) 条\*\*回归", patches).group(1))
...
清理器本身属于本步骤的共演进资产：每轮都要审查 `scripts/cleanup_policy.json` 的 required script 白名单、临时脚本/ignored 黑名单和 review 输出。新增、重命名或退役任何 outer 运维脚本、插件 verifier、ignored runtime 路径时，必须同步 policy、`test_cleanup_transient_artifacts.py`、`PATCH-GATEWAY-RESTART-CLEANUP` 与本 playbook；禁止为了让 restart 通过而把未知路径机械加入 keep，也禁止把并发 session 产物直接加入 remove。对首次使用才创建的运行态根，先读生产构造器确认来源与生命周期，再同步 `.gitignore` 与 keep 理由；上游退役模块后继续保留其用户数据与恢复证据（如 Collective Wisdom V1 留下的 `wisdom/`），同时退役仅服务已移除模块的缓存规则，不能把功能退役当成删除私有状态的授权；回归应使用仓库真实 ignore 规则，分别证明私有状态不入 Git、不会被清理，不能仅靠本次已有 ignored 清单推断完整性。`--dry-run --json --fail-on-review` 为机械下界，语义理由仍须写入 policy 的 reason 字段。
...
3. **修订（规则时效核查）**：抽查本 playbook 引用的锚点、命令、路径、默认值与当前脚本/上游是否仍一致（如 `--print-*` 只读入口、gate 名称、doctor 输出口径、wiki SCHEMA 路径映射）；描述已不准确的段落**就地重写**，不另开平行段落、不保留旧措辞注释。旧 PATCH 承诺与现实现不符时，先核对历史并构造行为反例，区分过期实现描述与丢失的不变量；不得通过删改承诺把漏修变成“文档清理”。失效即当轮修订，不留给下一轮撞上。
...
2. **真实执行与归属**：检查 node path/class/function 精确绑定、逐 PATCH 进程隔离、每个 runnable 文件真实 collect、support module 有消费者、Archive/dedicated probe 拒绝 skip/xfail/xpass/零执行，且同一结果不能跨 PATCH 复用。非 pytest 探针也必须穿过受管生产入口：镜像执行真实 Step 4b，原生扩展执行当前 build.sh 并加载新产物及已有安装产物；依赖 pin 精确绑定对应 extra/feature，不能只在文件全局搜到字符串；迁移同时执行隔离 dry-run 与真实 execute，分别核对目标环境和应保留的恢复归档。不能用探针里另一份 rsync/gcc 参数或只读报告替生产实现证明。故障注入必须改生产输入或真实脚本，并证明现有探针会拒绝。
...
6. **复杂度约束**：耗时回归使用 monotonic 度量持续时间，wall-clock 差值只作诊断；遇到超时失败先区分计时、环境与实际行为，不以放宽上限或 retry 后转绿收尾。亚秒级墙钟回归若被宿主调度误伤，改用局部受控单调时钟和同步器穿过真实生产 guard，分别验证 deadline 与资源释放；Timer 回调的真实线程身份仍须保留，禁止冻结 pytest/无关线程的全局时间。新增 gate 必须对应一个具体失效模型，具备确定输入、唯一失败信号、负向回归、可接受耗时和退场条件；能加强或替换已有检查时不平行叠加。不得仅因“还能想到更多检查”延长审计。
...
toolchain_audit_state:
  schema_version: 1
  last_deep_audit_date: 2026-09-12
  last_deep_audit_upstream_sha: 28bf67b17607af53c147431f117d07955dc870da
  last_deep_audit_outer_commit: 8203370a873c55d7b62e9f11076d979017a94b61
  trigger: explicit-full-patch-polling-registered-entry-and-dedicated-probe-audit
```
...
- `hermes doctor` 的无 active security advisory、config up-to-date、无 deprecated key，且不得残留“previous update did not restart gateways”告警；历史失败 receipt 只有在当前 live fleet 非空、严格 PID inventory 与 fleet 回执集合一致、每个实例均为 `state=current` 且 `code_sha == HEAD` 时才可被判为已由后续收敛覆盖，显式 pending marker、空/异常/不完整/unknown/stale/down 探针仍 fail closed。canonical 后再次运行 sandbox verifier；launchd supervisor PID、真实 Gateway child PID 与 `gateway_state.json` 的 PID/argv/code SHA 必须一致，spawn ledger 不得残留 pytest 记录；transaction=`none`
...
| 切换模型后图片/音频/视频变慢、重复分析或只剩路径提示                                              | 先区分官方能力、当前 provider wire 与本机 capability metadata；私有 route 通常需要在 `providers.<name>.models.<model>.supports_vision` 声明经验证的图片能力，不能从 provider 名或旧型号推断。按 Step 2c 对改变的 route 跑无敏感合成 canary；支持的走 native，不支持的由 `PATCH-MULTIMODAL-SIDECAR` 仅发送当前媒体 + 有界上下文给配置链上 capable route，STT/可信抽取成功则不旁路。上游提供统一、可探测且带真实 wire 回归的 modality routing 后删除本 row。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| live Gateway 已匹配当前 HEAD，但所有 CLI 仍提示 previous update 未重启                            | 历史失败 `update_receipts/latest.json` 的 pre-update plan 可能长期保留旧 SHA；若显式 `fleet_restart_pending` marker 已不存在，且当前 live fleet 非空、严格 PID inventory 与 fleet 回执集合一致、每个实例均为 `state=current` 且 `code_sha == HEAD`，应由 `PATCH-UPDATE-FLEET-RECEIPT-FRESHNESS` 让当前运行态事实覆盖旧 receipt，避免重复 warning/restart。空矩阵、探针异常、PID 集合不全、unknown/stale/down 或任一 SHA 不符继续 fail closed；final-audit 也必须直接拒绝该 Doctor 告警，不能只靠独立 `gateway_state.json` 检查拼绿。若 upstream 原生采用当前完整 fleet 作为 pending-restart 终态权威，可归档补丁并删除本 row。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 群消息只有 `@配置本人账号`，AI 味浓的正文位于直接引用中，分类却为 0                               | 不能把 direct quote 永远降级为“不评分的上下文”。若当前正文在匿名化 mention 后只剩空白/标点，则本次 @动作实际是在把引用正文主动投递给 configured human；`PATCH-FEISHU-GROUP-ADMISSION` 必须把有界 quote 提升为 classifier 的 `current_message`，同时继续禁止读取群历史、画像或推断 quote 作者。回归必须穿过真实 Feishu payload → mention trigger → quote fetch → classifier payload → refusal，并保留“当前消息有正文时 quote 仍只作上下文”的反例。2026-09-05 Data Pipeline Workshop 现场旧布局为 `0.0`，同一引用提升后只读 replay 为 `0.9`。若上游提供等价的 mention-targeted content provenance 与 authorship gate，可删除本 row。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| restart 前 cleanup policy 出现 review / required script 未跟踪                                    | `PATCH-GATEWAY-RESTART-CLEANUP` 故意 fail closed：先查看 `scripts/cleanup_transient_artifacts.py --dry-run --json` 的完整 `script_audit` / `ignored_audit`，确认新路径是必须保留、确定可清还是并发/未知产物；插件 verifier 要递归覆盖 `plugins/**/verify.sh`，包括 model-provider 嵌套目录；把理由写入 `cleanup_policy.json` 并补测试后再 reconcile。未完成事务才出现的 `.hermes-update-transaction` 是固定 `TARGET_SHA` 的持久恢复状态，必须和 `.hermes-update-transaction.lock/` 一样显式 keep；`.skills_prompt_snapshot.json` 是 manifest 校验的 0600 冷启动 prompt 元数据，也必须 keep；`.clean_shutdown` 是 Gateway 排空成功后写、下次启动消费的 graceful-exit receipt，必须 keep，删除或让其卡在 review 都会破坏 crash/restart 判定。规范 runner 预编译新出现的 `hermes-agent/website/**/__pycache__/` 属文档构建缓存，应精确列为 remove，不能把整个 website 树机械放行。禁止直接删 review、用宽泛 glob 把未知项全标 remove，或为通过 gate 把整个目录标 keep。该 row 在 cleanup policy 仍由本地外层脚本维护时长期有效；未来若上游提供等价三态治理与 restart gate，可随 PATCH 一并删除。                                                                                                                                                                                                                  |
...
| 用户插件 verifier 缺失或失败                                                                      | `PATCH-CLAUDE-SC-PROVIDER` / `PATCH-FEISHU-GROUP-SANDBOX` 类外层插件不得继续显示升级成功。恢复 `plugins/<name>/verify.sh` 的文件/执行位，修复根配置、插件配置或上游 API/hook/toolset 兼容性，直到 Step 8e 对照当前 Gateway PID 返回 0；不要把外层文件加入内层 `PATCHED_FILES`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 截断重试日志显示提高 output cap，但当前 transport 的 wire 仍用旧值                                | `_ephemeral_max_output_tokens` 是一次性跨重试状态，新增/重构 API mode 时必须逐分支确认它被请求构造消费并立即清空；不能只测 conversation-loop 设置了字段。当前 SC Claude fallback 走 Chat Completions facade，复用通用消费路径；Codex Responses 仍有独立 native 分支。处置：Step 8b 运行 `test_claude_sc_consumes_ephemeral_output_cap` 与 `test_codex_responses_consumes_ephemeral_output_cap`。当上游统一 output-cap 组装为单一、覆盖全部 transport 的入口后，可收缩本 row 与本地接线。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Feishu 引用回复、WS/profile 或 YAML bridge 重构与本地语义冲突                                     | 3-way 触及 adapter 时同时保留上游 profile ContextVar、本地 reconnect backfill hook、出站普通引用 `reply_in_thread=False` 与入站 session 边界。YAML bridge 使用上游 scope-aware `yaml_env_setter()`，并合并本地 admission/history/attachment 字段；不能保留缺少 `allow_bots` 就提前返回的分支，也不能直接写进程环境污染其他 profile。旧 env-only 测试须改验显式环境优先、profile extra 回退与真实 `load_gateway_config()` → adapter 接线，成对覆盖有/无 allow_bots 和默认/独立 profile。引用/回填与配置接线全部被上游等价吸收后可删除本 row。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Gateway PID/code SHA 正确，但 start_time 指纹相差整秒 | 2026-09-13 在 macOS / psutil 7.2.2 实抓长驻 Gateway 写入 `178923730624`，新探针读取 `178923730424`。`adjust_proc_create_time()` 依赖进程内 `INIT_BOOT_TIME` 与当前 boot_time，现象与跨进程时钟校正基准差异吻合。先核对 PID、argv、code SHA 与 active_agents；禁止手改运行态身份文件或放宽指纹比较。完成 cleanup dry-run/apply 后用 `hermes gateway restart` 优雅排空重启，让新进程写身份；验证不同的新 PID、状态文件与 live start_time 完全一致，再重跑完整 final-audit。若持续复现，需将同机身份迁为稳定时钟依据并按独立语义 PATCH 审查，不靠重复重启掩盖。上游提供等价稳定指纹并验证长驻/新进程一致后可删除本 row。 |
...
| sandbox verifier 的多能力 tool_search 查询为空，但对应工具仍可 describe/call | 新搜索器按单项能力做相关性判定，混合“cache + 文档 + 图片 + 图表”的 query 没有工具能完整回答。每项能力独立查询，并解析 JSON 的对应 `results[].matches`；不得仅在字符串或 available_sources 提示中找到工具名就判通过。继续执行真实 deferred bridge、scope/hook 和 handler 正反例。若上游提供稳定的 capability discovery 验证 API，迁移后删除本 row。 |
| uv pip check 报 SDK cryptography 上界冲突 | 先检查上游 `pyproject.toml` 的安全 pin 与 `override-dependencies`。当前 Alibaba SDK metadata 的 `<49` 与上游明确要求的 `cryptography>=50,<51` 冲突属于有意 override，不是整包丢失；保持锁定环境，报告实际受影响的可选 SDK 路径，不为消除提示降级。SDK 发布兼容 metadata 且上游删除 override 后删除本 row。 |

| CLI 启动先提示 env 引用未设置，Doctor/正式配置却正常 | 不得只凭 pre-dotenv 警告推断缺凭据。用 names-only/布尔检查确认 profile dotenv 是否声明非空值，再经正式 `load_hermes_dotenv` 与 `load_config` 检查对应 header 是否仍有占位符；不得打印 token 或 header 实值。加载后已解析属于启动时序提示，加载后仍未解析才按实际受影响能力分级；不能为消除早期警告把 secret 写入 tracked YAML。上游统一先加载凭据再解析引用后删除本 row。 |

| 镜像或原生构建实际失效，独立探针却仍通过 | 探针不得另写一份“正确参数”绕过受管实现。Skills 在临时 HERMES_HOME/HERMES_AGENT 执行真实 Step 4b，检查 runtime sidecars、删除、更新与 bytecode 排除；CJK 在临时副本运行真实 build.sh，再独立加载新产物和已安装产物并执行索引查询。删保护参数、坏构建脚本和损坏安装产物必须使原探针失败。上游提供等价的生产入口集成测试后可迁移并删除本 row。 |
| 群审批在免检分支提前放行，或 Skills 缺省与错误配置被显式生产配置掩盖 | 会话授权先于容器免审批；command、legacy、execute_code 三入口都要覆盖 group 拒绝与 owner DM 保持。纯扫描 warn/block 另测 yolo/off、allowlist、历史批准和隔离后端；最终通知出口无卡不能代替执行入口拒绝。Skill 创建显式 create_dir 优先，缺省取首个非官方 external root（目录可尚不存在）；只在显式 create_dir 下测试不能证明缺省路径。malformed policy 的冷读和热缓存均须拒绝。上游提供等价 scope floor 与配置 schema 验证后，随对应 PATCH 退役删除本 row。 |

| 定向 runner 收到不存在的测试路径却仍对剩余文件返回成功 | 当前 canonical runner 可能忽略不存在的显式文件参数。定向验证前先用 `rg --files` 发现真实路径、逐项确认存在，再核对 Summary 的实际文件数与目标集合；不能把部分执行的 exit 0 当成全部通过。终态仍由 full evidence 的逐文件 existence/collect 与 canonical 数量闭合验收。上游 runner 原生拒绝所有不存在的显式目标且有负向回归后删除本 row。 |
| 完整 PATCH 审计通过，但注册 wrapper 或协议专用入口绕过策略 | 文件调用轨迹只证明某段生产代码被执行，不代表同文件全部入口完整。Skills 缓存命中须在当前来源、allow/disabled、bundled/plugin 可见性之后；MCP Tasks update/cancel 在任何连接/lazy spawn 前继承 untrusted 审批，不能借同名 readOnlyHint 豁免。把 registry.dispatch 的撤权、来源切换、会话/进程平台对撞与 mutation 拒绝/许可反例绑入对应 PATCH 的 full evidence 和既有 gate。上游将全部入口统一到同一不可绕过的策略层并有等价回归后可删除本 row。 |
| 依赖或迁移专用 probe 只搜字符串/检查 dry-run 就置绿 | 依赖按 TOML extra 与实际 lazy tuple 分别核对，再检查安装版本；把 pin 移到无关 feature 必须失败。迁移使用临时 source/target/output 跑真实 execute，目标 .env 不启用废弃 key 且保留已有值，恢复 archive 则须完整保留源配置；只生成干净报告、实际写错 env 或丢 archive 都必须失败。探针返回这些分项的执行回执；上游提供等价生产边界验收后可迁移并删除本 row。 |
