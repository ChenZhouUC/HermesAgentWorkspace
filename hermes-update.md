...
2. **真实执行与归属**：检查 node path/class/function 精确绑定、逐 PATCH 进程隔离、每个 runnable 文件真实 collect、support module 有消费者、Archive/dedicated probe 拒绝 skip/xfail/xpass/零执行，且同一结果不能跨 PATCH 复用。非 pytest 探针也必须穿过受管生产入口：镜像执行真实 Step 4b，原生扩展执行当前 build.sh 并加载新产物及已有安装产物；不能用探针里另一份 rsync/gcc 参数替生产实现证明。故障注入必须改生产输入或真实脚本，并证明现有探针会拒绝。
...
6. **复杂度约束**：耗时回归使用 monotonic 度量持续时间，wall-clock 差值只作诊断；遇到超时失败先区分计时、环境与实际行为，不以放宽上限或 retry 后转绿收尾。新增 gate 必须对应一个具体失效模型，具备确定输入、唯一失败信号、负向回归、可接受耗时和退场条件；能加强或替换已有检查时不平行叠加。不得仅因“还能想到更多检查”延长审计。
...
toolchain_audit_state:
  ...
  last_deep_audit_upstream_sha: 3b45681c25a880477a2a806cdebe91d2f1bfe9ce
  last_deep_audit_outer_commit: 5be8f29e474fd6c2b44d9f44edecdea362122246
  trigger: post-green-feishu-admin-control-and-slash-confirm-audit
```
...
| Feishu 引用回复或 WS/profile 重构与本地语义冲突                                                   | 3-way 触及 adapter 时同时保留上游 profile ContextVar 传播、本地 reconnect backfill hook、出站普通引用 `reply_in_thread=False` 与入站不把 `root_id` 当 thread 的边界；上下游代码只因插入同位冲突时应并存。不得把旧“尚未跨过某 commit”的预案继续当现状。以真实 adapter/Gateway 回归验收；上游原生区分引用链与话题且提供等价回填后，可归档相关 PATCH 并删除本 row。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| upstream 新增 install 级运行态文件后外层出现 untracked | 先从创建/消费代码判定生命周期：`spawn-ledger.json*` 是进程身份与隔离证据，`shared-state.db*` 是 hosted-room 协调数据库及 SQLite journals，`fleet_restart_pending` 是官方 updater 尚未消除的重启义务。三者应精确 ignore + cleanup keep，并用临时仓库回归；不得清空数据库或删除 pending marker 来制造绿色。`hermes_wisdom/**/__pycache__` 只保留已安装模块的热缓存，不等于启用对外分享。上游迁入既有受管 runtime 目录或停止使用后，同步退役对应规则与本 row。 |
