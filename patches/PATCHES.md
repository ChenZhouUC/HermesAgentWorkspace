...
> 以上为 `hermes-update.sh` 中数组的快照（64 文件，2026-08-10 与脚本核对一致）。**脚本数组是唯一权威来源**；增删补丁文件后请同步刷新本快照。机器读取请用 `bash ~/.hermes/hermes-update.sh --print-patched-files`，不要解析本快照。
...
## 当前版本：v0.20.0 (upstream `main` `03fa32c92dd445eb64c7f67434dd91b32c40701d`，2026-08-10)
...
**最近一次升级（v0.20.0 → v0.20.0，+409 commits，basis `cd9fbf9f19d937024bde2fce16b07f5400099723` → `03fa32c92dd445eb64c7f67434dd91b32c40701d`，2026-08-10）要点**：

- 上游主线：安全/依赖侧一次性修补 31 个已知 CVE 并将 aiohttp pin 推至 3.14.3（`7537de9e7` / `45f31de4e` / `a1e4fee33`，含 h2 CVE-2026-71554 的 exclude-newer 临时例外）；Windows 编码大扫除——auth/.env/cron/tools 全链 UTF-8 / BOM / lone-surrogate 加固与 Telegram UTF-16 entity offsets（`762f1c588` / `566b5b16a` / `022d196f3` 等成批 salvaged PR）；personality 状态单一所有权 + v34 一次性重置迁移（`244d29664` / `a0d406dcd`）；`platform_toolsets` 无效 toolset 名 loud 校验（`hermes_cli/toolset_validation.py`，#38798 动机）；Gateway 侧 session identity 跨崩溃持久化（`d2a4d373e`）、恢复尊重 reset 边界（`784f733cf` / `6e8dcb8f4`）、queued media 投递连续性簇（`808c8570a`…`0b17b691d`）、finalized stream sends（`70d165222`）、/branch 子会话元数据补全；State/FTS5 sanitizer 与 corrupt fallback（`90311ee75` / `c595dcb95` / `1527a81b5`）；agent 侧 compression prune（`adf9549cd` / `e00965a7e`）、gpt-5.6 原生 server-side compaction（`5e1b50115`）、skill-turn stable/volatile 缓存边界（`214f2b82d`）；read_file ipynb 输出渲染 + jq hint（`a607b7628` / `ad2c7af86`）、PDF gap 标注（`7c02bfce8`）、special-file guard 与 unicode 文件名重试（`0e63ed1fe` / `fd452e26e`）；skills 新增 4 个 bundled（competitor-news-monitor / meeting-action-items / product-price-monitor / weekly-review-planning）、office 套件 clean-room MIT 替换（`51570f4da`）、SKILL.md 标准 CI 硬线（`55982159d`）、bundle 路径 NTFS ADS 冒号拦截（`6fa646e7d`）；Desktop HUD 模式全套与 Electron 回滚 40.10.2（`bb8280b75`）；Docker per-session 容器隔离与附件投递（`e95e13783` / `fe54ab4f9`）。
- patch apply / registry：64 个受管文件 3-way 自动融合一次通过，无需人工解冲突；28 个 Step 8b sentinel 全 OK。30 个活跃 PATCH 逐个复核上游吸收判断：0 新增 / 0 归档 / 0 收缩——重点核验 `PATCH-SKILL-CREATE-ROOT` 的 `_background_review_write_guard` 作用范围未扩大到前台 create（本轮 skills 工具改动仅 BOM 读 + create 建议性 linter）、`PATCH-FEISHU-SOCKS-DEPENDENCY` 的 feishu extra 上游仍未声明 SOCKS、`PATCH-DOCUMENT-EXTRACTION` 与上游 ipynb/PDF-gap 增量正交（EXTRACTABLE_EXTENSIONS 未动，firecrawl-anydoc 仍为可选转换、不构成吸收）、`PATCH-VERTEX-FALLBACK` 之 CredentialPool 仍无 per-entry SA 文件/独立 project 语义（`bf7c71664` 仅 rebind pool entry id）。bundle/base/full-index、cached 正向、worktree 反向和 index-clean 闭环全绿。
- 依赖：venv 未重建，仅 aiohttp 3.14.1 → 3.14.3（上游 CVE pin 回锁）；lazy features 19 项全部 current；pyproject 新增 linux_only/macos_only/windows_only OS 测试标记与 dev 工具链 pin 前移（pytest 9.1.1，本机 venv 仍 9.0.2 可用、未丢包不回装）。root `npm audit fix` 后残余 7 vulnerabilities（electron 2 high 修复越 stated range + node-tar 链 1 critical 4 high no-fix，全部位于 Desktop/get-windows/build 链）→ P2 上游 lock/range 阻挡、不影响飞书主链路，禁止 `--force`；Browser/Web/UI-TUI 三个 workspace 的 advisory 经上游 CVE 修补本轮清零（doctor 全 ✓）。package-lock 仅归一化噪音（无版本升降），保留 bundle 外。Skills：官方 sync +4 new / 26 updated；终态 reconcile mirror `+0/~1/-7`——llm-wiki 固定振荡如常，`-7` 为上游 office 套件 clean-room 替换（`51570f4da`）后 mirror 移除的 docx/office 陈旧脚本目录（2026-08-11 Step 5c 自审纠错：此前误记为"无振荡、精确一致"）。
- 已知摩擦：官方 updater 正常消费固定 SHA `03fa32c92`，无 GitHub fetch retry、无遗留 autostash。完整回归 26 files **868 passed / 0 failed / 3 skipped**：+21 全部来自上游 `test_read_extract.py`（40→54 函数）与 `test_run_progress_topics.py`（18→25）扩展，无用例丢失/改名；3 skipped 为上游新增 firecrawl-anydoc real-binding 测试类按设计跳过（本机未装该可选包；安装属全新依赖，留用户决策，P3）。测试/受管文件清单与上轮提交版脚本逐字节一致。
- 配置漂移：config v33 → v34 迁移完成（`hermes doctor --fix`）——一次性 personality 重置将本机 `display.personality: kawaii`（2026-04-15 初始提交以来未变的陈年状态）重置为 none，属上游防"已关闭人格复活"的有意设计，如需恢复执行 `/personality kawaii`；迁移伴随 YAML 引号/列表风格重排（语义无变化）。`--fix` 路径新校验暴露 `platform_toolsets.feishu_group` 两条警告：`vision_tools` 为自初始提交起的无效死条目（注册名实为 `vision`，解析恒为空；沙箱层 group_allowed 本就不含 vision_analyze），已按两层一致原则从 `config.yaml` 与 `plugins/sandbox/verify.sh` 契约删除，运行时行为零变化、verifier 21 passed 复绿；`sandbox_group` 为插件运行时注册名，doctor CLI 未加载网关插件属预期误报，普通 `hermes doctor` 不出现。其余 doctor 残留均为既有 P3（python-telegram-bot 可选未装、Nous/Codex/MiniMax/xAI 未登录、Skills Hub 未初始化、bfl/computer_use 等系统依赖未配）。Gateway PID 链：1414 →（官方 updater 重启）62537 →（Step 8d 排空重启）64390，sandbox verifier 绑定 64390 通过；config.yaml 后续变更经 Step 5b 终态屏障重启并在新 PID 下复验（见最终报告）。
