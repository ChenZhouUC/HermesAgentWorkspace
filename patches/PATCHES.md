...
Step 8e: User-plugin verification
  ├─ verifier 文件必须存在且可执行
  ├─ PATCH-FEISHU-GROUP-SANDBOX: YAML 契约 + owner/group 真实 toolset + 54 个行为测试 + launchd wrapper / 真实 Gateway 子进程双层 runtime trace
...
> 以上为 `hermes-update.sh` 中数组的快照（90 文件，2026-08-27 与脚本核对一致）。**脚本数组是唯一权威来源**；增删补丁文件后请同步刷新本快照。机器读取请用 `bash ~/.hermes/hermes-update.sh --print-patched-files`，不要解析本快照。
...
## 当前版本：v0.20.6 (upstream `main` `8966b0a70029cb226e35c49f91d0c2208ab1d8c4`，2026-08-27)
...
**最近一次升级（v0.20.6，`9aa7530f7b` → `8966b0a700`，+282 commits，2026-08-27）要点**：

- 上游主线：唯一 acquisition 固定 `TARGET_SHA=8966b0a700`，282 commits 覆盖 MCP stdio liveness/进程 ledger、Gateway control-socket 排空与 fleet restart 恢复、WeCom 原生流式、Desktop Bot/SSH 更新、Browser real-profile 安全、SQLite 配置、Skills schema 精简、approval timeout 和模型目录更新；取得 SHA 后所有恢复均为 no-network `--reconcile`，remote 与目标未移动。
- patch apply / registry：逐项轮询升级前 43 个 active + 9 个 Archive PATCH；24 个活跃 PATCH 与 17 个受管路径重叠：`agent/conversation_loop.py`、`agent/tool_executor.py`、`gateway/display_config.py`、`gateway/platforms/base.py`、`gateway/run.py`、`gateway/session.py`、`gateway/slash_commands.py`、`gateway/stream_consumer.py`、`hermes_cli/config_defaults.py`、`plugins/platforms/feishu/adapter.py`、`pyproject.toml`、`tests/gateway/test_display_config.py`、`tools/approval.py`、`tools/mcp_tool.py`、`tools/skill_manager_tool.py`、`uv.lock`、`website/docs/user-guide/configuration.md`。Archive 回归面另有 6 个 PATCH 与本轮 upstream 相交，覆盖 `agent/conversation_compression.py`、`gateway/run.py`、`tests/gateway/test_compression_progress_notices.py`、`hermes_cli/gateway.py`、`agent/auxiliary_client.py`、`hermes_cli/auth.py`、`agent/gemini_native_adapter.py`、`tests/agent/test_gemini_native_adapter.py`、`hermes_cli/main.py`、`tools/delegate_tool.py`；它们不再依赖当前 `PATCHED_FILES` 才进入 overlap 轮询。仅 `gateway/run.py` / `tools/mcp_tool.py` 发生真实 3-way 冲突：前者的 `PATCH-COMPACTION-LIFECYCLE-SILENCE` 被 upstream `7a21bfe68a` 完全吸收并归档；后者的 PID liveness 被 `98fce8e52d` / `2663117f72` / `ef46ec03e1` 吸收，`PATCH-MCP-STDIO-WATCHER-LIFECYCLE` 收缩为只保留 watcher 单次实例化。其余原有 41 个 active 与既有 Archive 均按各自吸收条件复核，未发现新增完整吸收；审计另新增 `PATCH-TEST-RUNTIME-STATE-ISOLATION`，封堵 pytest 清空环境后污染生产状态文件的假绿。终态为 43 active + 10 Archive，90-file bundle、35 active + 7 archived gates、53/53 full PATCH evidence 全绿。
  吸收矩阵：`PATCH-MCP-TASKS-ASYNC-HANDOFF`=未吸收；`PATCH-MCP-STDIO-WATCHER-LIFECYCLE`=部分吸收；`PATCH-SKILL-CREATE-ROOT`=未吸收；`PATCH-FEISHU-SOCKS-DEPENDENCY`=未吸收；`PATCH-FEISHU-GROUP-ADMISSION`=未吸收；`PATCH-FEISHU-MISSED-EVENT-BACKFILL`=未吸收；`PATCH-FEISHU-GROUP-SCOPE`=未吸收；`PATCH-PLATFORM-CAPABILITY-SCOPE`=未吸收；`PATCH-FEISHU-GROUP-APPROVAL`=未吸收；`PATCH-FEISHU-NORMAL-REPLY`=未吸收；`PATCH-FEISHU-QUOTE-CHAIN-SESSION`=未吸收；`PATCH-GATEWAY-FAILOVER-STATUS-SILENCE`=未吸收；`PATCH-FEISHU-FINAL-ONLY`=未吸收；`PATCH-LOCAL-PROFILES`=未吸收；`PATCH-FEISHU-RESOURCE-ACCESS`=未吸收；`PATCH-DOCUMENT-EXTRACTION`=部分吸收；`PATCH-FEISHU-MARKDOWN`=未吸收；`PATCH-FEISHU-RESPONSE-BUDGET`=未吸收；`PATCH-TRUNCATED-TOOL-CALL-RECOVERY`=未吸收；`PATCH-MODEL-CONFIGURED-ONLY`=未吸收；`PATCH-VERTEX-VIDEO-ROUTING`=未吸收；`PATCH-MULTIMODAL-SIDECAR`=未吸收；`PATCH-HISTORY-RETENTION`=未吸收；`PATCH-APPROVAL-DARWIN-TMP`=未吸收；`PATCH-COMPACTION-LIFECYCLE-SILENCE`=完全吸收；`PATCH-LAUNCHD-WRAPPER-SUPERVISOR`=完全吸收；`PATCH-VERTEX-FALLBACK`=完全吸收；`PATCH-GEMINI-CUSTOM-NATIVE-BASE`=完全吸收；`PATCH-DASHBOARD-BUILD-CACHE`=完全吸收；`PATCH-DELEGATE-ACP-ROUTING`=完全吸收；无路径相交=19。
- 依赖：官方 updater 将运行时更新到 v0.20.6；Skills mirror `+1/~0/-0`，恢复 `research/llm-wiki/SKILL.md`。`npm audit fix` 重排 workspace lockfile 并把残余降为 Electron/extract-zip 2 high；修复需要越过 upstream stated range 到 Electron 40.10.6，按 P2 保留且不使用 `--force`，不影响飞书主链路。
- 已知摩擦：首次官方 updater 已完成 checkout，却因 fleet version check 返回空行而非零退出；事务保留固定 SHA，手工 3-way 回贴后以 reconcile 恢复。新 upstream 开始在 machine Hermes root 写 `spawn-ledger.json`；已将主 ledger 与 `.corrupt` quarantine 纳入 `.gitignore`、cleanup keep policy 和回归，避免运行态身份账本污染外层工作树或被误删。后续审计又修复 ownership 子串、零 owner PATCH、Archive gate/node/process/overlap、单文件零 collect、过期 upgrade range、canonical 数量、测试后 bundle/sandbox/外层工作树与持久 Gateway 身份等假绿窗口。Gateway 终态排空重启曾需 launchd 强制重新 kickstart，但未强杀在途任务。canonical suite 为 **39 files / 1732 passed / 0 failed / 9 skipped**，另有 2 个 test support modules 做独立语法/ownership 校验；sandbox/identity-sync **60 passed**。
- 配置漂移：config 保持 v39，无 deprecated key 或 active security advisory；Azure Foundry 与 AWS Bedrock 连通。未登录 provider 与未配置可选工具仍属 P3；最终 canonical/evidence/Gateway 双 PID 与 cleanup 数字以本轮 `--final-audit --json` 为准。
...
**上游吸收判断**：PID liveness 子项已由 upstream `98fce8e52d` / `2663117f72` / `ef46ec03e1` 完全吸收。剩余补丁仅在 upstream 的 stdio RPC 路径只实例化一次 child watcher、同一 awaitable 用于判定与调度，并有 RuntimeWarning/调用次数回归后可删除；当前 `8966b0a700` 仍以 `_watch_children()` 分别执行 `inspect.isawaitable(...)` 与 `asyncio.ensure_future(...)`，故继续保留最小 hunk。
