...
- 已知摩擦：首次官方 updater 已完成 checkout，却因 fleet version check 返回空行而非零退出；事务保留固定 SHA，手工 3-way 回贴后以 reconcile 恢复。新 upstream 开始在 machine Hermes root 写 `spawn-ledger.json`；已将主 ledger 与 `.corrupt` quarantine 纳入 `.gitignore`、cleanup keep policy 和回归，避免运行态身份账本污染外层工作树或被误删。第二轮审计又修复 ownership 子串、零 owner PATCH、Archive gate 错绑、跨 PATCH/import-only 轨迹、过期 upgrade range、canonical/collect 数量、测试后 bundle/sandbox 与持久 Gateway 状态等假绿窗口。Gateway 终态排空重启曾需 launchd 强制重新 kickstart，但未强杀在途任务。canonical suite 为 **41 files / 1732 passed / 0 failed / 9 skipped**，sandbox/identity-sync **60 passed**。
...
本轮把上述第二轮缺口统一收敛到同一条链：ownership 只认反引号内展开后的完整路径，并拒绝零受管文件的 active engineering/dedicated PATCH；active 与 Archive gate header、唯一置绿变量、Step 8c 消费集合三方闭合；每个 PATCH 的 evidence node 在独立 pytest 进程执行，调用轨迹忽略 `<module>` import；PATCHES 当前摘要以 `` `PATCH-ID`=未吸收 | 部分吸收 | 完全吸收 `` 逐项登记全部 upstream-overlap PATCH，并记录无路径相交 active 数；evidence range 的终点必须等于 HEAD；canonical `passed+skipped` 必须等于 full collect；final-audit 在 canonical 后再次执行 sandbox verifier 与 bundle byte/cached/reverse/index-clean，并验证持久 `gateway_state.json` 的 PID、argv、code SHA 属于当前真实 Gateway，spawn ledger 不含 pytest 记录。
...
本轮负向回归进一步覆盖 ownership 子串、零 owner PATCH、归档 gate 错绑、gate 重复置绿、跨 PATCH pytest 进程隔离、import-only 伪覆盖、canonical/collect 数量不一致、过期 upgrade range、测试后 bundle 漂移、staged lockfile 和 pytest runtime-state 污染；任一故障注入必须令对应审计非零。
