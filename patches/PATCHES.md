...
## 当前版本：v0.21.0 (upstream `main` `79445a496c86a19332ad786494b8384d2167e2d0`，2026-09-10)
...
- 配置漂移：config v40 保持；新增的 `install_id`、`.install_id.lock` 与 `runtime/` 已纳入保留/忽略策略。当前模型链已收敛为 `azure-foundry/gpt-5.5 → claude-sc/claude-fable-5-1 → vertex/google/gemini-3.7-flash`；SC 通过用户级 model-provider 插件复用 `~/bin/claude-sc` 的 endpoint/token，Bedrock 专项图片识别、output-cap 接线与状态样例已从 replay overlay 移除或替换，未新增内层 provider patch。文本、high reasoning、prompt-cache 标记、工具调用、原生图片、真实 fallback 与 Gateway 重载均已验证。终态为 45 active + 10 Archive，**55/55 full PATCH evidence**、44 files / **2212 passed / 0 failed / 6 skipped**、另有 2 个 test support modules、2218 collected、22/22 probe、118-file bundle、36 active + 7 archived gates、sandbox/identity-sync **161 passed**；最终状态由本轮 `--final-audit --json` 闭合。
