...
## 当前版本：v0.20.5 (upstream `main` `9aa7530f7b53699e2c6d648ded8f6300503b3dc7`，2026-08-27)
...
**最近一次升级（v0.20.5，`503d863fcd` → `9aa7530f7b`，+889 commits，2026-08-27）要点**：

- 上游主线：唯一 acquisition 先因 HTTPS `HTTP2 framing` 在 `target_sha=pending` 退出，按恢复条款经进程级 SSH 443 rewrite 固定 `TARGET_SHA=9aa7530f7b`；889 commits 主要覆盖 Desktop/fleet session、Gateway reconnect/profile route、update 多后端与 SSH ownership、MCP transport/catalog、批量 Tool Search、lean compaction、terminal backend、安全审批以及 v39 BFL core-tool 迁移。取得 SHA 后全部收敛均为 no-network `--reconcile`，remote 未改、目标未移动。
- patch apply / registry：逐项轮询原 42 个活跃 + 9 个 Archive PATCH；25 个活跃 PATCH 与 36 个受管路径重叠：`agent/chat_completion_helpers.py`、`agent/conversation_loop.py`、`agent/image_routing.py`、`agent/prompt_builder.py`、`agent/skill_utils.py`、`agent/tool_executor.py`、`gateway/authz_mixin.py`、`gateway/platforms/base.py`、`gateway/run.py`、`gateway/session.py`、`gateway/slash_commands.py`、`hermes_cli/config_defaults.py`、`hermes_cli/doctor.py`、`hermes_cli/tools_config.py`、`pyproject.toml`、`tests/agent/test_image_routing.py`、`tests/hermes_cli/test_doctor.py`、`tests/hermes_cli/test_tools_config.py`、`tests/run_agent/test_provider_fallback.py`、`tests/run_agent/test_run_agent.py`、`tests/tools/test_approval.py`、`tests/tools/test_mcp_tool.py`、`tests/tools/test_skill_manager_tool.py`、`tests/tools/test_tool_search.py`、`tools/approval.py`、`tools/lazy_deps.py`、`tools/mcp_tool.py`、`tools/skill_manager_tool.py`、`tools/skills_tool.py`、`tools/tool_search.py`、`tools/vision_tools.py`、`toolsets.py`、`uv.lock`、`website/docs/reference/environment-variables.md`、`website/docs/user-guide/configuration.md`、`website/docs/user-guide/features/mcp.md`。5 个真实冲突均按“上游增强 + 本地不变量并存”解决：skill essential guard/平台 scope、background MEDIA repair/画像脱敏、vision probe/多模态回归、fallback telemetry/配置链去重、MCP stdio fast-fail/Tasks dispatch。没有既有 PATCH 完全吸收、进一步部分吸收、归档或收缩，`PATCH-DOCUMENT-EXTRACTION` 保持既有部分吸收；新增 `PATCH-MCP-STDIO-WATCHER-LIFECYCLE` 修复上游 watcher coroutine 泄漏，终态 43 active。2026-08-27 现场追审又演进 `PATCH-GATEWAY-FAILOVER-STATUS-SILENCE`：用真实 producer 生成 notice 穿过 Gateway consumer，补上 upstream 新 `Model fallback:` 文案。88-file bundle 重新生成；39 files **1736 passed / 0 failed / 9 skipped**（3 anydoc 可选依赖 + 6 无首发期 toolset 条件跳过），35 active + 6 archived gates、43+9 PATCH evidence 与 byte/cached/reverse/index-clean 全绿。
- 依赖：官方 updater 同步 venv 并新增 Snowball stemmer；Skills mirror `+1/~0/-11`（恢复 `research/llm-wiki`、清理 11 个 `.DS_Store`），runtime metadata 保留。Doctor 仍报 Web 4 high、UI-TUI 3 high；`npm audit fix` 被 Vite 8 peer `ERESOLVE` 阻挡，属构建工具链 P2 上游 lock/range blocker，不影响飞书主链路，未使用 `--force`。
- 已知摩擦：官方 updater 的自举 restart 一度留下 registered-but-down Gateway，固定事务由 reconcile 恢复；手工 3-way 后 quick evidence 错把旧 bundle parity 当 preflight 不变量，现改为 quick 只审结构/contract、full 才审 bundle，并新增负向测试；final-audit 也已从“当前版本号唯一”改为“当前 ISO 周唯一”，允许同一 release 跨周。上游 Tool Search 改为 `queries[]` / `names[]`，sandbox verifier 已同步并保持 owner/group 边界。8 月 27 日 SpaceSight Tech Sharing Group 现场又抓到 fallback producer 文案漂移、旧 consumer fixture 与 gate 同步假绿；现已改成 producer-derived 回归。同日主会话抓到 stdio PID 存活判定反转，修复后 `hermes mcp test hypertex` 以 1.116 秒连接并发现 9 个工具。插件与身份同步仍 **60 passed**，最终 Gateway wrapper/child 为 `35776/35778`。测试另保留一个上游 `_BarrierDB.flush_token_counts` fixture warning，不影响运行时或通过数。
- 配置漂移：config 由 v38 迁移到 v39，版本仍 v0.20.5；主模型/fallback/compression 与 Feishu owner/group 能力边界未变化。Doctor 无 active security advisory、deprecated key 或配置待迁移；仅保留 npm P2、未登录 provider 与未配置可选工具 P3。
...
| **状态** | 🟡 未上游合并（上游 `2f33833de8` 的 watcher 工厂调用两次且 PID 存活判定反转） |

**问题**：上游为 MCP stdio RPC 增加子进程死亡快速失败时有两个独立缺陷。第一，先调用一次异步 `_watch_stdio_children()` 检查返回值是否 awaitable，真正调度时又调用第二次；第一支 coroutine 从未被 await 或关闭，每个正常工具调用都会产生 `RuntimeWarning: coroutine ... was never awaited`。第二，`_stdio_children_dead()` 在 `psutil.pid_exists(pid)` 为真时返回 `True`，把“至少一个 child 存活”反向解释为“全部 child 已退出”，其后的正确 `return False` 永远不可达。2026-08-27 12:08:24，主会话调用 `hypertex_iterate_case` 时 HyperTeX MCP 正常启动并完成数据库初始化；12:08:25 Hermes 随即误报 subprocess exited 并在 1.59 秒内 fast-fail；12:09:26 同一 transport 又按配置完成 60 秒 idle recycle，证明进程并未崩溃、工具请求也没有真正提交给 HyperTeX。

**修复**：在真实 RPC 开始前只调用一次 watcher 工厂，把结果保存为 `_watch_coro`；同一个 awaitable 同时用于能力判定与 `asyncio.ensure_future()`。`_stdio_children_dead()` 改为“发现任一存活 PID 立即返回 `False`，遍历完仍无存活 PID 才返回 `True`”；无 PID 或 HTTP transport 保持 unknown/fail-open 为 `False`。普通 MCP `call_tool` 与 task-aware `call_task_aware_tool` 共用这条竞速与存活判定路径。

**验证**：`test_stdio_child_watcher_is_created_once_without_leaking_probe_coroutine` 用带调用计数的真实 async watcher 穿过 `_make_tool_handler()`，断言一次 RPC 只构造一次 watcher；`test_stdio_children_dead_is_false_when_any_child_is_alive` / `test_stdio_children_dead_is_true_when_all_children_exited` 直接锁定 alive/dead/mixed PID 语义；`test_live_stdio_child_does_not_fast_fail_tool_call` 再穿过真实 handler，证明活 child 的正常 `call_tool` 返回成功。Step 8b 以 `-W error::RuntimeWarning` 运行上述四条，并锚定 `_watch_coro` 被直接交给 `asyncio.ensure_future()`。同文件既有 lazy reconnect 与死子进程快速失败测试继续覆盖兼容路径。修复后的真机只读 canary `hermes mcp test hypertex` 在 1.116 秒内连接成功并发现 9 个工具；不执行任何 create/iterate mutation。

**上游吸收判断**：当 upstream 在 stdio RPC 路径只实例化一次 child watcher，正确实现“任一 child 存活则连接未死、全部退出才 fast-fail”，并有 RuntimeWarning、alive/dead/mixed PID 与 handler 边界回归后，可删除本补丁。
