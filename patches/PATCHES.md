...
| **状态** | 🟡 未上游合并（上游 `2f33833de8` 的 watcher 工厂调用两次） |

**问题**：上游为 MCP stdio RPC 增加子进程死亡快速失败时，先调用一次异步 `_watch_stdio_children()` 检查返回值是否 awaitable，真正调度时又调用第二次。第一支 coroutine 从未被 await 或关闭，每个正常工具调用都会产生 `RuntimeWarning: coroutine ... was never awaited`；若 watcher 工厂未来携带状态，探针对象与实际竞速对象还可能分叉。该缺陷在本轮 canonical suite 的 `test_successful_call` 与 lazy reconnect 用例中稳定出现，且与 MCP Tasks extension 是否启用无关。

**修复**：在真实 RPC 开始前只调用一次 watcher 工厂，把结果保存为 `_watch_coro`；同一个 awaitable 同时用于能力判定与 `asyncio.ensure_future()`。普通 MCP `call_tool` 与 task-aware `call_task_aware_tool` 共用这条竞速路径；不存在 watcher、stub 返回非 awaitable 或 RPC 本身非 awaitable 时继续走既有兼容分支。

**验证**：`tests/tools/test_mcp_tool.py::TestToolHandler::test_stdio_child_watcher_is_created_once_without_leaking_probe_coroutine` 用带调用计数的真实 async watcher 穿过 `_make_tool_handler()`，断言一次 RPC 只构造一次 watcher；Step 8b 以 `-W error::RuntimeWarning` 运行该节点，并锚定 `_watch_coro` 被直接交给 `asyncio.ensure_future()`。同文件既有成功调用、lazy reconnect 与死子进程快速失败测试继续覆盖兼容路径。

**上游吸收判断**：当 upstream 在 stdio RPC 路径只实例化一次 child watcher（或改为等价、无未 await coroutine 的生命周期实现），并有把 RuntimeWarning 提升为错误的回归后，可删除本补丁。
