...
## 当前版本：v0.18.2 (upstream `main` `ffa52575`，2026-07-14)
...
**最近一次升级（v0.18.2 main 滚动，+123 commits，basis `bd740f20` → `ffa52575`）要点**：

- 上游主线：可靠性/State——macOS 上 SQLite 强制 `synchronous=FULL` 防 btree 损坏（`9aba95b05`，对本机部署直接相关）；Session/CLI——close 持久化 salvage（#64004 `068450607`）：close 快照置于 staging lock 下、direct flush 与 close handoff 串行化、保留 resumed history / noted staged input / clean multimodal override（`32bdc67e1` / `962189d9e` / `69fd846ef` / `475922f2c` / `a27d51ef4` 等）；Compaction/Context——output-cap 错误豁免 compression-disabled guard 并按 provider available_out + 请求估算重算 retry cap（#55546 `62ea80058` / `57f181483` / `127f6e151`），fallback compaction 计为 ineffective 并持久化 breaker（`5ce827cac` / `af7dceaf7`），compression lock 异常 fail-closed（`8f29c9f4e` / `2627933f3`）；Gateway 生命周期——shutdown 时 drain in-flight api_server 运行、quiesce API/cron、drain 期间保留 cron fire（`021ee3454` / `104ffeae2` / `ffc10cc65` / `915f1bf1b`），delegation 后台完成持久化 + 去重投递（`67f4e1b4a` / `d0e9a42ce` / `94a7705bd`）；Cron——SessionDB init 有界超时防 wedge、timeout 读 config.yaml、running-set 检查失败时保留 live one-shots（`c675e7c79` / `ccb045ba7` / `ffa525754`）；Kanban——clean-exit protocol violation 有界重试 + 未汇报 worker nudge（`c3656e9f0` / `452861fdc` / `03fbf6edb`）；Providers——DeepInfra 正式成为 LLM provider 并加固多模态路由/前缀别名（`fe002eb12` / `2fc3f9c1f` / `658c01126`），Gemini 请求上下文改进 + TTS 限官方 host（`b8eb89f5c` / `226e8de82`），Anthropic `sk-ant-oat` pool 凭据 OAuth 归一化（`77763f00f` / `0512f06a6`），provider 自动探测后校验 credential pool（`78e844d44`），Codex `/usage reset` 兑换 banked resets（`89bd0fba9`）；Skills/Tools——skill slash command 防核心命令与 slug 冲突（`370ebf2d3`），tool discovery AST 前文本预过滤（`2d71e2f1e`）；Windows——Git Bash broken login shell 自愈 + 非登录 fallback coreutils PATH（`c4622a1d5` / `4a58e22e9`）；Desktop/CI/测试（本轮最大簇）——desktop 测试迁 vitest 与 React act() 修复、zoom restore 同步（`a1a4c8ce1`）、worktree base-branch picker + 自动 fetch（`6f7ee72be` / `f6d1fd511`）、node-pty spawn-helper 权限（`7fdae5d22`）、Thinking widget 渲染 reasoning（`19641fab7`）、dashboard 保留 memory.provider schema（#63886 `861d69c7b`）、CI timing report 与 workspace 发现 fail-closed（`f0b7cf383` / `757783420`）、禁止测试正则扫源码（`5265b3002`）。
- patch apply：首轮脚本整体 apply 失败并按设计回滚；手工 `git apply --3way` 后**仅一处真实冲突**：`hermes_cli/doctor.py`（PATCH-18 `"vertex"` slug 豁免与上游新增 DeepInfra 条目 `fe002eb12` 同集合撞行，`878→884`），按"并存"手工合并（上游 `"deepinfra"` 条目 + 注释保留，本地 `"vertex",` 移至其后）。重跑脚本后 59 files **clean apply**（含 `gateway/run.py` `10861→10971` 级、`agent/auxiliary_client.py` `5026→5057` 等行号漂移自动 rebase），全部 17 条活跃 sentinel、PATCH-2/3/4/5 upstream guards 与 sandbox plugin verify 全 OK；无新退役；`local-patches.diff` / `.local-patches.base` 已刷新且与内层实际 diff 逐字节一致。功能回归：23 个 patch 相关测试文件 **1747 passed / 1 failed**（上轮基准 1707/1，+40 为上游新增测试）——唯一失败仍是 `test_approval.py::TestDetectDangerousRm::test_nonrecursive_verification_artifact_cleanup_is_not_dangerous`，即上游 `0c8bcd339` 自带测试在 macOS 的 realpath 自身 bug，摩擦表在案，上游本轮未修。
- 依赖：上游 `pyproject.toml` / `uv.lock` 零变更，editable 维持 `0.18.2 (2026.7.7.2)` 重建；npm 仅新增 root `check` workspace script（`7b3f3047a`），root/ui-tui/web 重装并 rebuild web UI（vite 8.1.0），`npm audit` no vulnerabilities；Skills mirror 首跑 `+0/~1/-14`——上游本轮 skills/ 零改动，-14 为镜像目录内非上游文件的一次性清理（rsync 明细未留存；含 `.bundled_manifest` 基线，其余推断为当日 Finder 浏览产生的 .DS_Store 类残留），二跑回到已知振荡 `+0/~1/-1`，patched llm-wiki 已回同步且 manifest re-baselined；lazy backend 16 项中 15 项 current，`platform.matrix` 因 python-olm 构建失败保留既装版本（非阻塞）。
- 已知摩擦：① 升级窗口内补丁还原态，PATCH-21 锚点暂不生效，Matrix 重进 refresh 名单并因 python-olm 构建失败报 ⚠（固有时序，回贴后恢复）；② `package-lock.json` 在 npm audit fix 后 dirty（esbuild `"peer": true` 归一化移除，无版本变化），照旧留在内层工作树不提交；③ 上游 verifier temp cleanup 豁免在 macOS 失效持续（见上，行为等同修复落地前，无安全回归）；④ 本轮 `uv` 未触发 python-path fallback；⑤ 上游 main 当日高频推进：状态快照 `52cafa6f8` → 一跑落点 `78e844d4` → 二跑落点 `ffa52575`，收尾时又新增 ~6 commits（gateway ws_orphan_reap、压缩探测 fail-closed、update 跳过未变 lockfile 安装等），滚动目标属常态，留待下轮例行升级。
- 配置漂移：最终 `hermes doctor` 显示 `Config version up to date (v33)`、`All checks passed`（首轮补丁缺位时报的 2 条 vertex provider/slug 问题即 PATCH-18 缺位症状，回贴后消失）；Gateway plist matches current install，launchd PID `87758` 已重启加载 patched modules；仅保留未登录 auth provider、未配置 optional tool/API key、Skills Hub 未初始化等可选提示。
