...
## 当前版本：v0.20.5 (upstream `main` `9aa7530f7b53699e2c6d648ded8f6300503b3dc7`，2026-08-26)
...
**最近一次升级（v0.20.5，`503d863fcd` → `9aa7530f7b`，+889 commits，2026-08-26）要点**：
...
- 已知摩擦：官方 updater 的自举 restart 一度留下 registered-but-down Gateway，固定事务由 reconcile 恢复；手工 3-way 后 quick evidence 错把旧 bundle parity 当 preflight 不变量，现改为 quick 只审结构/contract、full 才审 bundle，并新增负向测试；final-audit 也已从“当前版本号唯一”改为“当前 ISO 周唯一”，允许同一 release 跨周。上游 Tool Search 改为 `queries[]` / `names[]`，sandbox verifier 已同步并保持 owner/group 边界；插件与身份同步仍 **60 passed**，最终 Gateway wrapper/child 为 `51103/51105`。测试另保留一个上游 `_BarrierDB.flush_token_counts` fixture warning，不影响运行时或通过数。
