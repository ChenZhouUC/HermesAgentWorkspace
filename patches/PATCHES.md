...
- 配置漂移：config 保持 v39；终态为 43 active + 10 Archive，**53/53 full PATCH evidence**、39 files / 1777 collected、21/21 probe，canonical suite 为 **39 files / 1771 passed / 0 failed / 6 skipped**，另有 2 个 test support modules；90-file bundle、35 active + 7 archived gates、sandbox/identity-sync **140 passed**。Gateway、Doctor、cleanup 与深度 toolchain 审计由最终 JSON 再闭合。
...
**2026-09-01 owner DM 飞书文档插图桥接增量**：主会话真实 Task 62/64 暴露了群聊素材桥未覆盖 owner DM：模型已导出图片并把 6 个路径写入 iterate 参数，但旧 bridge 仍以当前消息附件覆盖调用方路径，`media=0` 时实际传入空数组。现让固定 `feishu_doc_manage(read_url, include_images=true)` 在 owner DM 中把图片写入 `~/.hermes/tmp/feishu-doc-assets/<chat-id-hash>/`，仅返回相对 `image_path`；HyperTeX 只接受该精确会话 workspace 内的普通文件并再次复制到私有 staging。一次性历史目录 `~/.hermes/tmp/hypertex_doc_assets/` 明确不纳入信任根、不迁移旧素材。插件 **112 条**回归新增 owner reader 正例、私有 workspace → staging 正例、历史目录与任意外部路径反例；新任务必须重新走固定 reader，不能复用偶发手工路径。
