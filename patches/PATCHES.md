...
> 以上为 `hermes-update.sh` 中数组的快照（59 文件，2026-07-18 与脚本核对一致）。**脚本数组是唯一权威来源**；增删补丁文件后请同步刷新本快照。
...
## 当前版本：v0.18.2 (upstream `main` `c48d5341`，2026-07-18)
...
**最近一次升级（v0.18.2 main 滚动，+656 commits，basis `ffa52575` → `c48d5341`）要点**：

- 上游主线（656 commits，历轮最大跨度，间隔 4 天）：**安全**——webhook 签名头 / API bearer 含非 ASCII 时不再崩端点、改拒绝并推广到含 Feishu adapter 的全部 `compare_digest` 站点（`1b69c47e9` / `a6d9d1d2c`），Telegram 传输错误日志脱敏 bot token（`6e96b745d`），/resume 回放清洗终端转义序列 + 共享多用户会话消毒 sender-name 前缀（`17485cbcd` / `170959d80`，后者即本轮 PATCH-13/14 冲突来源），dashboard attach token 按会话作用域（`921c17af8`），平台事件验签 fail-closed（`14f023cd0`），approval"含执行动作选项"检测统一防审批绕过（`b90dbac1d`），kanban 附件 SSRF 防护（`c2e11bf41`），install-tree AGENTS.md 不再当项目上下文加载（`33513991b`）；**Gateway/multiplex**（本区间最大主题）——多 profile 多路复用主线：入站按 profile 路由、会话持久化 profile_name、secondary adapters / secret_scope / api_server `/p/<profile>/` 全链路加固（`5e65f6d79` / `e8b7ce8c1` / `8fc989b41` / `7aa21e336`）；**Gateway 生命周期**——会话 reset/recovery 边界统一、活跃进程检查出错 fail-closed 决不误清会话（`9fc0074ba` / `ca559a785`），转录追加失败自动重试（`53d358838`），api_server 直接 bind + 端口冲突终止无限重连（`164bca658` / `bda8bd76a`）；**State/SQLite**——FTS 损坏自愈、WAL 周期 flush 改 PASSIVE checkpoint 防 B-tree 损坏、备份快照 fail-closed（`9e1b1d753` / `c2a3b9ce5` / `332fbadd7`），restore 边界自愈 role 交替违规（`ee659d1d8`）；**Cron**——真实执行台账（`d9dd05b69`）、长任务防双跑（`cd5371876`）、HERMES_HOME 迟重指不误写 jobs 文件（`5c121f157`）、chronos 缓存 PyJWKClient 止 JWKS 风暴（`9884b4faa`）；**CLI/更新器**——退出 watchdog 改收到关停信号才布防（`c66891db0`），全局切模型同步 base_url/api_mode（`398634eed` / `c7aa01ff0`），lockfile 未变跳过 npm install（`aa56243a8`），config 子命令新增 get/unset（`53adb3fd9`）；**Compaction/Auxiliary**——压缩交接前缀明示"工具仍可用"（`c7205040c`），fallback 候选独立超时预算 + 冷却升级（`bd7e48023`），auxiliary runtime 按 turn 隔离修跨会话串扰（含 image_routing，`73057ed16` / `fdc6c32d7`），自动标题治理（原子写、不覆盖手动标题、尊重禁用，`d05cd7c1e` 等）；**模型/Reasoning**——**行为变化**：无模型时静默默认锁定 GLM-5.2、决不落到 Anthropic 旗舰（`080daa3f4`），per-model reasoning_effort 收敛单一 chokepoint、xhigh 改 clamp 不再降级（`d9cdb8192` / `633fc7ab8`），新增 Upstage Solar provider（`20502b407`），Kimi K3 上架退役 k2.x（`75ca29fb2`），Bedrock 流式失败回退 Converse API（`5e6a0d9ee`）；**Codex**——app-server 事件桥实时进 TUI/desktop 工具卡片（`18331b9bb`），commentary 独立流式通道 + 脱敏（`7041c56cd` / `a15397d61`），cache-scope header 截 64 字符修 400（`8051ebae3`）；**MCP/OAuth**——hosted OAuth 全链路加固（reauth 串行化、callback 端口 TOCTOU 关闭，`11eaa77da` / `95a0f9c83`），redirect_uri/redirect_host 可配置（`6297634d2`），ResourceLink/EmbeddedResource/Audio 块物化（`1d98f8dd9`）；**Skills/Tools**——SKILL.md frontmatter 剥 UTF-8 BOM 并推广到全部兄弟解析器（`a4ecb3da9` / `780e09807`，触及 PATCH-1/6 相关文件但无冲突），终端/文件工具 cwd 改 per-session 记录（四步重构，`be2a1290d` 等），混合批次非法工具名 / null 参数 / assistant block-list 死循环三修（`348e9912f` / `94456a128` / `296494db0`），**行为变化**：browser eval denylist 改 opt-in 放宽（`0f102fa4d`）；**平台**——Telegram 轮询健康门控挂真实进度、DoH 与系统 DNS 解绑修 init 卡死（`b8295cf6f` / `e16743b0d`），Slack Agent View 成套（`9a3b676fe`），Google Chat clarify 卡片化（`09505a393`），webhook 默认双栈 bind（`92876effe`），/reasoning /fast 内联选择器（`bd37ff913`），nemo-relay 0.5 动态插件（`0c8cf2188`）；**Desktop**——PR #59846 大重构（contribution shell / 插件 SDK / 多会话平铺 / 布局树，`77d5b2d57` / `99ff67eb0`），会话切换性能专项（`c856f3645` / `9b8b054c2`），会话串扰三修（冷恢复清转录 / stop 打断错会话 / 排队 prompt 排空，`7dc21f08a` / `11d36232c` / `432fca55a`）；**Dashboard/TUI**——主题闪烁治理（`72562be96`），图片路由用实时切换后模型（`ef9a98315`），后台进程通知路由回所属会话（`81fc24862`）；**Memory/Honcho**——首条消息注入 base context、honcho_search 真实跨会话搜索（`bef9eea3e` / `c1c59e347`），外部记忆 prefetch fail-fast（`d77c455d7`）；**CI/Windows**——负载下测试/工作流/归属可靠 + fork PR token 回退（`597615ade` / `1e01a4bbe`），JS 工具链统一（根级 eslint+prettier、JS 测试迁 vitest，`214cbf77f` / `64389a2ce`），Git Bash Mandatory ASLR 检测（`1f7d2be22`），credential-pool 日志锁风暴节流（`594308d4b`）。
- patch apply：首轮脚本整体 apply 失败并按设计回滚；手工 `git apply --3way` 后 56 files clean、**三个文件真实冲突**（上游本区间触碰 `gateway/run.py` ×37、`gateway/session.py` ×14 个 commit，为历轮最高）：① `gateway/run.py` 两处 hunk——import 区与作者前缀块，根因均为上游 `170959d80` 在 PATCH-13 替换的同一块引入 `neutralize_untrusted_inline_text` 显示名消毒，按"并存 + 吸收"合并：保留补丁的非 DM 锚定与幂等 `_with_current_author_prefix`，改为先消毒显示名再前缀（PATCH-13 语义增强，详见该节）；② `gateway/session.py`——PATCH-14 人物画像大块与上游新增 `neutralize_untrusted_inline_text` 函数同插入点，纯"并存"保留两者（PATCH-14 锚点 `380→401`，现挂在该新函数之后）；③ `tests/gateway/test_session.py` import 撞行，双导入并存。重跑脚本后 59 files **clean apply**（作者前缀 hunk `10558→10882` 级行号漂移自动 rebase），全部 17 条活跃 sentinel、PATCH-2/3/4/5 upstream guards 与 sandbox plugin verify 全 OK；无新退役；`local-patches.diff` / `.local-patches.base` 已刷新且与内层实际 diff 逐字节一致。功能回归：23 个 patch 相关测试文件 **1767 passed / 1 failed**（上轮基准 1747/1，+20 为上游新增测试）——唯一失败仍是 `test_approval.py::TestDetectDangerousRm::test_nonrecursive_verification_artifact_cleanup_is_not_dangerous`，即上游 `0c8bcd339` 自带测试在 macOS 的 realpath 自身 bug，摩擦表在案，上游本轮未修。
- 依赖：上游 pyproject/uv.lock 变更——slack-bolt `1.27.0→1.29.0`、slack-sdk `3.40.1→3.43.0`（`d6590f8b1`），honcho-ai `2.0.1→2.2.0`（`29e047170`），nemo-relay `0.3→>=0.5,<1.0`（`0c8cf2188`），新增 pytest marker `real_agent_prewarm`；editable 维持 `0.18.2 (2026.7.7.2)` 重建；lazy backend 16 项：`platform.slack` ↑refreshed（随 SDK bump）、14 项 current、`platform.matrix` python-olm 构建失败保留既装版本（已知非阻塞）；npm root 新增 `tests-js` workspace 与 `fix` script、移除 `@assistant-ui/store` override（`ddd6ad43f` / `214cbf77f`），root/ui-tui/web 重装并 rebuild web UI（vite 8.1.0），`npm audit` no vulnerabilities；Skills：上游本轮有真实改动——新增 `hermes-desktop-plugins`（`a57b33782`）、humanizer 模式扩充（`ef010f874`）、hermes-agent webhooks 文档；内层 bundled sync `+1 new / ↑3 updated`（hermes-desktop-plugins / llm-wiki+humanizer+hermes-agent），外层 mirror 首跑 `+0/~1/-4`（-4 为镜像内非上游残留清理，既往同型）、二跑 `+0/~1/-1` 已知振荡，patched llm-wiki 已回同步且 manifest re-baselined。
- 已知摩擦：① 升级窗口内补丁还原态，PATCH-21 锚点暂不生效，Matrix 重进 refresh 名单并因 python-olm 构建失败报 ⚠（固有时序，回贴后恢复）；② `package-lock.json` 在 npm audit fix 后 dirty（esbuild `"peer": true` 归一化移除，无版本变化），照旧留在内层工作树不提交；③ 上游 verifier temp cleanup 豁免在 macOS 失效持续（见上，行为等同修复落地前，无安全回归）；④ 本轮 `uv` 未触发 python-path fallback；⑤ 本轮 +656 commits 为历轮最大跨度，首见 3 文件同轮真实冲突，其中 run.py/test_session.py 两处根因同一上游安全 commit `170959d80`——上游对"共享会话 sender-name"的安全化与 PATCH-13/14 的定制区高度重叠，后续升级该区域冲突概率上升；收尾复查上游未再推进（0 behind，与上轮"当日高频"不同）。
- 配置漂移：最终 `hermes doctor` 显示 `Config version up to date (v33)`、`All checks passed`（首轮补丁缺位时报的 2 条 vertex provider/slug 问题即 PATCH-18 缺位症状，回贴后消失）；Gateway plist matches current install，launchd PID `29928` 已重启加载 patched modules；仅保留未登录 auth provider、未配置 optional tool/API key、Skills Hub 未初始化等可选提示。
