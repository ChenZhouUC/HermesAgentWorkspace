...
> 以上为 `hermes-update.sh` 中数组的快照（124 文件，2026-09-12 与脚本核对一致）。**脚本数组是唯一权威来源**；增删补丁文件后请同步刷新本快照。机器读取请用 `bash ~/.hermes/hermes-update.sh --print-patched-files`，不要解析本快照。
...
## 当前版本：v0.21.2 (upstream `main` `28bf67b17607af53c147431f117d07955dc870da`，2026-09-12)

**活跃补丁**：当前共 47 个语义补丁。38 个工程内补丁由 Step 8b/8c 管理；`PATCH-NPM-DEPENDENCY-HYGIENE`、`PATCH-REPLAY-BUNDLE-FULL-INDEX`、`PATCH-UPDATE-GATE-EXIT-STATUS`、`PATCH-UPDATE-GIT-FETCH-RETRY`、`PATCH-UPDATE-TRANSACTION-PIN`、`PATCH-SKILLS-MIRROR-METADATA`、`PATCH-GATEWAY-RESTART-CLEANUP` 是运行时补丁，由对应 update step 管理；`PATCH-CLAUDE-SC-PROVIDER` 与 `PATCH-FEISHU-GROUP-SANDBOX` 是配置仓库用户插件补丁、由 Step 8e 管理。完整活跃 ID 以上方执行链清单为准；Archive 中的定义只保留历史与重新启用条件，不计入活跃数。

**最近一次升级（v0.21.2，`3b45681c25` → `28bf67b176`，+204 commits，2026-09-12）要点**：

本次为用户明确要求的同 SHA 全量轮询，固定 `28bf67b176`，未调用 update/fetch/pull。下述 upstream 差量与交叉矩阵保留最近一次升级的历史区间，不代表本次获取了新提交。基线完整审计 56 项全部通过后，注册入口反例复现 Skills 缓存/平台策略与 MCP task mutation 审批共 24 个失败；修复并扩展到 41 个持续回归实例。专用 probe 再注入 eager/lazy pin 错位、execute 写旧环境和丢失归档，旧审计四种均误绿；现精确核对声明位置和实际迁移产物。四个 PATCH 的实现/探针、证据和既有 gate 同步增强；终审另新增独立 `PATCH-CODEX-WATCHDOG-TEST-CLOCK` 稳定超时行为回归，新增受管文件仅为 Skills 的 dedup/文件读取模块。

- 上游主线：最近一次升级的一次 acquisition 固定 `28bf67b17607af53c147431f117d07955dc870da`，所有后续执行均为 no-network `--reconcile`。多 profile 的授权、工具策略、MCP 同名连接和缓存隔离加强（`9c9e7ab6e5`、`ceaf622c6d`、`199c66f70f`）；Gateway 旧 turn 的 lease/finalizer 与单轮模型恢复按 generation 隔离（`436ec48985`、`93ececb383`）；失败 turn 写入稳定边界以防未来消息重放旧副作用（`25c69ee63c`、`044a77b3b6`）。上游退役 Collective Wisdom V1（`0dcadf6f41`）、补 cron 重启缺口 catch-up（`eaa5ac94ea`），curator 缺省时限改为 14/30 天（`be2f7e9c36`）。
- patch apply / registry：`plugins/platforms/feishu/adapter.py`、`tools/skills_tool.py` 与 `toolsets.py` 三处 3-way 冲突按语义合并。Feishu 使用上游 scope-aware YAML setter，并合并本地 admission/history/attachment 字段；无 allow_bots 时仍返回其他配置。Skills 保留 allowlist/可见性与只读工具集，移除上游已退役的 Wisdom 工具和 editorial 描述。旧 env-only 断言改为显式环境优先、profile extra 回退；根 YAML → adapter 增加默认/独立 profile 和有/无 allow_bots 四组合。现有部分吸收项保留剩余不变量，本轮新增 `PATCH-CODEX-WATCHDOG-TEST-CLOCK`；无完整吸收或归档 PATCH。终态为 47 active + 10 Archive，38 active + 7 archived gates；124-file bundle 正反向回放、独立 index 字节比较及来源 SHA 闭合。
  上游重叠/吸收审查：本轮 25 个 active 与 35 个受管路径相交，5 个 Archive 与 5 个声明路径相交，active/Archive 去重后 38 条。相交路径全集：`agent/auxiliary_client.py`、`agent/models_dev.py`、`agent/prompt_builder.py`、`agent/skill_utils.py`、`agent/tool_executor.py`、`gateway/authz_mixin.py`、`gateway/platforms/base.py`、`gateway/run.py`、`gateway/run_agent_cache.py`、`gateway/run_busy.py`、`gateway/run_inbound.py`、`gateway/run_turn.py`、`gateway/session_state.py`、`gateway/slash_commands.py`、`gateway/slash_commands_model.py`、`gateway/slash_commands_session.py`、`hermes_cli/auth.py`、`hermes_cli/config_defaults.py`、`hermes_cli/gateway.py`、`hermes_cli/main.py`、`hermes_cli/update_cmd_fleet.py`、`hermes_state_messages.py`、`plugins/platforms/feishu/adapter.py`、`pyproject.toml`、`tests/conftest.py`、`tests/gateway/test_config.py`、`tests/tools/test_skills_tool.py`、`tools/approval.py`、`tools/mcp_tool_discovery.py`、`tools/mcp_tool_handlers.py`、`tools/mcp_tool_registration.py`、`tools/mcp_tool_transport.py`、`tools/read_extract.py`、`tools/skill_manager_tool.py`、`tools/skills_tool.py`、`toolsets.py`、`uv.lock`、`website/docs/user-guide/configuration.md`。
  吸收矩阵：`PATCH-MCP-TASKS-ASYNC-HANDOFF`=未吸收；`PATCH-MCP-STDIO-WATCHER-LIFECYCLE`=部分吸收；`PATCH-SKILL-CREATE-ROOT`=部分吸收；`PATCH-UPDATE-FLEET-RECEIPT-FRESHNESS`=部分吸收；`PATCH-FEISHU-SOCKS-DEPENDENCY`=未吸收；`PATCH-FEISHU-GROUP-ADMISSION`=未吸收；`PATCH-FEISHU-MISSED-EVENT-BACKFILL`=未吸收；`PATCH-FEISHU-GROUP-SCOPE`=未吸收；`PATCH-PLATFORM-CAPABILITY-SCOPE`=未吸收；`PATCH-FEISHU-GROUP-APPROVAL`=未吸收；`PATCH-FEISHU-ADMIN-CONTROL-SCOPE`=未吸收；`PATCH-FEISHU-NORMAL-REPLY`=未吸收；`PATCH-FEISHU-QUOTE-CHAIN-SESSION`=未吸收；`PATCH-GATEWAY-FAILOVER-STATUS-SILENCE`=未吸收；`PATCH-LOCAL-PROFILES`=未吸收；`PATCH-FEISHU-RESOURCE-ACCESS`=未吸收；`PATCH-DOCUMENT-EXTRACTION`=部分吸收；`PATCH-FEISHU-MARKDOWN`=未吸收；`PATCH-FEISHU-RESPONSE-BUDGET`=未吸收；`PATCH-MODEL-CONFIGURED-ONLY`=未吸收；`PATCH-TEST-RUNTIME-STATE-ISOLATION`=未吸收；`PATCH-IMAGE-NATIVE-ROUTING`=未吸收；`PATCH-VERTEX-VIDEO-ROUTING`=未吸收；`PATCH-MULTIMODAL-SIDECAR`=未吸收；`PATCH-HISTORY-RETENTION`=未吸收；`PATCH-COMPACTION-LIFECYCLE-SILENCE`=完全吸收；`PATCH-LAUNCHD-WRAPPER-SUPERVISOR`=完全吸收；`PATCH-VERTEX-FALLBACK`=完全吸收；`PATCH-GEMINI-CUSTOM-NATIVE-BASE`=完全吸收；`PATCH-DASHBOARD-BUILD-CACHE`=完全吸收；无路径相交=22。全部 47 个 active 的吸收条件已重读；无路径相交项仍按语义判断，本轮上游没有提供其剩余本地职责的等价实现。
- 依赖：199 个既有 Python 包全部保留，仅 Hermes 0.21.1 → 0.21.2。官方 skill 更新 1 项并清理 1 条 manifest；wrapper 镜像 +1/~0/-2（删除 Wisdom 安装 skill 的文件和目录，补齐外部同名 llm-wiki 镜像），后续收敛无新的上游内容。npm audit fix 后仍为 11 项（2 moderate / 9 high / 0 critical），npm explain 均定位到 dev Desktop/build/test 链；Electron 40.10.2、electron-builder 26.15.3、Vitest 4.1.10 为精确 pin，修复要求越现有范围，归 P2 等待上游，不影响飞书主链路。lockfile 的传递版本升级、workspace hoist 与 peer 标记归一化由双 SHA review receipt 绑定。cryptography 50 与 Alibaba SDK <49 的 metadata 提示来自上游显式安全 override，保留安全 pin，不降级。
- 已知摩擦：三文件冲突已通过固定 SHA 恢复，npm stash 精确恢复。旧 env-only 测试契约已跟随 profile 配置语义修订；当前 checkout 全部 46 files / 2891 passed / 0 failed / 6 skipped，另有 2 个 test support modules，2897 collected、57/57 full PATCH evidence、22/22 probe、sandbox/identity-sync 161 passed；6 skips 为既有条件测试，均非 PATCH 唯一证据。与旧摘要 2623 passed 的差量为前一提交 owner 多 ID 矩阵 +228、升级配置矩阵 +3、上游退役 editorial 测试 -5、本次注册入口回归 +41、超时 event/watchdog 双路径净增 +1；没有测试文件丢失。本轮因上游 runtime identity/MCP/conftest 与 cleanup 生命周期变化执行深度审计，沿用并复验 74 项 auditor 负例和 22 项 cleanup 回归，未新增重复 gate。旧 Wisdom 数据继续私有保留，已移除模块的缓存白名单与对应 fixture 退役；Playbook 补入配置桥冲突恢复及定向 runner 路径/实际执行数量核对规则。首次 canonical 的旧断言失败已修复，完成状态以本轮 full final-audit JSON 为准。
- 配置漂移：官方迁移 v42 → v44，仅更新配置版本和 curator stale 30 → 14、archive 90 → 30 天；这些是官方缺省迁移，不回写旧值。已启用的本地技能维护与 my-skills 创建/隔离策略继续通过回归；不启用 Collective Wisdom 分享或额外 provider。Doctor config up-to-date、无 deprecated key、无 active security advisory；主模型/fallback/权限配置保持既有声明。ChatBI pre-dotenv 提示继续按正式加载后的布尔解析结果验收；未登录 provider 和未启用工具为 P3，可选路径不影响飞书主链路。
...
**上游吸收判断**：PID liveness 子项已由 upstream `98fce8e52d` / `2663117f72` / `ef46ec03e1` 完全吸收。剩余补丁仅在 upstream 的 stdio RPC 路径只实例化一次 child watcher、同一 awaitable 用于判定与调度，并让注册日志携带可由 runtime verifier 绑定的进程身份且有对应负向回归后可删除；当前 `28bf67b176` 仍缺少后两项，故继续保留最小 hunk。
...
**验证**：Step 8b 保留结构 sentinel；`scripts/test_patch_evidence.py::audit_socks_dependency` 用 TOML 解析精确检查 `project.optional-dependencies.feishu`，并在 Hermes venv 真实导入 `tools.lazy_deps.LAZY_DEPS["platform.feishu"]` 与 `python_socks`、核对安装 pin。字符串出现在注释或其他 feature 不算依赖声明。`scripts/test_patch_evidence_auditor.py::PatchEvidenceAuditorTest::test_socks_probe_requires_both_feishu_dependency_entries` 分别把 eager/lazy pin 移到无关 feature：旧探针两种均误绿，新探针拒绝；正确双声明正例保留。
...
**验证**：Step 8b grep 确认迁移脚本、英文文档与 zh-Hans 文档都不再出现 `HERMES_GATEWAY_TOKEN` / `gateway.auth.token` / `Gateway 认证 token`；`scripts/test_patch_evidence.py::audit_openclaw_token_migration` 在独立临时目录用同一真实 CLI 分别执行 dry-run 和 `--execute --include gateway-config`。报告不得暴露旧 token；目标 `.env` 不生成废弃变量且保留原值；`archive/gateway-config.json` 则必须完整保存来源 gateway 配置作为恢复证据。归档保留源数据不等于启用旧凭据，不能误写成“归档也不含 token”。`scripts/test_patch_evidence_auditor.py::PatchEvidenceAuditorTest::test_openclaw_probe_checks_execute_env_and_preserves_gateway_archive` 注入 dry-run 正常但 execute 写旧 env、以及丢失 archive 的两个假绿反例，并保留正确执行正例。
...
配置桥接跟随上游 profile 隔离：`allow_bots` 使用 scope-aware setter 并写入当前 adapter extra，本地 admission/history/attachment 键在同一返回字典合并。缺少 `allow_bots` 不能丢弃其他配置；`test_feishu_load_settings_prefers_env_over_profile_extra_allow_bots` 验证显式环境优先及 extra 回退，资源 PATCH 的根 YAML → adapter 回归另覆盖默认/独立 profile 和进程环境不污染。旧 env-only 假设已退役，群准入与 owner/DM 权限不变量保持。
...
| 字段     | 内容                                                                                                                                                                                                                                                                                                                  |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **文件** | `agent/{skill_utils.py,prompt_builder.py,skill_commands.py}`, `tools/{skills_tool.py,skills_tool_dedup.py,skills_tool_plugin.py}`, `hermes_cli/{config_defaults.py,prompt_size.py}`, `toolsets.py`, `tests/agent/test_skill_commands.py`, `tests/hermes_cli/test_skills_config.py`, `tests/tools/test_skills_tool.py` |
| **状态** | 🟡 未上游合并                                                                                                                                                                                                                                                                                                         |
...
### [PATCH-CODEX-WATCHDOG-TEST-CLOCK] Codex watchdog 回归独立于宿主调度和墙钟

| 字段     | 内容                                                                                                             |
| -------- | ---------------------------------------------------------------------------------------------------------------- |
| **文件** | `tests/agent/{test_auxiliary_client.py,test_codex_ttfb_watchdog.py}`（仅辅助 deadline 与主请求物理重连阶段回归） |
| **状态** | 🟡 未上游合并；仅测试补丁，生产超时预算不变                                                                      |

**问题**：旧回归用 30ms sleep 生成 keepalive，并要求整个 adapter.create 在 140ms 墙钟内结束；并行 suite 的准备/调度延迟也被计入，实际 TimeoutError 已发生仍会误红。本轮终审复现 2.26s，从而触发 canonical gate；不能靠重试或放宽上限消除这条失败。主请求的物理重连阶段回归另以 3s sleep/5s hard ceiling 假设宿主时钟与调度稳定，终审出现 151s elapsed 并走错误的 stale-kill 分支。两者共享“watchdog 测试只衡量 deadline/phase、不能依赖宿主时序”的不变量，归入同一测试补丁；与媒体路由、状态隔离分别可失效/回滚/上游吸收。

**修复**：仅在测试中替换 auxiliary 模块自己的单调时钟与 Timer 调度器，不改全局 time/threading。保持 50ms 请求预算，依次送入受控 30ms/60ms keepalive，必须在第二帧触发 no-progress TimeoutError；分别通过 per-event 检查和真实独立线程执行的 production watchdog callback 验证。后者仍走 stranger-thread 清理分支，断言 stream 释放、Timer 取消、client close 只由请求 owner 线程执行。5s join 只防测试线程悬挂，不用于放宽请求 deadline。主请求测试让真实 Codex event producer 与 monitor 共用局部受控时钟，并用 Event/真实 worker join 保证 monitor 至少检查一次虚拟 3s gap 后，健康流才继续；stall/零事件重连则先退休 token 再解除等待。2s TTFB/idle 与 5s hard ceiling 保持，10s rendezvous 仅作为测试同步失效保护。

**验证**：`tests/agent/test_auxiliary_client.py::TestCodexAuxiliaryAdapterTimeout::test_keepalive_stream_cannot_extend_no_progress_deadline` 的 event/watchdog 两个实例加入独立 Step 8b gate、full evidence 和 canonical。隔离 pytest plugin 将真实 on_event 错改为把 keepalive 也计作进展时，两条反例均失败；未注入时通过，并保留 timeout 传给 Responses.create 的正例。旧 140ms 墙钟断言退役，实际 50ms 语义和资源清理都没有削弱。`tests/agent/test_codex_ttfb_watchdog.py::test_event_stale_phase_is_scoped_to_physical_stream_attempt` 保留 initial_gap/stall/retry_gap/retry_no_event 四种真实流/重连路径；故障注入把 idle_requires_progress 置 false 时，三个 phase 反例失败而合法 stall 对照仍通过。两文件共六个实例进入同一 gate/full evidence；与辅助 keepalive-as-progress 注入合计五个失败反例。

**上游吸收判断**：上游采用等价的可控时钟/调度同步，在真实辅助/主请求 adapter 与 watchdog 上覆盖 keepalive 不延长 deadline、物理 retry phase 隔离、及时清理和 owner-thread FD 释放，且不再依赖亚秒级宿主墙钟上界后，可删除本测试 hunk 并归档。
