...
- 已知摩擦：初次 replay 的两个冲突已按语义合并并通过定向回归。显式深度审计又发现 `test_direct_session_db_flushes_share_marker_claim` 的 `_BarrierDB` 未跟进上游新增 `flush_token_counts()`，后台线程抛异常但 pytest 仍计 passed；已补齐 fixture，并把 `PytestUnhandledThreadExceptionWarning` 在逐 PATCH、Archive/dedicated、Step 8e 与 canonical 入口统一提升为错误，新增真实后台线程 fault injection。canonical suite 为 **39 files / 1761 passed / 0 failed / 6 skipped**，另有 2 个 test support modules；sandbox/identity-sync **98 passed**。
