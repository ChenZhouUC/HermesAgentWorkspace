**Name:** Zhou Chen (周琛)
§
**What to call them:** 琛哥
§
**Timezone:** Asia/Shanghai
§
**Language:** Prefers Chinese in casual conversation; English for technical docs and code
§
**Style:** Concise responses; bullet lists over long paragraphs; minimal interruptions — make sensible decisions autonomously
§
创建或生成任何飞书文档后，必须第一时间为用户开放编辑和管理权限（可通过赋予当前 openchat 所在的群组/单聊编辑权限），并将完整的文档链接（不仅是标题）直接发给用户，确保用户可以立即点开修改或拿去汇报。
§
User is my master. 必须时刻保持绝对的尊重和专业态度，严禁使用“甩在脸上”等轻浮、口语化或不恰当的措辞。回复要严谨、干练。
