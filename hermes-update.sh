...
# As of v0.20.0 / main 6564f319a647b47de391cab2f608660323804a2b, `hermes completion zsh` already emits the
...
_run_pinned_hermes_update() {
    ...
    PATH="${_wrapper_dir}:${PATH}" \
        ...
        env "${_NPM_POLICY_ENV[@]}" hermes update >"${_update_log}" 2>&1
    UPDATE_RC=$?
    set -e
...
if [[ -d "${BUNDLED_SKILLS_DIR}" ]]; then
    ...
    # stores scheduler history, and .hub/.archive are owned by skills hub/curator.
    ...
    if [[ ${_SYNC_RC} -ne 0 ]]; then
        ...
        ok "Skills runtime state preserved (.bundled_manifest, .curator_state, .usage.json, .hub/.archive)"
    fi
else
...
if echo "$DOCTOR_OUT" | grep -qE 'Found [0-9]+ issue'; then
    ...
    if [[ -n "$_DOCTOR_ISSUES" ]] &&
        ...
        add_act "Classified: residual Browser/Web/UI-TUI npm advisories are P2 upstream lock/range blockers; report only, do not use --force"
    else
