...
# Note: completions/_hermes (PATCH-4) is handled separately in step 6 via
...
if [[ $COMP_RC -eq 0 ]]; then
    ...
    # PATCH-4: fix invalid _arguments spec syntax generated by upstream.
...
with open(path, 'w') as f:
    f.write(content)
PYEOF
        ok "PATCH-4 applied: fixed _arguments invalid syntax in completion script"
    else
        ok "PATCH-4: upstream completion output already uses correct syntax — no fix needed"
    fi
