...
sources: [modelcontextprotocol.io, mcp-spec-2025-11-25, mcp-roadmap-2026-03-05, anthropic-blog, json-rpc-2.0, lsp-spec]
---

> **关于版本**：截至 2026-06-15，MCP 的当前稳定规范版本是 `2025-11-25`。本文以该版本为主，兼顾 2026-03 官方路线图中已经明确的工程方向。具体字段名、错误码、能力标志请以 modelcontextprotocol.io 的最新 schema 为准；本文不追求与某次微小修订完全字面一致，而是抽取**长期不变的工程要点**。
...
| 通知 | server→client 推送诊断信息                    | server→client 推送 list-changed、log、progress |
...
- **2026**：MCP 从"Anthropic 提出的协议"演化为多公司共同维护的开放标准，项目治理落在 LF Projects LLC 体系下；官方 MCP Registry 进入 preview；路线图把重点放在可扩展传输、无状态/可横向扩展部署、Server Cards、Tasks 生命周期、企业审计/鉴权/网关、conformance tests 上。

值得注意的是：协议的**核心面（三原语 + JSON-RPC + initialize 握手）**自始至终保持稳定，演化集中在传输层、鉴权、安全、可观测性、长任务和"长尾扩展能力"上。这意味着今天写的 MCP server 即便协议再迭代，主干概念可以保持兼容；但远程 server 的传输、OAuth 和安全实现必须跟随规范版本。
...
**关键区别**：Resources 是"等待被引用"的素材库。**不是**模型自己决定要读哪个 resource——而是 host UI / 用户 / 或者 host 内部的检索逻辑选定后把内容塞进上下文。这避免了模型在"先列资源 → 再读资源"上耗费多轮 turn。
...
2025-03 修订引入。核心改变：**单一 endpoint，由响应内容协商是否启用流式**。
...
- Streamable HTTP 是**HTTP/1.1 + SSE**，不依赖 WebSocket / HTTP/2 push，几乎所有反向代理都能跑。
- 但 SSE 流不能经过会"缓冲整条响应再返回"的中间件——常见坑是某些 CDN / nginx 默认配置会缓冲。
- Server 端若想水平扩展，必须尽量支持**无状态**或外置 session 状态（Redis / DB）。官方 2026 路线图把可扩展、无状态传输和 session 迁移列为重点方向，说明这是当前生产部署仍在演进的区域。
...
Model Context Protocol. "Specification 2025-11-25." Accessed June 2026. https://modelcontextprotocol.io/specification/2025-11-25/

Model Context Protocol. "Key Changes 2025-03-26 / 2025-06-18 / 2025-11-25." Accessed June 2026. https://modelcontextprotocol.io/specification/2025-11-25/changelog

Model Context Protocol. "Transports." Accessed June 2026. https://modelcontextprotocol.io/specification/2025-11-25/basic/transports

Model Context Protocol. "Lifecycle." Accessed June 2026. https://modelcontextprotocol.io/specification/2025-11-25/basic/lifecycle

Model Context Protocol. "Authorization." Accessed June 2026. https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization

Model Context Protocol. "Tools / Resources / Prompts." Accessed June 2026. https://modelcontextprotocol.io/specification/2025-11-25/server/tools

Model Context Protocol. "Tasks." Accessed June 2026. https://modelcontextprotocol.io/specification/2025-11-25/basic/utilities/tasks

Model Context Protocol. "Security Best Practices." Accessed June 2026. https://modelcontextprotocol.io/specification/2025-11-25/basic/security_best_practices

Model Context Protocol. "Roadmap." Last updated 2026-03-05. https://modelcontextprotocol.io/development/roadmap

Model Context Protocol. "The MCP Registry." Accessed June 2026. https://modelcontextprotocol.io/registry/about

Model Context Protocol. "Governance and Stewardship." Accessed June 2026. https://modelcontextprotocol.io/community/governance

Anthropic. "Introducing the Model Context Protocol." 2024-11-25. https://www.anthropic.com/news/model-context-protocol

OpenAI. "Model context protocol (MCP) - OpenAI Agents SDK." Accessed June 2026. https://openai.github.io/openai-agents-python/mcp/

Google. "MCP Tools - Agent Development Kit." Accessed June 2026. https://google.github.io/adk-docs/tools-custom/mcp-tools/
