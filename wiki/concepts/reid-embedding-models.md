...
- **Rank-N** + **mAP**：检索质量；
- **跨域泛化**：在 A 域训练、在 B 域评测时的性能掉点幅度——线下零售场景与学术数据集差异巨大，这一项决定能否实战可用。^[[[_living/Whale-SpaceSight/ReID-Embedding-Models|ReID-Embedding-Models]]]
...
ImageNet 与人体外观分布有巨大 gap；**领域内大规模无监督预训练**（LUPerson 系列）成为新的标准起点。CLIP 的视觉 - 语言对齐是另一条思路，强项是跨域泛化而非域内精度。^[[[_living/Whale-SpaceSight/ReID-Embedding-Models|ReID-Embedding-Models]]]
...
上述候选表与决策维度共同构成生产选型依据；最终结论必须由目标域测试验证。
