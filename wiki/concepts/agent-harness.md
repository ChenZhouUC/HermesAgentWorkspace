...
sources: [_living/AI-Infrastructure/AI-Agent-Harness-Engineering.md]
confidence: high
---
...
- **自主循环控制（Autonomous Loop）**：接管 [[chain-of-thought|ReAct]] 循环（Reason → Act → Observe）的底层调度
...
本仓库的 [[hermes-agent]]（及其前代 [[openclaw]]）是典型的 Harness 实现：

- **编排层**：事件驱动 + 连续对话调度（[[agent-mid-turn-input-modes]]）
- **上下文层**：技能库（skills）+ 记忆系统（memories）+ 工作区
- **沙盒层**：通过 MCP (Model Context Protocol) 隔离工具调用
- **HITL 层**：飞书 Bot 交互界面，支持实时干预
- **协议层**：[[markdown-llm-protocol|Markdown 协议]]作为人机交互载体
...
- [[agent-frameworks]] — Agent 框架选型全景
- [[hermes-agent]] — 本仓库的 Harness 实现
- [[openclaw]] — Hermes 的前代 Harness
- [[chain-of-thought]] — ReAct 循环是 Harness 的核心编排模式
- [[agent-mid-turn-input-modes]] — Hermes 的连续对话调度机制
- [[markdown-llm-protocol]] — Hermes 的人机交互协议层
