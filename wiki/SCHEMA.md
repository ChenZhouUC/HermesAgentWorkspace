...
- **Meta Pages:** `SCHEMA.md`、`index.md`、`log.md`。这些页面也必须有 frontmatter，但不参与 Layer 2 节点计数或本地文档图；它们之间以及其他页面指向它们的导航使用纯文本 / 代码路径，不使用 wikilink 或本地 Markdown link/embed。
...
- Active Layer 2 页面使用 `\[\[wikilinks\]\]` 建立语义边；不设最低出链数，所有边必须通过 `Evidence-Gated Linking`。0 条有据可依的边优于任何弱关联；指向 Layer 1（`_living/` / `raw/`）的溯源脚注不计入语义出链。
...
- 所有 Active Layer 2 页面必须注册到 `index.md` 的对应区块下，且**恰好出现一次**；登记行以带类型目录的行内代码路径开头（如 `entities/slug.md`），后接一句话摘要，不得使用 wikilink 或本地 Markdown link/embed，以免索引页成为语义图谱的伪中心节点
...
- **实体与流程解耦 (Entity-Process Decoupling)**：软件、硬件、理论等名词性对象提炼为 Entity 或 Concept；项目专属的完整操作手册、部署流程和业务流水线保留在 Layer 1。不得把来源文档一对一包装成节点；其中可跨场景复用的问题解决方法，只有满足下方 query 标准时才可独立提炼。
- **创建页面：** 对象必须是可独立定义和研究的主题，并有足够来源支撑其核心主张。多次出现只是发现候选的信号，不是建页的充分条件。
- **拆分页面：** 超过 200 行时触发拆分评估；只有存在可独立成立的子主题时才拆分，不为满足行数而破坏内聚性。
...
- **父子并存允许**：如果父节点（如某个框架）和子节点（如该框架的某个核心子模块）**双方独立都满足"整体研究对象"判据**，允许同时提取；只有来源能够证明父子关系时才建立 wikilink。
...
- **必须具备**：
  ...
  - 围绕解决问题所必需的 entity/concept 形成可追溯的方法链；每条连接都必须通过 `Evidence-Gated Linking`，不设连接数量目标。
...
### Evidence-Gated Linking (有证据才关联)

- **核心门槛**：没有显著、可追溯的证据，就不建立 wikilink。显著证据不要求固定来源数量，但必须足以让复核者指出一条明确陈述或可观察事实，而不是只接受 Agent 的主题联想。
- **先写关系谓词**：每条边必须在正文中说清关系是什么，例如“X 是 Y 的组成部分”“X 实现 Y”“X 生产的数据由 Y 消费”“X 是 Y 的抽象”“X 与 Y 是同一决策中的替代方案”。说不出具体谓词，就不应连接。
- **可接受证据**：来源文档中的直接陈述；正文中可复核的定义、证明或推导；可复核的代码、schema、API 或数据流事实；具有明确适用范围的标准、论文或官方文档；用户明确确认并已沉淀到 living source 的关系。关系句所在段落应带相邻溯源标记，或能从该页 `sources` 与正文位置直接定位证据。
- **不充分证据**：共享标签或关键词、同属一个大领域、仅在同一文档中出现、向量/词法相似、传递性猜测、反链对称、图连通率、中心性指标，以及“另一个页面已经链回来”。这些信号只能触发复核，不能单独产生边。
- **不设最低度数**：节点允许 0 条出边或入边。禁止为满足连接数、双向率、可视化完整度或页尾 `Related / 相关概念` 清单而补边；正文已经解释并连接目标时也不重复链接。
- **持续复核**：当证据被删除、否定、超出适用版本或无法重新定位时，应删除边或降为无链接的普通文字。证据存在争议时，先保留争议与来源，不把推断写成确定关系。

### Meta Graph Isolation

- Meta 集合固定为 `SCHEMA.md`、`index.md`、`log.md`。它们是运维载体，不是知识节点；在本地 Wiki 文档图中必须满足零度约束：$\operatorname{in\text{-}degree}=\operatorname{out\text{-}degree}=0$。
- 零度约束同时覆盖 Obsidian wikilink 与标准 Markdown 本地文档链接/嵌入，包括 `[[target]]`、`[label](target.md)` 和 `![label](target.md)`。三个 Meta 页面之间也不得互连，任何其他 Wiki 页面也不得指向它们。
- 外部 URL、同页 `#anchor`、纯文本路径和代码路径不构成本地文档图边。语法示例必须转义或放入代码块 / 行内代码，不能产生真实链接。
- `index.md` 使用带目录的代码路径登记 Active Layer 2 节点；`log.md` 和 `SCHEMA.md` 同样只用纯文本或代码路径提及 Wiki 文件。该约束由 lint 双向检查。
...
3. 仅添加通过 `Evidence-Gated Linking` 的已解析 Layer 2 出链；没有合格证据时允许 0 条出链
4. 注册到 `index.md`
...
15. `SCHEMA.md`、`index.md`、`log.md` 在本地 Wiki 文档图中必须同时保持零入边和零出边；lint 必须双向检查 wikilink 与标准 Markdown 本地文档 link/embed，`index.md` 仅用代码路径注册 Active Layer 2 节点
...
以上 18 项是机械一致性检查。`Evidence-Gated Linking` 的证据强度、关系谓词和适用范围必须人工审查；lint 通过不代表语义边已经获得充分证据。
...
3. 若新增规则属于证据强度、关系谓词、颗粒度、保守链接、实现细节过滤等不可可靠机械校验内容，必须在 `SCHEMA.md` 中明确标注为人工审查约束，不强行塞进 lint。
...
6. **图谱隔离**：日志正文只使用纯文本或代码路径记录节点与文件，不使用 wikilink 或标准 Markdown 本地文档 link/embed，也不接受其他页面以上述形式建立的入边；日志可审计性不得依赖图谱反向链接。
...
1. 先区分事实修正、版本演进、适用范围不同和真实观点冲突；不要把所有差异都视为“新内容覆盖旧内容”。
2. 比较来源是否为一手材料、方法是否可靠、版本与场景是否适用，再考虑发布日期。较新不自动等于更正确；证据质量和适用性优先于时间。
3. 已被高质量证据明确取代的事实应更新正文并保留可追溯来源；只在历史演进本身有价值时保留旧状态及其有效时间范围。
4. 对尚无定论的学术或工程争议，同时保留主要观点、证据、日期和适用条件，不强行合并为单一结论。
5. 只有 Active Layer 2 页面之间存在直接主张冲突时，才使用 `contradictions: [page-name]`；一般差异、互补关系或版本先后不标记为 contradiction。
...
4. **通过 Layer 2 溯源建立来源关系**：所有指向 living source 的溯源必须由 Active Layer 2（entities / concepts / comparisons / queries）页面通过**紧凑的内联脚注语法**（例如：`^[[[_living/xxx/xxx|显示别名]]]`）**单向、主动地指向** `_living/` 子层。
