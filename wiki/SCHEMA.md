...
4. **通过 Layer 2 溯源建立图谱关系**：所有的知识网状连线，必须由 Layer 2 (Concepts/Entities) 页面通过标准脚注的形式（例如正文使用 `[^1]`，文末使用 `[^1]: [[_living/xxx/xxx|xxx]]`）**单向、主动地指向** Layer 1，利用 Obsidian 的反向链接（Backlinks）机制在逻辑上实现双向关联。禁止在脚注中使用内联语法 `^[ [[...]] ]`，以防解析器冲突。
