...
4. **通过 Layer 2 溯源建立图谱关系**：所有的知识网状连线，必须由 Layer 2 (Concepts/Entities) 页面通过带双链的脚注溯源标记（例如 `^[ [[_living/xxx/xxx|xxx]] ]`）**单向、主动地指向** Layer 1，利用 Obsidian 的反向链接（Backlinks）机制在逻辑上实现双向关联。
